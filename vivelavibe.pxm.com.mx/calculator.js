?(function () {
  ...
  function openRoomGallery(images, title) {
    const list = Array.isArray(images) ? images.filter(Boolean) : [];
    if (!list.length) return;
    const overlayId = 'calcPhotoLightbox';
    ...
    if (!overlay) {
      ...
      overlay.innerHTML = `
        <button type="button" id="${overlayId}-prev" style="position:absolute;left:30px;font-size:32px;color:#fff;background:transparent;border:0;cursor:pointer">‹</button>
        <button type="button" id="${overlayId}-next" style="position:absolute;right:30px;font-size:32px;color:#fff;background:transparent;border:0;cursor:pointer">›</button>
        <button type="button" id="${overlayId}-close" style="position:absolute;right:30px;top:20px;font-size:32px;color:#fff;background:transparent;border:0;cursor:pointer">×</button>
        <div style="max-width:90vw;max-height:85vh;display:flex;align-items:center;justify-content:center">
          <img id="${overlayId}-img" style="max-width:90vw;max-height:80vh;border-radius:14px;box-shadow:0 18px 40px rgba(0,0,0,.5)" alt="">
        </div>`;
    ...
    let index = 0;
    const imgElement = document.getElementById(`${overlayId}-img`);
    const showImage = () => {
      imgElement.src = list[index];
      imgElement.alt = title || 'Galería';
    };
    document.getElementById(`${overlayId}-prev`).onclick = () => {
      index = (index - 1 + list.length) % list.length;
      showImage();
    };
    document.getElementById(`${overlayId}-next`).onclick = () => {
  ...
  function resolveRoomImages(room, fallback) {
  ...
  function buildRoomCard(room, options = {}) {
    ...
    const currency = room.currency || options.currency || state.currency;
    const showThumb = options.showThumbnail !== false;
    const images = showThumb ? resolveRoomImages(room, options.fallbackImages) : [];
    if (showThumb) {
      card.classList.add('has-thumb');
      if (images.length) {
        const thumb = document.createElement('button');
        thumb.type = 'button';
        thumb.className = 'calc-room-thumb';
        thumb.style.backgroundImage = `url("${images[0]}")`;
        thumb.setAttribute('aria-label', `Ver fotos de ${room.roomTypeName || room.lodgingName || 'habitacion'}`);
        thumb.addEventListener('click', () => openRoomGallery(images, room.roomTypeName || room.lodgingName));
        card.appendChild(thumb);
      } else {
        const filler = document.createElement('div');
        filler.className = 'calc-room-thumb empty';
        card.appendChild(filler);
      }
    }
    const perNight = safeNumber(room.pricePerNight) ?? safeNumber(room.subtotalPerNight) ?? null;
    const content = document.createElement('div');
    content.className = 'calc-room-content';
    content.innerHTML = `
      ...
      <p class="calc-room-meta">
        ${room.count > 1 ? `${room.count} unidades - ` : ''}
        ${perNight != null ? formatCurrency(perNight, currency) : 'Precio por definir'} por noche
      </p>
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .calc-room-card.has-thumb{
        grid-template-columns:92px 1fr;
        gap:14px;
        align-items:flex-start;
      }
      .calc-room-content{ display:grid; gap:8px; }
      .calc-room-thumb{
        width:92px;
        height:92px;
        border-radius:12px;
        ...
        background-color:color-mix(in srgb,var(--surface, #fffdf7) 90%, transparent);
        cursor:pointer;
        transition:border-color .15s ease, transform .12s ease;
      }
      .calc-room-thumb.empty{ background-image:linear-gradient(135deg, color-mix(in srgb,var(--muted, #badfdb) 45%, transparent), transparent); cursor:default; }
      .calc-room-thumb:not(.empty):hover{ border-color:var(--accent, #ffa4a4); transform:scale(1.02); }
      ...
      .calc-vive-panel{ display:flex; flex-direction:column; }
      .calc-vive-list{
        display:flex;
        flex-direction:column;
        gap:14px;
      ...
      .calc-vive-list::-webkit-scrollbar{ width:6px; }
      .calc-vive-list::-webkit-scrollbar-thumb{
        background:color-mix(in srgb,var(--muted, #badfdb) 60%, transparent);
        border-radius:8px;
      }
      .calc-vive-card{
        border:1px solid color-mix(in srgb,var(--muted, #badfdb) 45%, transparent);
        ...
        <div class="calc-body">
            ...
            <div class="calc-section-body calc-lodging-body">
              <div id="calcLodgingPanels" class="calc-tab-panels">
                ...
                <div id="calcVivePanel" class="calc-tab-panel calc-vive-panel">
  ...
  function prioritizeViveCombos(list) {
  ...
  function renderViveOffers() {
    ...
    if (state.viveLodgings.length > 1) {
      state.viveLodgings = prioritizeViveCombos(state.viveLodgings);
    }
    ...
    state.viveLodgings.forEach((offer, index) => {
      ...
      if (Array.isArray(offer.rooms) && offer.rooms.length) {
        offer.rooms.forEach((room) => {
          if (!room) return;
          const cardRoom = buildRoomCard(room, {
            currency: room.currency || currency,
            fallbackImages: offer.images
          });
  ...
  async function handleCalculate(button, texts) {
    ...
    try {
      ...
      if (hasTransformCombos) {
        const prioritized = prioritizeViveCombos(viveCombos);
        state.viveLodgings = prioritized;
  ...
  function ensureDefaultCheckInDate() {
    const input = document.querySelector('#bookerCard input[type="date"]');
    if (!input) return;
