...
export class ConnectionTestService {
  ...
  private async resolveConfig(
    ...
    const mergedInput = runtimeConfigInputSchema.parse({
      ...
      optimization: {
        cheapModeEnabled:
          candidateConfig.optimization?.cheapModeEnabled ??
          existing?.optimization.cheapModeEnabled ??
          true,
        debugModelOverride:
          candidateConfig.optimization?.debugModelOverride ??
          existing?.optimization.debugModelOverride ??
          "gpt-5-mini",
        disableBroadBrainSnapshots:
          candidateConfig.optimization?.disableBroadBrainSnapshots ??
          existing?.optimization.disableBroadBrainSnapshots ??
          true,
        skipFinalLlmForSimpleReads:
          candidateConfig.optimization?.skipFinalLlmForSimpleReads ??
          existing?.optimization.skipFinalLlmForSimpleReads ??
          true,
        logEstimatedCost:
          candidateConfig.optimization?.logEstimatedCost ??
          existing?.optimization.logEstimatedCost ??
          true,
        maxRecentConversationMessages:
          candidateConfig.optimization?.maxRecentConversationMessages ??
          existing?.optimization.maxRecentConversationMessages ??
          4,
        maxDocs:
          candidateConfig.optimization?.maxDocs ?? existing?.optimization.maxDocs ?? 4,
        maxDocsBundleBytes:
          candidateConfig.optimization?.maxDocsBundleBytes ??
          existing?.optimization.maxDocsBundleBytes ??
          24_000
      }
    });
    ...
    return {
      ...
      optimization: {
        cheapModeEnabled: mergedInput.optimization?.cheapModeEnabled ?? true,
        debugModelOverride: mergedInput.optimization?.debugModelOverride ?? "gpt-5-mini",
        disableBroadBrainSnapshots:
          mergedInput.optimization?.disableBroadBrainSnapshots ?? true,
        skipFinalLlmForSimpleReads:
          mergedInput.optimization?.skipFinalLlmForSimpleReads ?? true,
        logEstimatedCost: mergedInput.optimization?.logEstimatedCost ?? true,
        maxRecentConversationMessages:
          mergedInput.optimization?.maxRecentConversationMessages ?? 4,
        maxDocs: mergedInput.optimization?.maxDocs ?? 4,
        maxDocsBundleBytes: mergedInput.optimization?.maxDocsBundleBytes ?? 24_000
      },
