      ...
      >
        <form className="config-form" onSubmit={handleSubmit}>
          ...
          <p className="form-note">
            Cheap/debug mode is intended for low-cost local testing. Environment variables still win over these saved values.
          </p>
          <div className="form-grid">
            <label className="toggle">
              <input
                checked={form.optimization?.cheapModeEnabled ?? true}
                onChange={(event) =>
                  onChange({
                    ...form,
                    optimization: {
                      ...form.optimization,
                      cheapModeEnabled: event.target.checked
                    }
                  })
                }
                type="checkbox"
              />
              <span>Enable cheap/debug mode</span>
            </label>
            <label className="toggle">
              <input
                checked={form.optimization?.disableBroadBrainSnapshots ?? true}
                onChange={(event) =>
                  onChange({
                    ...form,
                    optimization: {
                      ...form.optimization,
                      disableBroadBrainSnapshots: event.target.checked
                    }
                  })
                }
                type="checkbox"
              />
              <span>Disable broad Brain snapshots</span>
            </label>
            <label className="toggle">
              <input
                checked={form.optimization?.skipFinalLlmForSimpleReads ?? true}
                onChange={(event) =>
                  onChange({
                    ...form,
                    optimization: {
                      ...form.optimization,
                      skipFinalLlmForSimpleReads: event.target.checked
                    }
                  })
                }
                type="checkbox"
              />
              <span>Skip final LLM on simple reads</span>
            </label>
            <label className="toggle">
              <input
                checked={form.optimization?.logEstimatedCost ?? true}
                onChange={(event) =>
                  onChange({
                    ...form,
                    optimization: {
                      ...form.optimization,
                      logEstimatedCost: event.target.checked
                    }
                  })
                }
                type="checkbox"
              />
              <span>Log estimated model cost</span>
            </label>
            <label>
              <span>Debug Model Override</span>
              <select
                className="input"
                value={form.optimization?.debugModelOverride ?? "gpt-5-mini"}
                onChange={(event) =>
                  onChange({
                    ...form,
                    optimization: {
                      ...form.optimization,
                      debugModelOverride: event.target.value
                    }
                  })
                }
              >
            ...
            <label>
              <span>Max recent messages (debug)</span>
              <input
                className="input"
                type="number"
                min={1}
                value={form.optimization?.maxRecentConversationMessages ?? 4}
                onChange={(event) =>
                  onChange({
                    ...form,
                    optimization: {
                      ...form.optimization,
                      maxRecentConversationMessages: Number(event.target.value)
                    }
                  })
                }
              />
            ...
            <label>
              <span>Max docs per prompt</span>
              <input
                className="input"
                type="number"
                min={1}
                value={form.optimization?.maxDocs ?? 4}
                onChange={(event) =>
                  onChange({
                    ...form,
                    optimization: {
                      ...form.optimization,
                      maxDocs: Number(event.target.value)
                    }
                  })
                }
              />
            ...
            <label>
              <span>Max docs bundle bytes</span>
              <input
                className="input"
                type="number"
                min={1000}
                step={1000}
                value={form.optimization?.maxDocsBundleBytes ?? 24000}
                onChange={(event) =>
                  onChange({
                    ...form,
                    optimization: {
                      ...form.optimization,
                      maxDocsBundleBytes: Number(event.target.value)
                    }
                  })
                }
              />
