...
if (-not (Test-Path (Join-Path $dataDir 'mysql'))) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Falló la inicialización de MariaDB local. Código: $LASTEXITCODE"
    }
}
...
if ($Reimport -or -not $dbExists) {
    ...
    $schemaExitCode = Import-SqlFile -ClientExe $clientExe -Host $dbHost -Port $dbPort -InputFile $schemaPath
    ...
    $seedExitCode = Import-SqlFile -ClientExe $clientExe -Host $dbHost -Port $dbPort -InputFile $seedPath
