...
if ($currentUser) {
    pms_require_permission('ota_ical.sync', null, true);
    ...
    if ($propertyCode !== '') {
        $propertyId = (int)pms_lookup_property_id_for_company($companyId, $propertyCode, false);
    }
}
...
try {
    ...
    if ($propertyId <= 0) {
        ...
        if ($propertyCode !== '') {
            if ($currentUser) {
                pms_require_property_access($propertyCode, true);
            }
            $propertyId = (int)pms_lookup_property_id_for_company($companyId, $propertyCode, false);
        }
    }
    if ($currentUser && $propertyId <= 0 && !pms_is_owner_user()) {
        $allowedCodes = pms_allowed_property_codes();
        if (count($allowedCodes) === 1) {
            $propertyId = (int)pms_lookup_property_id_for_company($companyId, $allowedCodes[0], false);
        } else {
            http_response_code(400);
            echo json_encode(array(
                'ok' => false,
                'error' => 'property_code is required for scoped users'
            ));
            exit;
        }
    }
