...
function rpm_require_action_permission($action)
{
    $readActions = array('list_modifiers', 'preview_night', 'preview_calendar');
    if (in_array((string)$action, $readActions, true)) {
        pms_require_permission('rateplans.view', null, true);
        return;
    }
    pms_require_permission('rateplans.edit', null, true);
}

function rpm_assert_property_access($propertyCode)
{
    $code = strtoupper(trim((string)$propertyCode));
    if ($code === '') {
        return;
    }
    pms_require_property_access($code, true);
}
...
rpm_require_action_permission($action);
...
try {
    if ($action === 'list_modifiers') {
        ...
        $owned = rpm_one($db, 'SELECT rp.id_rateplan, p.code AS property_code FROM rateplan rp JOIN property p ON p.id_property = rp.id_property WHERE rp.id_rateplan = ? AND p.id_company = ? AND rp.deleted_at IS NULL LIMIT 1', array($idRateplan, $companyId));
        if (!$owned) rpm_json(array('error' => 'rateplan_not_found'), 404);
        rpm_assert_property_access(isset($owned['property_code']) ? $owned['property_code'] : '');
    ...
    if ($action === 'upsert_modifier') {
        ...
        if ($idModifier > 0) {
            $row = rpm_one($db, 'SELECT rm.id_rateplan_modifier, p.code AS property_code FROM rateplan_modifier rm JOIN rateplan rp ON rp.id_rateplan = rm.id_rateplan JOIN property p ON p.id_property = rp.id_property WHERE rm.id_rateplan_modifier = ? AND p.id_company = ? LIMIT 1', array($idModifier, $companyId));
            if (!$row) rpm_json(array('error' => 'modifier_not_found'), 404);
            rpm_assert_property_access(isset($row['property_code']) ? $row['property_code'] : '');
    ...
    if ($action === 'upsert_schedule' || $action === 'delete_schedule') {
        ...
        $ownerSql = 'SELECT s.id_rateplan_modifier_schedule AS id_schedule, rm.id_rateplan_modifier AS id_modifier, p.code AS property_code
    ...
    if ($action === 'upsert_condition' || $action === 'delete_condition') {
        ...
        $ownerSql = 'SELECT c.id_rateplan_modifier_condition AS id_condition, rm.id_rateplan_modifier AS id_modifier, p.code AS property_code
                     FROM rateplan_modifier rm
                     JOIN rateplan rp ON rp.id_rateplan = rm.id_rateplan
                     JOIN property p ON p.id_property = rp.id_property
    ...
    if ($action === 'upsert_scope' || $action === 'delete_scope') {
        ...
        $ownerSql = 'SELECT sc.id_rateplan_modifier_scope AS id_scope, rm.id_rateplan_modifier AS id_modifier, p.code AS property_code
                     FROM rateplan_modifier rm
                     JOIN rateplan rp ON rp.id_rateplan = rm.id_rateplan
                     JOIN property p ON p.id_property = rp.id_property
    ...
    if ($action === 'preview_night' || $action === 'preview_calendar') {
        ...
        $owned = rpm_one($db, 'SELECT rp.id_rateplan, rp.id_property, p.code AS property_code FROM rateplan rp JOIN property p ON p.id_property = rp.id_property WHERE rp.id_rateplan = ? AND p.id_company = ? LIMIT 1', array($idRateplan, $companyId));
        if (!$owned) rpm_json(array('error' => 'rateplan_not_found'), 404);
        rpm_assert_property_access(isset($owned['property_code']) ? $owned['property_code'] : '');
        $idProperty = isset($owned['id_property']) ? (int)$owned['id_property'] : $idProperty;
