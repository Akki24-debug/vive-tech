...
pms_require_permission('incomes.view');
...
function incomes_apply($companyId, $lineItemId, $mode, $inputCents, $methodId, $notes, $actorUserId)
    {
        ...
        try {
            ...
            if ($propertyCode !== '' && !pms_is_owner_user()) {
                $allowed = pms_allowed_property_code_set();
                if (!isset($allowed[$propertyCode])) {
                    throw new Exception('No tienes acceso a la propiedad del ingreso seleccionado.');
                }
            }
...
if ($filters['incomes_property'] !== '') {
    pms_require_property_access($filters['incomes_property']);
}
...
$incomesAction = isset($_POST['incomes_action']) ? (string)$_POST['incomes_action'] : '';
if ($incomesAction !== '') {
    pms_require_permission('incomes.reconcile');
}
...
try {
    ...
    if (!pms_is_owner_user()) {
        $allowedPropertyCodes = pms_allowed_property_codes();
        if (!$allowedPropertyCodes) {
            $rows = array();
            $sql = '';
        } else {
            $inAllowed = implode(',', array_fill(0, count($allowedPropertyCodes), '?'));
            $sql .= ' AND p.code IN (' . $inAllowed . ')';
            foreach ($allowedPropertyCodes as $allowedCode) {
                $params[] = strtoupper((string)$allowedCode);
            }
        }
    }
    if ($sql === '') {
        throw new Exception('No tienes propiedades asignadas para consultar ingresos.');
    }
    ...
    if ($filters['incomes_search'] !== '') {
        $sql .= ' AND (
            ...
            OR COALESCE(g.maiden_name, "") LIKE ? OR COALESCE(g.email, "") LIKE ? OR COALESCE(li.item_name, lic.item_name, "") LIKE ?
