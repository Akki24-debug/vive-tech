...
if (!function_exists('ota_price_adjustment_mode_normalize')) {
    function ota_price_adjustment_mode_normalize($value)
    {
        $mode = strtolower(trim((string)$value));
        return in_array($mode, array('none', 'percent', 'fixed'), true) ? $mode : 'none';
    }
}

if (!function_exists('ota_has_price_adjustment_columns')) {
    function ota_has_price_adjustment_columns(PDO $pdo)
    {
        static $ready = null;
        if ($ready !== null) {
            return $ready;
...
if (!function_exists('ota_has_secondary_price_adjustment_column')) {
    function ota_has_secondary_price_adjustment_column(PDO $pdo)
    {
        static $ready = null;
        if ($ready !== null) {
            return $ready;
...
if (isset($_POST['ota_action']) && $_POST['ota_action'] === 'save_ota_account') {
    ...
    $otaPriceAdjustmentMode = ota_price_adjustment_mode_normalize(isset($_POST['ota_price_adjustment_mode']) ? (string)$_POST['ota_price_adjustment_mode'] : 'none');
    $otaPriceAdjustmentValueRaw = isset($_POST['ota_price_adjustment_value']) ? trim((string)$_POST['ota_price_adjustment_value']) : '';
    if ($otaPriceAdjustmentValueRaw === '') {
        $otaPriceAdjustmentMode = 'none';
    }
    $otaPriceAdjustmentValue = $otaPriceAdjustmentValueRaw !== ''
        ? (float)str_replace(',', '.', $otaPriceAdjustmentValueRaw)
        : 0.0;
    $otaSecondaryAdjustmentPctRaw = isset($_POST['ota_secondary_price_adjustment_pct']) ? trim((string)$_POST['ota_secondary_price_adjustment_pct']) : '';
    $otaSecondaryAdjustmentPct = $otaSecondaryAdjustmentPctRaw !== ''
        ? (float)str_replace(',', '.', $otaSecondaryAdjustmentPctRaw)
        : 0.0;
    ...
    } elseif ($otaPriceAdjustmentMode !== 'none' && !is_numeric(str_replace(',', '.', $otaPriceAdjustmentValueRaw))) {
        $error = 'El valor del ajuste OTA debe ser numerico.';
    } elseif ($otaSecondaryAdjustmentPctRaw !== '' && !is_numeric(str_replace(',', '.', $otaSecondaryAdjustmentPctRaw))) {
        $error = 'El modificador general OTA debe ser numerico.';
    } else {
        try {
            $pdoPricingCheck = pms_get_connection();
            if ($otaSecondaryAdjustmentPctRaw !== '' && (!function_exists('ota_has_secondary_price_adjustment_column') || !ota_has_secondary_price_adjustment_column($pdoPricingCheck))) {
                throw new Exception('La columna secondary_price_adjustment_pct aun no existe en la BD. Aplica la migracion migrate_ota_secondary_price_adjustment.sql antes de guardar este campo.');
            }
            ...
            if ($savedOtaId > 0) {
                ...
                if (function_exists('ota_has_price_adjustment_columns') && ota_has_price_adjustment_columns($pdoColor)) {
                    $secondarySetSql = (function_exists('ota_has_secondary_price_adjustment_column') && ota_has_secondary_price_adjustment_column($pdoColor))
                        ? 'secondary_price_adjustment_pct = ?,'
                        : '';
                    $stmtPricing = $pdoColor->prepare(
                        'UPDATE ota_account
                            SET price_adjustment_mode = ?,
                    ...
                    $pricingParams = array(
                        $otaPriceAdjustmentMode,
                        $otaPriceAdjustmentMode === 'none' ? null : $otaPriceAdjustmentValue,
                    );
                    if ($secondarySetSql !== '') {
                        $pricingParams[] = $otaSecondaryAdjustmentPctRaw === '' ? null : $otaSecondaryAdjustmentPct;
                    }
                    $pricingParams[] = $actorUserId;
                    $pricingParams[] = $savedOtaId;
                    $pricingParams[] = $companyId;
                    $stmtPricing->execute($pricingParams);
                }
        ...
        if (pms_ota_account_has_color_hex_column($pdoColor)) {
            $stmtColor = $pdoColor->prepare(
                'SELECT
                    ...
                    ' . (ota_has_price_adjustment_columns($pdoColor)
                        ? 'COALESCE(NULLIF(TRIM(price_adjustment_mode), \'\'), \'none\') AS price_adjustment_mode,
                           ...
                           ' . (ota_has_secondary_price_adjustment_column($pdoColor)
            ...
            foreach ($stmtColor->fetchAll() as $colorRow) {
                ...
                $extraById[$colorId] = array(
                    'color_hex' => isset($colorRow['color_hex']) ? (string)$colorRow['color_hex'] : '',
                    'price_adjustment_mode' => isset($colorRow['price_adjustment_mode']) ? (string)$colorRow['price_adjustment_mode'] : 'none',
                    'price_adjustment_value' => isset($colorRow['price_adjustment_value']) ? (float)$colorRow['price_adjustment_value'] : 0,
                    'secondary_price_adjustment_pct' => isset($colorRow['secondary_price_adjustment_pct']) ? (float)$colorRow['secondary_price_adjustment_pct'] : 0
                );
            }
            foreach ($otaAccounts as $idx => $otaRow) {
                $rowId = isset($otaRow['id_ota_account']) ? (int)$otaRow['id_ota_account'] : 0;
                if ($rowId > 0 && isset($extraById[$rowId])) {
                    $otaAccounts[$idx]['color_hex'] = $extraById[$rowId]['color_hex'];
                    $otaAccounts[$idx]['price_adjustment_mode'] = $extraById[$rowId]['price_adjustment_mode'];
                    $otaAccounts[$idx]['price_adjustment_value'] = $extraById[$rowId]['price_adjustment_value'];
                    $otaAccounts[$idx]['secondary_price_adjustment_pct'] = $extraById[$rowId]['secondary_price_adjustment_pct'];
                }
            }
        }
    }
...
if ($selectedTabId > 0 && isset($otaAccountsById[$selectedTabId])) {
    ...
    $otaForm['price_adjustment_mode'] = ota_price_adjustment_mode_normalize(isset($oa['price_adjustment_mode']) ? (string)$oa['price_adjustment_mode'] : 'none');
    $otaForm['price_adjustment_value'] = $otaForm['price_adjustment_mode'] === 'none'
        ? ''
        : (isset($oa['price_adjustment_value']) ? (string)$oa['price_adjustment_value'] : '');
    $otaForm['secondary_price_adjustment_pct'] = isset($oa['secondary_price_adjustment_pct']) && (float)$oa['secondary_price_adjustment_pct'] != 0.0
        ? (string)$oa['secondary_price_adjustment_pct']
        : '';
...
if (isset($_POST['ota_action']) && $_POST['ota_action'] === 'save_ota_account' && $error) {
    ...
    $otaForm['price_adjustment_mode'] = ota_price_adjustment_mode_normalize(isset($_POST['ota_price_adjustment_mode']) ? (string)$_POST['ota_price_adjustment_mode'] : 'none');
    $otaForm['price_adjustment_value'] = isset($_POST['ota_price_adjustment_value'])
        ? trim((string)$_POST['ota_price_adjustment_value'])
        : '';
    $otaForm['secondary_price_adjustment_pct'] = isset($_POST['ota_secondary_price_adjustment_pct'])
        ? trim((string)$_POST['ota_secondary_price_adjustment_pct'])
        : '';
...
<div class="panel">
  ...
  <form method="post" class="form-grid grid-2">
    ...
    <label>Precio especial OTA
      <select name="ota_price_adjustment_mode">
        <option value="none" <?php echo (string)$otaForm['price_adjustment_mode'] === 'none' ? 'selected' : ''; ?>>Usar precio operativo</option>
        <option value="percent" <?php echo (string)$otaForm['price_adjustment_mode'] === 'percent' ? 'selected' : ''; ?>>Aplicar porcentaje</option>
        <option value="fixed" <?php echo (string)$otaForm['price_adjustment_mode'] === 'fixed' ? 'selected' : ''; ?>>Sumar monto fijo</option>
      </select>
    </label>
    <label>Valor ajuste OTA
      <input type="number" step="0.01" name="ota_price_adjustment_value" value="<?php echo htmlspecialchars((string)$otaForm['price_adjustment_value'], ENT_QUOTES, 'UTF-8'); ?>" placeholder="Ej. 12.5 o 150.00">
    </label>
    <label>Modificador general OTA (%)
      <input type="number" step="0.01" name="ota_secondary_price_adjustment_pct" value="<?php echo htmlspecialchars((string)$otaForm['secondary_price_adjustment_pct'], ENT_QUOTES, 'UTF-8'); ?>" placeholder="Ej. -5 o 8.5">
    </label>
