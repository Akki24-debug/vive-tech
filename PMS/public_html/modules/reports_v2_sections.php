<?php
if (!function_exists('reports_v2_render_report_cell_content')) {
    function reports_v2_render_report_cell_content(array $cell, $wrapDisplaySpan = false)
    {
        $displayValue = isset($cell['display']) ? (string)$cell['display'] : '';
        $href = isset($cell['href']) ? trim((string)$cell['href']) : '';
        $openTag = $wrapDisplaySpan ? '<span class="report-v2-inline-cell-display">' : '';
        $closeTag = $wrapDisplaySpan ? '</span>' : '';
        if ($href !== '' && $displayValue !== '') {
            return $openTag
                . '<a class="report-v2-entity-link" href="' . reports_v2_h($href) . '">'
                . reports_v2_h($displayValue)
                . '</a>'
                . $closeTag;
        }
        return $openTag . reports_v2_h($displayValue) . $closeTag;
    }
}

if (!function_exists('reports_v2_render_inline_editor_control')) {
    function reports_v2_render_inline_editor_control($rowFormId, $fieldId, $inputMeta, $inputValue)
    {
        ...
        if ($inputType === 'select') {
            ...
            >
              <?php foreach ((isset($inputMeta['options']) && is_array($inputMeta['options'])) ? $inputMeta['options'] : array() as $optionRow): ?>
                <?php $optionValue = isset($optionRow['value']) ? (string)$optionRow['value'] : ''; ?>
                <option value="<?php echo reports_v2_h($optionValue); ?>" <?php echo (string)$inputValue === $optionValue ? 'selected' : ''; ?>>
                  <?php echo reports_v2_h(isset($optionRow['label']) ? (string)$optionRow['label'] : $optionValue); ?>
                </option>
              <?php endforeach; ?>
            </select>
            <?php
...
if (!function_exists('reports_v2_render_run_state_hidden_inputs')) {
    function reports_v2_render_run_state_hidden_inputs(array $ctx, $includeView = true)
    {
        ...
        <?php if (!empty($ctx['reportGridLineItemVisualization'])): ?>
            <input type="hidden" name="report_grid_line_item_visualization" value="<?php echo reports_v2_h((string)$ctx['reportGridLineItemVisualization']); ?>">
        <?php endif; ?>
        ...
        <?php endif; ?>
        <input type="hidden" name="report_grid_subdivide_sort_level_1" value="<?php echo reports_v2_h(isset($ctx['reportGridSubdivideSortLevel1']) ? (string)$ctx['reportGridSubdivideSortLevel1'] : 'asc'); ?>">
        <input type="hidden" name="report_grid_subdivide_sort_level_2" value="<?php echo reports_v2_h(isset($ctx['reportGridSubdivideSortLevel2']) ? (string)$ctx['reportGridSubdivideSortLevel2'] : 'asc'); ?>">
...
if (!function_exists('reports_v2_render_grouped_result_row')) {
    function reports_v2_render_grouped_result_row(array $reportRow, array $templateFields, array $ctx, array &$renderedEditableReservationIds, $indentLevel = 0, array $groupIds = array())
    {
        ...
        $rowKind = isset($reportRow['row_kind']) ? (string)$reportRow['row_kind'] : '';
        if ($rowKind !== '') {
            $rowClasses[] = 'report-v2-result-row--' . preg_replace('/[^a-z0-9_-]+/i', '-', $rowKind);
        }
        if ($rowIsEditing) {
        ...
        <tr <?php echo implode(' ', $rowAttributes); ?>>
          <?php foreach ($templateFields as $fieldRow): ?>
            ...
            ?>
            <td data-field-id="<?php echo $fieldId; ?>" data-cell-display="<?php echo reports_v2_h($cellDisplay); ?>" data-cell-filter-value="<?php echo reports_v2_h($cellFilterValue); ?>" data-cell-raw="<?php echo reports_v2_h(is_scalar($cellRaw) || $cellRaw === null ? (string)$cellRaw : ''); ?>" data-cell-type="<?php echo reports_v2_h(isset($cellMeta['data_type']) ? (string)$cellMeta['data_type'] : 'text'); ?>" data-cell-href="<?php echo reports_v2_h(isset($cell['href']) ? (string)$cell['href'] : ''); ?>" title="<?php echo reports_v2_h(isset($cell['error']) ? $cell['error'] : ''); ?>">
              <?php if ($canEditCell): ?>
                <?php
                ...
                ?>
                <?php echo reports_v2_render_report_cell_content($cell, true); ?>
                <?php reports_v2_render_inline_editor_control($rowFormId, $fieldId, $inputMeta, $inputValue); ?>
              <?php else: ?>
                <?php echo reports_v2_render_report_cell_content($cell); ?>
              <?php endif; ?>
            </td>
          <?php endforeach; ?>
...
if (!function_exists('reports_v2_render_group_marker_tbody')) {
    function reports_v2_render_group_marker_tbody(array $node, array $templateFields, $hasEditableReservationFields, $nodeId, $parentNodeId = '')
    {
        ...
        $groupFieldLabel = trim((string)(isset($node['label_kicker']) ? $node['label_kicker'] : ''));
        if ($groupFieldLabel === '') {
...
if (!function_exists('reports_v2_render_folio_summary_footer_row')) {
    function reports_v2_render_folio_summary_footer_row(array $summary, array $templateFields, $hasEditableReservationFields)
    {
        $colspan = count($templateFields) + ($hasEditableReservationFields ? 1 : 0);
        ?>
        <tr class="report-v2-folio-summary-row">
          <td colspan="<?php echo $colspan; ?>">
            <div class="report-v2-folio-summary-text">
              <strong>Cargos:</strong> <span><?php echo reports_v2_h(isset($summary['charges_display']) ? (string)$summary['charges_display'] : ''); ?></span>
              <strong>Pagos:</strong> <span><?php echo reports_v2_h(isset($summary['payments_display']) ? (string)$summary['payments_display'] : ''); ?></span>
              <strong>Balance:</strong> <span><?php echo reports_v2_h(isset($summary['balance_display']) ? (string)$summary['balance_display'] : ''); ?></span>
            </div>
          </td>
        </tr>
        <?php
...
foreach ($templates as $templateRow) {
    $categoryLabel = isset($templateRow['category_name']) ? trim((string)$templateRow['category_name']) : '';
    if ($categoryLabel === '') {
        $categoryLabel = 'Sin categoria';
    }
    if (!isset($templatesByCategory[$categoryLabel])) {
        $templatesByCategory[$categoryLabel] = array();
    }
    $templatesByCategory[$categoryLabel][] = $templateRow;
}
...
if (!empty($templateFields) && !empty($lineItemCatalogs) && is_array($lineItemCatalogs)) {
    foreach ($templateFields as $fieldRow) {
        $fieldId = isset($fieldRow['id_report_template_field']) ? (int)$fieldRow['id_report_template_field'] : 0;
        if ($fieldId <= 0) {
            continue;
...
?>
<style>
.report-v2-inline-total-row--level-2 td {
  background: linear-gradient(180deg, rgba(21, 44, 74, 0.96), rgba(16, 36, 62, 0.96));
  border-top: 1px solid rgba(56, 189, 248, 0.28);
  color: #f8fafc;
  font-weight: 800;
}
.report-v2-inline-total-row--level-2 td:first-child {
  padding-left: 22px;
  box-shadow: inset 4px 0 0 rgba(56, 189, 248, 0.9);
  position: relative;
}
.report-v2-inline-total-row--level-2 td:first-child::before {
  content: 'Seccion';
  display: block;
  margin-bottom: 2px;
...
.report-v2-inline-total-row--level-3 td {
  background: linear-gradient(180deg, rgba(13, 30, 52, 0.9), rgba(11, 24, 43, 0.9));
  border-top: 1px dashed rgba(94, 234, 212, 0.24);
  color: #dbeafe;
  font-weight: 700;
}
.report-v2-inline-total-row--level-3 td:first-child {
  padding-left: 44px;
  box-shadow: inset 4px 0 0 rgba(45, 212, 191, 0.7);
  position: relative;
}
.report-v2-inline-total-row--level-3 td:first-child::before {
  content: 'Subseccion';
  display: block;
  margin-bottom: 2px;
...
.report-v2-inline-total-row--level-3 td:empty {
  background-color: rgba(11, 24, 43, 0.9);
}
</style>
...
<div class="report-v2-grid">
  <section class="report-v2-card report-v2-stack report-v2-card--full">
    ...
    <div class="report-v2-list">
      ...
      <?php else: ?>
        <?php foreach ($templatesByCategory as $templateCategoryLabel => $categoryTemplates): ?>
          <div class="report-v2-stack" style="gap:12px;">
            <div class="report-v2-muted" style="font-size:12px;text-transform:uppercase;letter-spacing:.08em;"><?php echo reports_v2_h($templateCategoryLabel); ?></div>
            <?php foreach ($categoryTemplates as $templateRow): ?>
              ...
              <div class="report-v2-list-item <?php echo $templateId === $selectedTemplateId ? 'is-selected' : ''; ?>">
                ...
                <div class="report-v2-pill-list">
                  ...
                  <span class="report-v2-chip"><?php echo reports_v2_h($templateCategoryLabel); ?></span>
                  ...
                  <?php if (in_array((isset($templateRow['row_source']) ? (string)$templateRow['row_source'] : 'reservation'), array('line_item', 'combined'), true) && !empty($templateRow['line_item_type_scope'])): ?>
      ...
      <div class="report-v2-card report-v2-stack">
        ...
        <form method="post" class="report-v2-stack">
          ...
          <?php
            ...
            $selectedTemplateCategoryName = $selectedTemplate && isset($selectedTemplate['category_name'])
                ? (string)$selectedTemplate['category_name']
                : '';
          ?>
          <div class="report-v2-form-grid report-v2-form-grid--4">
            ...
            <label><span>Categoria</span><input type="text" name="template_category_name" value="<?php echo reports_v2_h($selectedTemplateCategoryName); ?>" placeholder="Ejemplo: Contabilidad, Operaciones, Ventas"></label>
            ...
            <label data-template-row-source-panel="line_item,combined"><span>Tipo de line item</span><select name="template_line_item_type_scope"><?php foreach ($lineItemRowTypeOptions as $typeCode => $typeLabel): ?><option value="<?php echo reports_v2_h($typeCode); ?>" <?php echo $selectedTemplateLineItemTypeScope === $typeCode ? 'selected' : ''; ?>><?php echo reports_v2_h($typeLabel); ?></option><?php endforeach; ?></select></label>
          </div>
          <div class="report-v2-form-grid report-v2-form-grid--4">
        ...
        <?php if ($selectedTemplate): ?>
          <div class="report-v2-actions">
            ...
            <form method="post" onsubmit="return confirm('Se clonara la plantilla seleccionada con sus campos y filtros guardados.');"><input type="hidden" name="reports_action" value="clone_template"><input type="hidden" name="reports_tab" value="templates"><input type="hidden" name="selected_report_calculation_id" value="<?php echo $selectedCalculationId; ?>"><input type="hidden" name="template_id" value="<?php echo (int)$selectedTemplate['id_report_template']; ?>"><button type="submit">Clonar</button></form>
        ...
        <div class="report-v2-card report-v2-stack">
          ...
          <form method="post" class="report-v2-stack">
            ...
            <div class="report-v2-form-grid" data-field-panel="reservation">
              <label>
                <span>Campo de reservacion</span>
                <select name="reservation_field_code" id="reports-v2-reservation-field-code">
                  <?php
                  $fieldGroups = array();
                  foreach ($reservationFieldCatalog as $fieldCode => $fieldMeta) {
                      $groupName = isset($fieldMeta['group']) ? (string)$fieldMeta['group'] : 'Otros';
                      if (!isset($fieldGroups[$groupName])) {
                          $fieldGroups[$groupName] = array();
                      }
                      $fieldGroups[$groupName][$fieldCode] = $fieldMeta;
                  }
                  $orderedFieldGroups = array();
                  if (isset($fieldGroups['Entidades'])) {
                      $orderedFieldGroups['Entidades'] = $fieldGroups['Entidades'];
                      unset($fieldGroups['Entidades']);
                  }
                  foreach ($fieldGroups as $groupName => $groupFields) {
                      $orderedFieldGroups[$groupName] = $groupFields;
                  }
                  foreach ($orderedFieldGroups as $groupName => $groupFields):
                  ?>
                    <optgroup label="<?php echo reports_v2_h($groupName); ?>">
                      <?php foreach ($groupFields as $fieldCode => $fieldMeta): ?>
                        <option value="<?php echo reports_v2_h($fieldCode); ?>" <?php echo $selectedReservationFieldCode === $fieldCode ? 'selected' : ''; ?>>
                          <?php echo reports_v2_h($fieldMeta['label']); ?>
                        </option>
                      <?php endforeach; ?>
                    </optgroup>
                  <?php endforeach; ?>
            ...
            <div class="report-v2-form-grid report-v2-form-grid--3" data-field-panel="line_item" style="display:none;">
              ...
              <label><span>Line item principal</span><select name="line_item_catalog_id"><?php if (in_array($selectedTemplateForFieldRowSource, array('line_item', 'combined'), true)): ?><option value="0" <?php echo $selectedLineItemCatalogId <= 0 ? 'selected' : ''; ?>>Registro actual</option><?php endif; ?><?php foreach ($lineItemCatalogs as $catalogRow): $optionLabel = trim((string)$catalogRow['item_name']); if ($optionLabel === '') { $optionLabel = 'Line item #' . (int)$catalogRow['id_line_item_catalog']; } ?><option value="<?php echo (int)$catalogRow['id_line_item_catalog']; ?>" <?php echo $selectedLineItemCatalogId === (int)$catalogRow['id_line_item_catalog'] ? 'selected' : ''; ?>><?php echo reports_v2_h($optionLabel); ?></option><?php endforeach; ?></select></label>
            </div>
            <div class="report-v2-form-grid report-v2-form-grid--3" data-field-panel="line_item" style="display:none;">
...
<?php
$editableInputCatalog = function_exists('reports_v2_prepare_editable_input_catalog')
    ? reports_v2_prepare_editable_input_catalog($pdo, $companyId)
    : array();
?>
...
<section class="report-v2-card report-v2-stack">
  <?php if ($reportsManageMode): ?>
    <div>
      <h2>Ejecucion</h2>
      <p class="report-v2-muted">Aqui defines la vista preliminar y los filtros por defecto de la plantilla. El reporte real se abre desde el menu lateral de reportes.</p>
      <div class="report-v2-tabs" style="margin-top:12px;">
        ...
        <a class="report-v2-tab" href="<?php echo reports_v2_h('?view=reports&reports_tab=templates&reports_manage=1&selected_report_template_id=' . $selectedTemplateId); ?>">Plantillas</a>
        <a class="report-v2-tab" href="<?php echo reports_v2_h('?view=reports&reports_tab=calculations&reports_manage=1&selected_report_template_id=' . $selectedTemplateId . '&selected_report_calculation_id=' . $selectedCalculationId); ?>">Calculos</a>
      </div>
    </div>
    <?php if (!$selectedTemplate): ?>
      <p class="report-v2-muted">Selecciona una plantilla en `Plantillas` para configurar su ejecucion.</p>
    <?php else: ?>
      <div class="report-v2-list-item">
        <h4><?php echo reports_v2_h($selectedTemplate['report_name']); ?></h4>
        <div class="report-v2-pill-list">
          <?php if (!empty($selectedTemplate['category_name'])): ?><span class="report-v2-chip"><?php echo reports_v2_h($selectedTemplate['category_name']); ?></span><?php endif; ?>
          <span class="report-v2-chip"><?php echo reports_v2_h(isset($rowSourceOptions[$selectedTemplate['row_source']]) ? $rowSourceOptions[$selectedTemplate['row_source']] : 'Reservacion'); ?></span>
          <?php if (in_array((isset($selectedTemplate['row_source']) ? (string)$selectedTemplate['row_source'] : 'reservation'), array('line_item', 'combined'), true) && !empty($selectedTemplate['line_item_type_scope'])): ?>
            <span class="report-v2-chip"><?php echo reports_v2_h(isset($lineItemRowTypeOptions[$selectedTemplate['line_item_type_scope']]) ? $lineItemRowTypeOptions[$selectedTemplate['line_item_type_scope']] : $selectedTemplate['line_item_type_scope']); ?></span>
          <?php endif; ?>
          <span class="report-v2-chip"><?php echo count($templateFields); ?> campos</span>
        </div>
      </div>
      <form method="get" class="report-v2-stack" data-report-template-run-form="1">
        <input type="hidden" name="view" value="reports">
        <input type="hidden" name="reports_tab" value="run">
        <input type="hidden" name="reports_manage" value="1">
        <input type="hidden" name="selected_report_template_id" value="<?php echo (int)$selectedTemplateId; ?>">
        <input type="hidden" name="selected_report_calculation_id" value="<?php echo (int)$selectedCalculationId; ?>">
        <input type="hidden" name="template_id" value="<?php echo (int)$selectedTemplateId; ?>">
        <input type="hidden" name="report_default_grid_state_json" id="report-v2-template-grid-state-json" value="">
        <div class="report-v2-form-grid">
          <label><span>Propiedad por defecto</span><select name="report_property_code"><option value="">Todas</option><?php foreach ($properties as $propertyRow): $propertyCode = isset($propertyRow['property_code']) ? (string)$propertyRow['property_code'] : ''; $propertyName = isset($propertyRow['property_name']) ? (string)$propertyRow['property_name'] : $propertyCode; ?><option value="<?php echo reports_v2_h($propertyCode); ?>" <?php echo strtoupper($runFilters['property_code']) === strtoupper($propertyCode) ? 'selected' : ''; ?>><?php echo reports_v2_h($propertyName); ?></option><?php endforeach; ?></select></label>
          <label><span>Estatus por defecto</span><select name="report_status"><option value="">Todos</option><option value="activas" <?php echo $runFilters['status'] === 'activas' ? 'selected' : ''; ?>>Activas</option><?php foreach (array('apartado', 'confirmado', 'en casa', 'salida', 'no-show', 'cancelada') as $statusOption): ?><option value="<?php echo reports_v2_h($statusOption); ?>" <?php echo $runFilters['status'] === $statusOption ? 'selected' : ''; ?>><?php echo reports_v2_h($statusOption); ?></option><?php endforeach; ?></select></label>
          <label><span>Tipo de fecha por defecto</span><select name="report_date_type"><?php foreach ($dateTypeOptions as $dateTypeCode => $dateTypeLabel): ?><option value="<?php echo reports_v2_h($dateTypeCode); ?>" <?php echo $runFilters['date_type'] === $dateTypeCode ? 'selected' : ''; ?>><?php echo reports_v2_h($dateTypeLabel); ?></option><?php endforeach; ?></select></label>
          <label><span>Rango por defecto</span><div class="pms-date-range-picker report-v2-inline-range" data-pms-date-range-picker><button type="button" class="pms-date-range-picker-trigger" data-pms-date-range-trigger><span data-pms-date-range-display-start><?php echo reports_v2_h($runFilters['date_from']); ?></span><span class="pms-date-range-picker-separator">y</span><span data-pms-date-range-display-end><?php echo reports_v2_h($runFilters['date_to']); ?></span></button><input type="hidden" name="report_date_from" value="<?php echo reports_v2_h($runFilters['date_from']); ?>" data-pms-date-range-start><input type="hidden" name="report_date_to" value="<?php echo reports_v2_h($runFilters['date_to']); ?>" data-pms-date-range-end></div></label>
        </div>
        <div class="report-v2-actions">
          <button type="submit">Aplicar vista preliminar</button>
          <button type="submit" name="reports_action" value="save_template_run_filters" formmethod="post">Guardar filtros en plantilla</button>
          <a class="report-v2-chip" href="<?php echo reports_v2_h('?view=reports&reports_tab=templates&reports_manage=1&selected_report_template_id=' . (int)$selectedTemplateId); ?>">Editar plantilla</a>
          <a class="report-v2-chip" href="<?php echo reports_v2_h('?view=reports&reports_tab=run&selected_report_template_id=' . (int)$selectedTemplateId . '&report_property_code=' . urlencode($runFilters['property_code']) . '&report_status=' . urlencode($runFilters['status']) . '&report_date_type=' . urlencode($runFilters['date_type']) . '&report_date_from=' . urlencode($runFilters['date_from']) . '&report_date_to=' . urlencode($runFilters['date_to'])); ?>">Abrir reporte real</a>
        </div>
      </form>
      <div class="report-v2-form-grid report-v2-form-grid--4">
        <div class="report-v2-list-item"><h4>Filas preliminares</h4><div class="report-v2-muted">Maximo 500 por corrida</div><div style="font-size:28px;font-weight:700;"><?php echo count($reportRows); ?></div></div>
        <div class="report-v2-list-item"><h4>Columnas visibles</h4><div style="font-size:28px;font-weight:700;"><?php echo count(array_filter($templateFields, function ($fieldRow) { return !isset($fieldRow['is_visible']) || !empty($fieldRow['is_visible']); })); ?></div></div>
        <div class="report-v2-list-item"><h4>Objeto por fila</h4><div style="font-size:20px;font-weight:700;"><?php echo reports_v2_h(isset($rowSourceOptions[$selectedTemplate['row_source']]) ? $rowSourceOptions[$selectedTemplate['row_source']] : 'Reservacion'); ?></div></div>
        <div class="report-v2-list-item"><h4>Fecha activa</h4><div style="font-size:16px;font-weight:700;"><?php echo reports_v2_h(isset($dateTypeOptions[$runFilters['date_type']]) ? $dateTypeOptions[$runFilters['date_type']] : $runFilters['date_type']); ?></div><div class="report-v2-muted"><?php echo reports_v2_h($runFilters['date_from']); ?> a <?php echo reports_v2_h($runFilters['date_to']); ?></div></div>
      </div>

      <div class="report-v2-stack">
        <div class="report-v2-section-heading">
          <h3>Grid preliminar</h3>
          <p class="report-v2-muted">Vista previa de los resultados con los filtros actuales de la plantilla.</p>
        </div>
        <div class="report-v2-table-wrap">
          <?php if (empty($templateFields)): ?>
          ...
          <?php elseif (!empty($reportSubdivisions)): ?>
            <?php
              $groupedRenderContext = array(
                  ...
                  'reportGridSubdivideSortLevel1' => isset($reportGridSubdivideSortLevel1) ? (string)$reportGridSubdivideSortLevel1 : 'asc',
                  'reportGridSubdivideSortLevel2' => isset($reportGridSubdivideSortLevel2) ? (string)$reportGridSubdivideSortLevel2 : 'asc',
            ...
            <table class="report-v2-table report-v2-result-table" data-result-scope="main">
              ...
              <tbody>
                <?php foreach ($reportRows as $reportRow): ?>
                  <?php reports_v2_render_grouped_result_row($reportRow, $templateFields, array(
                      'selectedTemplateId' => $selectedTemplateId,
                      'selectedCalculationId' => $selectedCalculationId,
                      'runFilters' => $runFilters,
                      'reportGridSubdivideFieldId' => $reportGridSubdivideFieldId,
                      ...
                      'editableInputCatalog' => $editableInputCatalog,
                      'reportHasCalculatedTotals' => $reportHasCalculatedTotals,
                  ), $renderedEditableReservationIds); ?>
                <?php endforeach; ?>
              </tbody>
              <?php if ($reportHasCalculatedTotals): ?>
                <tfoot><tr><?php foreach ($templateFields as $fieldRow): $fieldId = isset($fieldRow['id_report_template_field']) ? (int)$fieldRow['id_report_template_field'] : 0; $totalCell = isset($reportDisplayTotals[$fieldId]) ? $reportDisplayTotals[$fieldId] : array('display' => ''); ?><td data-total-field-id="<?php echo $fieldId; ?>"><?php echo reports_v2_h(isset($totalCell['display']) ? $totalCell['display'] : ''); ?></td><?php endforeach; ?><?php if ($hasEditableReservationFields): ?><td></td><?php endif; ?></tr></tfoot>
  ...
  <?php else: ?>
    ...
    <form method="get" class="report-v2-stack">
      ...
      <input type="hidden" name="report_grid_subdivide_sort_level_1" value="<?php echo reports_v2_h($reportGridSubdivideSortLevel1); ?>">
      <input type="hidden" name="report_grid_subdivide_sort_level_2" value="<?php echo reports_v2_h($reportGridSubdivideSortLevel2); ?>">
      ...
      <?php if (!empty($reportLineItemVisualizationAvailable)): ?><input type="hidden" name="report_grid_line_item_visualization" value="<?php echo reports_v2_h((string)$reportGridLineItemVisualization); ?>"><?php endif; ?>
      <div class="report-v2-form-grid">
        ...
        <label><span>Plantilla</span><select name="selected_report_template_id"><?php foreach ($templatesByCategory as $templateCategoryLabel => $categoryTemplates): ?><optgroup label="<?php echo reports_v2_h($templateCategoryLabel); ?>"><?php foreach ($categoryTemplates as $templateRow): ?><option value="<?php echo (int)$templateRow['id_report_template']; ?>" <?php echo (int)$templateRow['id_report_template'] === $selectedTemplateId ? 'selected' : ''; ?>><?php echo reports_v2_h($templateRow['report_name']); ?></option><?php endforeach; ?></optgroup><?php endforeach; ?></select></label>
      </div>
      <div class="report-v2-actions"><button type="submit">Ejecutar</button><span class="report-v2-chip">Maximo 500 <?php echo ($runTemplate && isset($runTemplate['row_source']) && (string)$runTemplate['row_source'] === 'line_item') ? 'line items' : 'reservaciones'; ?> base por corrida</span><?php if ($runTemplate): ?><span class="report-v2-chip"><?php echo reports_v2_h($runTemplate['report_name']); ?></span><span class="report-v2-chip"><?php echo reports_v2_h(isset($rowSourceOptions[$runTemplate['row_source']]) ? $rowSourceOptions[$runTemplate['row_source']] : 'Reservacion'); ?></span><?php if (in_array((string)$runTemplate['row_source'], array('line_item', 'combined'), true) && !empty($runTemplate['line_item_type_scope'])): ?><span class="report-v2-chip"><?php echo reports_v2_h(isset($lineItemRowTypeOptions[$runTemplate['line_item_type_scope']]) ? $lineItemRowTypeOptions[$runTemplate['line_item_type_scope']] : $runTemplate['line_item_type_scope']); ?></span><?php endif; ?><?php endif; ?></div>
    </form>

    <div class="report-v2-grid-filter-bar report-v2-stack">
      <?php if ($hasTemplateSubdivideColumn || !empty($reportLineItemVisualizationAvailable)): ?>
      <form method="get" class="report-v2-grid-division-form">
        ...
        <?php if (!empty($reportLineItemVisualizationAvailable)): ?>
          <div class="report-v2-grid-division-block">
            <label>
              <span>Visualizacion</span>
              <select name="report_grid_line_item_visualization" class="report-v2-auto-submit-division-filter">
                <?php foreach ($reportLineItemVisualizationOptions as $visualizationCode => $visualizationLabel): ?>
                  <option value="<?php echo reports_v2_h($visualizationCode); ?>" <?php echo $reportGridLineItemVisualization === $visualizationCode ? 'selected' : ''; ?>><?php echo reports_v2_h($visualizationLabel); ?></option>
                <?php endforeach; ?>
              </select>
            </label>
            <?php if ($reportGridLineItemVisualization === 'folio'): ?>
              <span class="report-v2-chip">Agrupa por folio, deja pagos al final y calcula cargos vs pagos vs balance</span>
            <?php endif; ?>
          </div>
        <?php endif; ?>
        <?php if ($reportGridLineItemVisualization !== 'folio'): ?>
          <div class="report-v2-grid-division-block">
            <label>
              <span class="report-v2-grid-division-heading">
                <span>Primera division</span>
                <button type="button" class="report-v2-division-sort-button <?php echo $reportGridSubdivideSortLevel1 === 'desc' ? 'is-desc' : ''; ?>" data-division-sort-level="1" data-division-sort-direction="<?php echo reports_v2_h($reportGridSubdivideSortLevel1); ?>" title="Cambiar orden de primera division" <?php echo $reportSubdivideByFieldId > 0 ? '' : 'disabled'; ?>>
                  <?php echo $reportGridSubdivideSortLevel1 === 'desc' ? '&darr;' : '&uarr;'; ?>
                </button>
              </span>
          ...
          <div class="report-v2-grid-division-block">
            <label>
              <span class="report-v2-grid-division-heading">
                ...
                <button type="button" class="report-v2-division-sort-button <?php echo $reportGridSubdivideSortLevel2 === 'desc' ? 'is-desc' : ''; ?>" data-division-sort-level="2" data-division-sort-direction="<?php echo reports_v2_h($reportGridSubdivideSortLevel2); ?>" title="Cambiar orden de segunda division" <?php echo $reportSubdivideByFieldIdLevel2 > 0 ? '' : 'disabled'; ?>>
                  <?php echo $reportGridSubdivideSortLevel2 === 'desc' ? '&darr;' : '&uarr;'; ?>
                </button>
              </span>
      ...
      <div class="report-v2-grid-toolbar">
        ...
        <div class="report-v2-grid-toolbar-actions">
          <?php if ($runTemplate && !empty($reportRows)): ?>
            <?php
              $exportBaseQuery = array(
                  'selected_report_template_id' => $selectedTemplateId,
                  'report_property_code' => $runFilters['property_code'],
                  'report_status' => $runFilters['status'],
                  'report_date_type' => $runFilters['date_type'],
                  'report_date_from' => $runFilters['date_from'],
                  'report_date_to' => $runFilters['date_to'],
              );
              if ($reportGridSubdivideFieldId > 0) {
                  $exportBaseQuery['report_grid_subdivide_field_id'] = $reportGridSubdivideFieldId;
              }
              if (!empty($reportGridSubdivideFieldIdLevel2)) {
                  $exportBaseQuery['report_grid_subdivide_field_id_level_2'] = (int)$reportGridSubdivideFieldIdLevel2;
              }
              if (!empty($reportGridSubdivideFieldIdLevel3)) {
                  $exportBaseQuery['report_grid_subdivide_field_id_level_3'] = (int)$reportGridSubdivideFieldIdLevel3;
              }
              $exportBaseQuery['report_grid_subdivide_sort_level_1'] = $reportGridSubdivideSortLevel1;
              $exportBaseQuery['report_grid_subdivide_sort_level_2'] = $reportGridSubdivideSortLevel2;
              if ($reportGridSubdivideShowTotalsLevel1 !== null) {
                  $exportBaseQuery['report_grid_subdivide_show_totals_level_1'] = $reportGridSubdivideShowTotalsLevel1 ? '1' : '0';
              }
              if ($reportGridSubdivideShowTotalsLevel2 !== null) {
                  $exportBaseQuery['report_grid_subdivide_show_totals_level_2'] = $reportGridSubdivideShowTotalsLevel2 ? '1' : '0';
              }
              if (!empty($reportLineItemVisualizationAvailable)) {
                  $exportBaseQuery['report_grid_line_item_visualization'] = (string)$reportGridLineItemVisualization;
              }
              $excelExportUrl = 'api/report_v2_export.php?' . http_build_query($exportBaseQuery + array('format' => 'excel'));
              $googleExportUrl = 'api/report_v2_export.php?' . http_build_query($exportBaseQuery + array('format' => 'csv'));
            ?>
            <button type="button" class="report-v2-export-button" id="report-v2-export-excel" data-export-url="<?php echo reports_v2_h($excelExportUrl); ?>">Excel</button>
            <button type="button" class="report-v2-export-button" id="report-v2-export-google" data-export-url="<?php echo reports_v2_h($googleExportUrl); ?>">Google Sheets/Drive (CSV)</button>
          <?php endif; ?>
      ...
      <?php elseif (!empty($reportSubdivisions)): ?>
        <?php
          $groupedRenderContext = array(
              ...
              'reportGridSubdivideShowTotalsLevel1' => isset($reportGridSubdivideShowTotalsLevel1) ? $reportGridSubdivideShowTotalsLevel1 : null,
              'reportGridSubdivideShowTotalsLevel2' => isset($reportGridSubdivideShowTotalsLevel2) ? $reportGridSubdivideShowTotalsLevel2 : null,
              'reportGridLineItemVisualization' => isset($reportGridLineItemVisualization) ? (string)$reportGridLineItemVisualization : 'standard',
              'hasEditableReservationFields' => $hasEditableReservationFields,
              'editReservationId' => $editReservationId,
              'rowEditDraftValues' => $rowEditDraftValues,
              'editableInputCatalog' => $editableInputCatalog,
            ...
            <div class="report-v2-stack report-v2-result-subdivision">
              ...
              <table class="report-v2-table report-v2-result-table" data-result-scope="subdivision">
                ...
                <?php else: ?>
                  <?php reports_v2_render_grouped_rows_tbody($reportSubdivision['rows'], $templateFields, $groupedRenderContext, $renderedEditableReservationIds); ?>
                <?php endif; ?>
                <?php if (($reportHasCalculatedTotals && !empty($reportSubdivision['show_totals'])) || !empty($reportSubdivision['footer_summary'])): ?>
                  <tfoot>
                    <?php if ($reportHasCalculatedTotals && !empty($reportSubdivision['show_totals'])): ?>
                      <tr><?php foreach ($templateFields as $fieldRow): $fieldId = isset($fieldRow['id_report_template_field']) ? (int)$fieldRow['id_report_template_field'] : 0; $totalCell = isset($reportSubdivision['totals'][$fieldId]) ? $reportSubdivision['totals'][$fieldId] : array('display' => ''); ?><td data-total-field-id="<?php echo $fieldId; ?>"><?php echo reports_v2_h(isset($totalCell['display']) ? $totalCell['display'] : ''); ?></td><?php endforeach; ?><?php if ($hasEditableReservationFields): ?><td></td><?php endif; ?></tr>
                    <?php endif; ?>
                    <?php if (!empty($reportSubdivision['footer_summary'])): ?>
                      <?php reports_v2_render_folio_summary_footer_row($reportSubdivision['footer_summary'], $templateFields, $hasEditableReservationFields); ?>
                    <?php endif; ?>
                  </tfoot>
                <?php endif; ?>
              </table>
