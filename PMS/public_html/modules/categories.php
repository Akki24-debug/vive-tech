...
function categories_guest_full_name(array $row)
{
    $parts = array(
    ...
    return isset($row['guest_full_name']) ? trim((string)$row['guest_full_name']) : '';
}

function categories_format_short_date($value)
{
    $raw = trim((string)$value);
...
<?php if ($categories): ?>
  <?php
    ...
    $liveReservationByRoomId = array();
    $liveBlockByRoomId = array();
    $roomIdsForOccupancy = array();
    foreach ($roomsList as $roomTmp) {
        $roomIdTmp = isset($roomTmp['id_room']) ? (int)$roomTmp['id_room'] : 0;
        if ($roomIdTmp > 0) {
            $roomIdsForOccupancy[$roomIdTmp] = true;
        }
    }
    if ($roomIdsForOccupancy) {
        try {
            $pdo = isset($pdo) ? $pdo : pms_get_connection();
            $roomIdList = array_keys($roomIdsForOccupancy);
            $roomIdPlaceholders = implode(',', array_fill(0, count($roomIdList), '?'));
            ...
            $reservationStmt = $pdo->prepare(
                'SELECT
                 ...
                 WHERE r.id_room IN (' . $roomIdPlaceholders . ')
                   AND p.id_company = ?
                   AND r.deleted_at IS NULL
                   AND r.status IN (\'apartado\', \'confirmado\', \'en casa\')
                   AND r.check_in_date <= ?
            ...
            $reservationStmt->execute(array_merge($roomIdList, array($companyId, $todayDate, $todayDate)));
            foreach ($reservationStmt->fetchAll() as $reservationRow) {
                $roomIdTmp = isset($reservationRow['id_room']) ? (int)$reservationRow['id_room'] : 0;
                if ($roomIdTmp <= 0 || isset($liveReservationByRoomId[$roomIdTmp])) {
                    continue;
                }
                $liveReservationByRoomId[$roomIdTmp] = $reservationRow;
            }
            ...
            $blockStmt = $pdo->prepare(
                'SELECT
                    ...
                    rb.end_date
                 FROM room_block rb
                 JOIN property p ON p.id_property = rb.id_property
                 WHERE rb.id_room IN (' . $roomIdPlaceholders . ')
                   AND p.id_company = ?
            ...
            $blockStmt->execute(array_merge($roomIdList, array($companyId, $todayDate, $todayDate)));
            foreach ($blockStmt->fetchAll() as $blockRow) {
                $roomIdTmp = isset($blockRow['id_room']) ? (int)$blockRow['id_room'] : 0;
                if ($roomIdTmp <= 0 || isset($liveReservationByRoomId[$roomIdTmp]) || isset($liveBlockByRoomId[$roomIdTmp])) {
                    continue;
                }
                $liveBlockByRoomId[$roomIdTmp] = $blockRow;
            }
        } catch (Exception $e) {
            $error = $error ? $error : $e->getMessage();
        }
    ...
    // Precompute occupancy per category using live reservations / blocks for today
    ...
    foreach ($roomsList as $room) {
        ...
        $reservationMeta = ($roomId > 0 && isset($liveReservationByRoomId[$roomId])) ? $liveReservationByRoomId[$roomId] : null;
        $blockMeta = ($roomId > 0 && isset($liveBlockByRoomId[$roomId])) ? $liveBlockByRoomId[$roomId] : null;
        $status = isset($room['status']) ? strtolower(trim((string)$room['status'])) : '';
        $isOccupied = ($reservationMeta !== null || $blockMeta !== null);
        if ($isOccupied) {
              ...
              <div class="category-room-list">
                <?php foreach ($roomsForCategory as $room): ?>
                  <?php
                    ...
                    $roomReservationMeta = isset($room['reservation_meta']) && is_array($room['reservation_meta']) ? $room['reservation_meta'] : null;
                    $roomBlockMeta = isset($room['block_meta']) && is_array($room['block_meta']) ? $room['block_meta'] : null;
                    $roomStatusRaw = isset($room['status']) ? trim((string)$room['status']) : '';
                    $roomGuestName = $roomReservationMeta ? categories_guest_full_name($roomReservationMeta) : '';
                    $roomCheckoutInfo = $roomReservationMeta && !empty($roomReservationMeta['check_out_date'])
                        ? categories_format_short_date($roomReservationMeta['check_out_date'])
                        : '';
                    $roomCheckInInfo = $roomReservationMeta && !empty($roomReservationMeta['check_in_date'])
                        ? categories_format_short_date($roomReservationMeta['check_in_date'])
                        : '';
                    if ($roomReservationMeta) {
                        $roomStatusText = 'Ocupada';
                        $roomStatusRaw = isset($roomReservationMeta['status']) ? trim((string)$roomReservationMeta['status']) : 'occupied';
                    } elseif ($roomBlockMeta) {
                        $roomStatusText = 'Bloqueada';
                        $roomStatusRaw = 'blocked';
                    } else {
                        if ($roomStatusRaw === '') {
                            $roomStatusRaw = 'available';
                        }
                        $roomStatusText = 'Disponible';
                    }
                    $roomStatusToneClass = $roomReservationMeta ? 'is-reservation' : ($roomBlockMeta ? 'is-blocked' : 'is-available');
                    $roomStatusDetail = '';
                    if ($roomReservationMeta) {
                        $reservationCodeInfo = isset($roomReservationMeta['reservation_code']) ? trim((string)$roomReservationMeta['reservation_code']) : '';
                        $roomStatusDetail = $roomGuestName !== '' ? $roomGuestName : 'Reserva activa';
                        if ($reservationCodeInfo !== '') {
                            $roomStatusDetail .= ' / ' . $reservationCodeInfo;
                        }
                    } elseif ($roomBlockMeta) {
                        $blockCodeInfo = isset($roomBlockMeta['block_code']) ? trim((string)$roomBlockMeta['block_code']) : '';
                        $blockDescriptionInfo = isset($roomBlockMeta['description']) ? trim((string)$roomBlockMeta['description']) : '';
                        $roomStatusDetail = $blockCodeInfo !== '' ? $blockCodeInfo : 'Bloqueo';
                        if ($blockDescriptionInfo !== '') {
                            $roomStatusDetail .= ' / ' . $blockDescriptionInfo;
                        }
                    } else {
                        $roomStatusDetail = 'Sin reserva activa';
                    }
                    $roomStatusMeta = '';
                    if ($roomReservationMeta) {
                        $statusLabelInfo = isset($roomReservationMeta['status']) ? ucfirst(trim((string)$roomReservationMeta['status'])) : 'Activa';
                        $roomStatusMeta = $statusLabelInfo;
                        if ($roomCheckInInfo !== '' || $roomCheckoutInfo !== '') {
                            $roomStatusMeta .= ' / ' . trim($roomCheckInInfo . ' - ' . $roomCheckoutInfo, ' -');
                        }
                    } elseif ($roomBlockMeta) {
                        $blockStartInfo = !empty($roomBlockMeta['start_date']) ? categories_format_short_date($roomBlockMeta['start_date']) : '';
                        $blockEndInfo = !empty($roomBlockMeta['end_date']) ? categories_format_short_date($roomBlockMeta['end_date']) : '';
                        $roomStatusMeta = trim($blockStartInfo . ' - ' . $blockEndInfo, ' -');
                    }
                  ?>
                  <form method="post" action="index.php?view=rooms" class="category-room-card <?php echo $roomIsOccupied ? 'is-occupied' : 'is-available'; ?>">
                    ...
                    <button type="submit" class="category-room-link">
                      <span class="category-room-main">
                        <span class="category-room-title"><?php echo htmlspecialchars($roomCode . ($roomName !== '' ? ' | ' . $roomName : ''), ENT_QUOTES, 'UTF-8'); ?></span>
                        <span class="category-room-detail"><?php echo htmlspecialchars($roomStatusDetail, ENT_QUOTES, 'UTF-8'); ?></span>
                        <?php if ($roomStatusMeta !== ''): ?>
                          <span class="category-room-meta"><?php echo htmlspecialchars($roomStatusMeta, ENT_QUOTES, 'UTF-8'); ?></span>
                        <?php endif; ?>
                      </span>
                      <span class="category-room-status <?php echo htmlspecialchars($roomStatusToneClass, ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars($roomStatusText, ENT_QUOTES, 'UTF-8'); ?></span>
                    </button>
                  </form>
                <?php endforeach; ?>
              </div>
...
.category-room-status.is-reservation {
  background: rgba(127, 29, 29, 0.6);
  color: #ffe4e6;
}
.category-room-status.is-blocked {
  background: rgba(120, 53, 15, 0.64);
  color: #fde68a;
}
.category-room-status.is-available {
