...
$otaInfoHasAliasColumn = false;

try {
    $pdo = pms_get_connection();
    $stmt = $pdo->query("SHOW COLUMNS FROM ota_account_info_catalog LIKE 'display_alias'");
    $otaInfoHasAliasColumn = (bool)$stmt->fetch(PDO::FETCH_ASSOC);
} catch (Exception $e) {
    $otaInfoHasAliasColumn = false;
}
...
try {
    $pdo = pms_get_connection();
    $aliasSelectSql = $otaInfoHasAliasColumn ? 'oaic.display_alias' : 'CAST(NULL AS CHAR) AS display_alias';
    $stmt = $pdo->prepare(
        'SELECT
            oaic.id_ota_account,
...
if (!function_exists('reservations_build_return_url')) {
    function reservations_build_return_url(array $filters, $reservationId = 0)
    {
        ...
        $url = 'index.php?view=reservations';
        if ($reservationId > 0) {
            $url .= '&open_reservation=' . $reservationId;
        }
        return $url;
    }
}
    ...
    if ($propertyCode === '' || $roomCode === '' || $checkIn === '' || $checkOut === '') {
        ...
        try {
            ...
            if ($createdId) {
                ...
                $paymentConceptOptions = reservations_payment_catalogs_for_property($paymentCatalogsByProperty, $propertyCode);
                ...
                $needsFolio = !empty($paymentQueue) || !empty($filteredExtraConcepts);
                $folioId = 0;
                if ($needsFolio) {
                    if ($createdRow && isset($createdRow['id_folio'])) {
                        $folioId = (int)$createdRow['id_folio'];
                    }
                    if ($folioId <= 0) {
                        try {
                            $db = pms_get_connection();
                            $stmt = $db->prepare('SELECT id_folio FROM folio WHERE id_reservation = ? AND deleted_at IS NULL ORDER BY id_folio DESC LIMIT 1');
                            $stmt->execute(array($createdId));
                            $folioRow = $stmt->fetch(PDO::FETCH_ASSOC);
                            if ($folioRow && isset($folioRow['id_folio'])) {
                                $folioId = (int)$folioRow['id_folio'];
                            }
                        } catch (Exception $e) {
                            $financeErrors[$createdId] = 'No se pudo obtener el folio para registrar cargos/pagos: ' . $e->getMessage();
                        }
                    }
                    if ($folioId <= 0 && !isset($financeErrors[$createdId])) {
                        try {
                            ...
                            $folioId = (int)$db->lastInsertId();
                        } catch (Exception $e) {
                            $financeErrors[$createdId] = 'No se pudo crear el folio para registrar cargos/pagos: ' . $e->getMessage();
                        }
                    }
                }

                if ($folioId > 0 && !empty($filteredExtraConcepts)) {
                    $createdCode = '';
                ...
                } elseif ($needsFolio && $folioId <= 0 && !isset($financeErrors[$createdId])) {
                    $financeErrors[$createdId] = 'No se encontro un folio para registrar cargos/pagos.';
                }

                if ($folioId > 0 && !empty($paymentQueue) && !isset($financeErrors[$createdId])) {
                ...
                /* Cerrar pestaÃ±a de â€œNueva reservaâ€ y limpiar su estado */
...
if ($action === 'create_sale_item' || $action === 'update_sale_item' || $action === 'delete_sale_item') {
    ...
    /* Normalizar datos desde catÃ¡logo en altas */
    ...
    if ($reservationId > 0 && ($folioId > 0 || $saleId > 0)) {
        try {
            ...
            $financeMessages[$reservationId] = $actionCode === 'create'
                ? 'Cargo agregado.'
                : ($actionCode === 'update'
                    ? 'Cargo actualizado y derivados recalculados.'
                    : ($skipPercentRecalcOnDelete === 1 ? 'Derivado eliminado.' : 'Cargo eliminado junto con sus derivados.'));
            $_POST[$moduleKey . '_subtab_action'] = 'open';
            $_POST[$moduleKey . '_subtab_target'] = 'dynamic:reservation:' . $reservationId;
            $clearDirtyTargets[] = 'dynamic:reservation:' . $reservationId;
        } catch (Exception $e) {
...
if ($action === 'create_payment' || $action === 'update_payment' || $action === 'delete_payment') {
    ...
    if ($reservationId > 0 && ($folioId > 0 || $paymentId > 0)) {
        try {
            ...
            $paymentConceptOptions = reservations_payment_catalogs_for_property($paymentCatalogsByProperty, $propertyCodeForReservation);
            $paymentConceptOptionMap = array();
            foreach ($paymentConceptOptions as $opt) {
                $optId = isset($opt['id_payment_catalog']) ? (int)$opt['id_payment_catalog'] : 0;
                if ($optId > 0) {
...
if ($redirectAfterSaveUrl === '' && $returnView === 'calendar') {
    ...
    if (in_array($action, $calendarReturnActions, true) && !$hasPostActionErrors) {
        $redirectAfterSaveUrl = reservations_build_return_url($filters);
    }
}
...
/* Si se pidiÃ³ abrir una pestaÃ±a, forzarla como activa y agregarla a open */
...
/* Normaliza pestaÃ±a activa para evitar quedarse en tabs inexistentes (ej. static:new removida) */
$activeKey = isset($subtabState['active']) ? (string)$subtabState['active'] : 'static:general';

/* Si llegamos desde el calendario pidiendo abrir la pestaÃ±a de nueva reserva */
...
.reservation-folios-card > summary {
  list-style: none;
  cursor: pointer;
  padding: 12px 14px;
}
.reservation-folios-card > summary::-webkit-details-marker {
  display: none;
}
      ...
      <tbody>
        ...
        ?>
          <tr class="<?php echo $isOpen ? 'is-selected' : ''; ?>">
            ...
            <td><?php echo htmlspecialchars(isset($row['guest_email']) ? (string)$row['guest_email'] : '', ENT_QUOTES, 'UTF-8'); ?></td>
            <td>
              <form method="post">
                <?php pms_subtabs_form_state_fields($moduleKey, $subtabState); ?>
  ...
  <?php
    ...
    if ($createProperty !== '' && $lodgingAllowedMap) {
        foreach ($conceptsByProperty[$createProperty] as $c) {
            ...
            $childDefault = $child ? (int)$child['default_unit_price_cents'] : 0;
            $childTaxRuleIds = '';
            $lodgingOptions[] = array(
                'id' => $cid,
                'label' => ($cat !== '' ? $cat . ' / ' : '') . $label,
    ...
    if ($createProperty !== '' && isset($conceptsByProperty[$createProperty])) {
        foreach ($conceptsByProperty[$createProperty] as $c) {
            ...
            $extraConceptOptions[] = array(
                'id' => $cid,
                'label' => ($cat !== '' ? $cat . ' / ' : '') . $label,
                'default_cents' => isset($c['default_unit_price_cents']) ? (int)$c['default_unit_price_cents'] : 0
            );
    ...
    if ($createProperty !== '' && $newReservationValues['room_code'] !== '' && $newReservationValues['check_in'] !== '' && $newReservationValues['check_out'] !== '') {
        try {
            ...
            if ($nights > 0) {
                $rateplanSets = pms_call_procedure('sp_rateplan_calendar', array(
                    $createProperty,
                    '',
                    '',
                ...
                $rateRows = isset($rateplanSets[0]) ? $rateplanSets[0] : array();
                $sum = 0;
...
foreach ($openReservationIds as $reservationId) {
    ...
    $isVisibleSaleItemInFolio = function ($siRow) use ($saleItemsById, $addToFatherMap, $showInFolioRelationMap) {
        ...
        if ($parentSaleId > 0 && !isset($saleItemsById[$parentSaleId])) {
        ...
        if ($parentSaleId > 0) {
            ...
            if ($relationKey !== '' && isset($addToFatherMap[$relationKey])) {
                if ((int)$addToFatherMap[$relationKey] !== 0) {
                    return false;
                }
            } elseif ($addToFatherRow !== null && $addToFatherRow !== 0) {
                return false;
            }
    ...
    $summaryHasCharges = $hasVisibleSaleItems;
    ...
    $hasFolios = !empty($folios);
    $statusEditLocked = (!$hasGuestAssigned || !$summaryHasCharges);
    $missingRequirementsForStatus = array();
    if (!$hasGuestAssigned) {
        $missingRequirementsForStatus[] = 'hu&eacute;sped';
    }
    if (!$summaryHasCharges) {
    ...
    $summaryRoomCodeForHeader = $roomCodeDetail !== '' ? $roomCodeDetail : (isset($detail['room_code']) ? (string)$detail['room_code'] : '');
    ...
    if ($isHold) {
        ...
        $headerAlertWizardStep = '1';
    } elseif (!$hasFolios && in_array($summaryStatusNormalized, array('confirmado', 'en casa'), true)) {
        $headerAlertVisible = true;
        $headerAlertText = 'Reservaci&oacute;n incompleta';
        $headerAlertButton = 'Agregar cargos';
        $headerAlertClass = '';
        $headerAlertWizardStep = '2';
    }
    ...
    if ($bestScoreOtaId <= 0 && $reservationId > 0 && $otaLodgingCatalogByCatalogId) {
        try {
            ...
            $stmtOtaDetect = $pdoOtaDetect->prepare(
                'SELECT li.id_line_item_catalog
                 FROM line_item li
                 JOIN folio f
                   ON f.id_folio = li.id_folio
            ...
            $stmtOtaDetect->execute(array((int)$reservationId));
    ...
    if ($bestScoreOtaId > 0) {
        ...
        if ($detailOtaId > 0 && isset($otaMetaById[$detailOtaId])) {
            $otaDetectedId = $detailOtaId;
            $otaDetectedByLodging = false;
        }
    }

    if ($otaDetectedId > 0 && isset($otaInfoCatalogsByAccount[$otaDetectedId])) {
        foreach ($otaInfoCatalogsByAccount[$otaDetectedId] as $cfg) {
        ...
        foreach ($otaInfoSourceRows as $otaInfoRow) {
            ...
            $serviceDate = isset($otaInfoRow['service_date']) ? trim((string)$otaInfoRow['service_date']) : '';
            if ($serviceDate !== '') {
    ...
    <div class="reservation-detail-header reservation-detail-header-enhanced">
      <div class="reservation-detail-title">
        <h3 class="reservation-title-main">
          ...
          <?php endif; ?>
          <?php if ($summaryRoomCodeForHeader !== ''): ?>
            <span class="reservation-title-room"><?php echo htmlspecialchars($summaryRoomCodeForHeader, ENT_QUOTES, 'UTF-8'); ?></span>
          <?php endif; ?>
        </h3>
      </div>
      ...
      <div class="reservation-detail-header-right">
        ...
        <div class="reservation-detail-balance">
          <span class="label">Balance</span>
          <span class="value"><?php echo htmlspecialchars(reservations_format_money($summaryBalanceCents, $summaryCurrency), ENT_QUOTES, 'UTF-8'); ?></span>
        </div>
      </div>
    </div>
    <?php if ($detailError): ?>
    ...
    <?php else: ?>
      ...
      <?php
        ...
        foreach ($saleItems as $nightSaleRow) {
            ...
            if (isset($isVisibleSaleItemInFolio) && is_callable($isVisibleSaleItemInFolio)) {
                if (!$isVisibleSaleItemInFolio($nightSaleRow)) {
                    continue;
                }
            }
            ...
            $nightlyAmountsByDate[$nightDate] += (int)$finalTotalForSaleItem($nightSaleId);
        }

        if ($nightlyAmountsByDate) {
            ksort($nightlyAmountsByDate);
            foreach ($nightlyAmountsByDate as $nightDate => $nightAmountCents) {
                $nightlyRows[] = array(
                    'date' => $nightDate,
        ...
        $availableRoomsForCategory = $availableRooms;
      ?>
      ...
      <div class="reservation-summary-card" data-res-summary-card>
        <form method="post" class="reservation-summary-inline-form" data-res-summary-form>
          ...
          <input type="hidden" name="reservation_property_code" value="<?php echo htmlspecialchars($propertyCodeDetail, ENT_QUOTES, 'UTF-8'); ?>">
          ...
          <div class="reservation-summary-grid">
            ...
            <div class="reservation-summary-field">
              ...
              >
                ...
                <?php foreach ($statusOptionsForEdit as $option): ?>
                  ...
                  ?>
                  <option value="<?php echo htmlspecialchars($option, ENT_QUOTES, 'UTF-8'); ?>" <?php echo $selectedStatus ? 'selected' : ''; ?>>
            ...
            <div class="reservation-summary-field">
              ...
              <span class="value"><?php echo htmlspecialchars($summaryProperty !== '' ? $summaryProperty : '--', ENT_QUOTES, 'UTF-8'); ?></span>
            </div>
            <div class="reservation-summary-field">
            ...
            <div class="reservation-summary-field">
              ...
              <select name="reservation_room_code" data-summary-editable data-field-edit disabled>
                <option value="">(sin cambio)</option>
                <?php foreach ($availableRoomsForCategory as $room):
                  ...
                  $baseCents = isset($room['default_base_price_cents']) ? (int)$room['default_base_price_cents'] : 0;
                ?>
                  <option value="<?php echo htmlspecialchars($code, ENT_QUOTES, 'UTF-8'); ?>" data-base="<?php echo (int)$baseCents; ?>" <?php echo strtoupper($code) === strtoupper($roomCodeDetail) ? 'selected' : ''; ?>>
                    <?php echo htmlspecialchars($label, ENT_QUOTES, 'UTF-8'); ?>
                  </option>
                <?php endforeach; ?>
              </select>
      ...
      <div class="reservation-pricing-card">
        ...
        <?php else: ?>
          <p class="muted">Sin desglose por noches para esta reserva.</p>
        <?php endif; ?>
      </div>
      </div>
      <div class="reservation-side-stack">
        ...
        <div class="reservation-ota-info-card">
          <div class="reservation-ota-info-body">
            <div class="reservation-ota-info-head">
              <span class="reservation-ota-info-title">Conceptos personalizados por OTA</span>
              <?php if ($otaDetectedId > 0): ?>
                <?php
                  $detectedOtaName = isset($otaMetaById[$otaDetectedId]['ota_name']) ? (string)$otaMetaById[$otaDetectedId]['ota_name'] : ('Origen #' . $otaDetectedId);
                  $detectedOtaPlatform = isset($otaMetaById[$otaDetectedId]['platform']) ? strtoupper((string)$otaMetaById[$otaDetectedId]['platform']) : '';
                ?>
                <span class="reservation-ota-info-pill">
                  <?php echo htmlspecialchars(trim($detectedOtaName . ($detectedOtaPlatform !== '' ? (' [' . $detectedOtaPlatform . ']') : '')), ENT_QUOTES, 'UTF-8'); ?>
                </span>
              <?php else: ?>
                <span class="reservation-ota-info-pill is-muted">Sin origen detectado</span>
              <?php endif; ?>
            </div>

            <?php if ($otaDetectedId <= 0): ?>
              <p class="muted">Esta reserva no tiene conceptos de hospedaje vinculados a un origen autom&aacute;tico.</p>
            <?php elseif (!$otaInfoRows): ?>
              <p class="muted">El origen detectado no tiene conceptos configurados para este cuadro.</p>
            <?php else: ?>
            <div class="table-scroll reservation-ota-info-table-wrap">
              <table class="reservation-ota-info-table">
                ...
                <tbody>
                  <?php foreach ($otaInfoRows as $otaInfo): ?>
                    <?php
        ...
        <?php
          $foliosChargesTotalCents = 0;
          $foliosPaymentsTotalCents = 0;
          $foliosBalanceTotalCents = 0;
          foreach ($folios as $folioMetric) {
              $metricFolioId = isset($folioMetric['id_folio']) ? (int)$folioMetric['id_folio'] : 0;
              $metric = ($metricFolioId > 0 && isset($displayMetricsByFolio[$metricFolioId])) ? $displayMetricsByFolio[$metricFolioId] : null;
              $foliosChargesTotalCents += $metric ? (int)$metric['charges_cents'] : 0;
              $foliosPaymentsTotalCents += $metric ? (int)$metric['payments_net_cents'] : 0;
              $foliosBalanceTotalCents += $metric ? (int)$metric['balance_cents'] : 0;
          }
          $foliosCardOpen = isset($financeErrors[$reservationId]) || isset($financeMessages[$reservationId]);
        ?>
        <details class="reservation-folios-card" <?php echo $foliosCardOpen ? 'open' : ''; ?>>
        <summary>
          <div class="reservation-folios-head">
            <strong>Folios</strong>
            <div class="reservation-folios-metrics">
              <span>Balance: <?php echo htmlspecialchars(reservations_format_money($foliosBalanceTotalCents, $summaryCurrency), ENT_QUOTES, 'UTF-8'); ?></span>
              <span>Cargos: <?php echo htmlspecialchars(reservations_format_money($foliosChargesTotalCents, $summaryCurrency), ENT_QUOTES, 'UTF-8'); ?></span>
              <span>Pagos: <?php echo htmlspecialchars(reservations_format_money($foliosPaymentsTotalCents, $summaryCurrency), ENT_QUOTES, 'UTF-8'); ?></span>
            </div>
          </div>
        </summary>
        <div class="subtab-info">
        <div class="folio-topbar">
          <h4>Folios y cobros</h4>
          <button type="button" id="btn-new-folio-toggle" class="button-secondary">Nuevo folio</button>
        </div>
        <form method="post" class="form-inline" id="new-folio-form" style="display:none; margin-top:10px;">
          <?php pms_subtabs_form_state_fields($moduleKey, $subtabState); ?>
          <?php reservations_render_filter_hiddens($filters); ?>
          <input type="hidden" name="reservations_action" value="create_folio">
          <input type="hidden" name="folio_reservation_id" value="<?php echo (int)$reservationId; ?>">
          <label>
            Nombre folio
            <input type="text" name="folio_name" placeholder="Principal">
          </label>
          <label>
            Notas
            <input type="text" name="folio_notes" placeholder="Opcional">
          </label>
          <button type="submit">Crear</button>
        </form>

        <?php if ($folios): ?>
          <div class="folio-accordion">
            <?php foreach ($folios as $folio):
              ...
              $allItems = isset($itemsByFolio[$fid]) ? $itemsByFolio[$fid] : array();
              ...
              $folioMetric = isset($displayMetricsByFolio[$fid]) ? $displayMetricsByFolio[$fid] : array();
              $toggleId = 'folio-' . $fid;
              $folioTotal = isset($folioMetric['charges_cents']) ? (int)$folioMetric['charges_cents'] : 0;
              $folioPaymentsCents = isset($folioMetric['payments_cents']) ? (int)$folioMetric['payments_cents'] : 0;
              $folioRefundsCents = isset($folioMetric['refunds_cents']) ? (int)$folioMetric['refunds_cents'] : 0;
              $folioBalanceCents = isset($folioMetric['balance_cents']) ? (int)$folioMetric['balance_cents'] : 0;
              $folioPaymentsNet = $folioPaymentsCents - $folioRefundsCents;
            ?>
            <details class="folio-item" id="wrap-<?php echo $toggleId; ?>">
              <summary class="folio-header">
                <div class="folio-summary">
                  <strong><?php echo htmlspecialchars((string)$folio['folio_name'], ENT_QUOTES, 'UTF-8'); ?></strong>
                  <span class="muted">(<?php echo htmlspecialchars((string)$folio['status'], ENT_QUOTES, 'UTF-8'); ?>)</span>
                  <span class="muted">Balance: <?php echo reservations_format_money($folioBalanceCents, $folioCurrency); ?></span>
                </div>
              </summary>
              <div class="folio-body" id="<?php echo $toggleId; ?>">
                <div class="subtab-info">
                  <div class="folio-body-header">
                    <p class="folio-totals">Total: <?php echo reservations_format_money($folioTotal, $folioCurrency); ?> | Pagos: <?php echo reservations_format_money($folioPaymentsCents, $folioCurrency); ?> | Reembolsos: <?php echo reservations_format_money($folioRefundsCents, $folioCurrency); ?> | Balance: <?php echo reservations_format_money($folioBalanceCents, $folioCurrency); ?></p>
                    <form method="post" class="form-inline folio-inline-action folio-close-action">
                      <?php pms_subtabs_form_state_fields($moduleKey, $subtabState); ?>
                      <?php reservations_render_filter_hiddens($filters); ?>
                      ...
                      <input type="hidden" name="folio_reservation_id" value="<?php echo (int)$reservationId; ?>">
                      <button type="submit" class="button-secondary"><?php echo $folio['status'] === 'closed' ? 'Reabrir folio' : 'Cerrar folio'; ?></button>
                    </form>
                  </div>
                  ...
                  <div class="folio-section">
                    <div class="folio-section-header">
                      <h5>Cargos</h5>
                      <button type="button" class="button-secondary folio-section-toggle" data-toggle-target="sale-form-<?php echo $fid; ?>" data-toggle-display="flex">Nuevo cargo</button>
                    </div>
                    <form method="post" class="form-inline sale-form" id="sale-form-<?php echo $fid; ?>" data-sale-fields="sale-fields-<?php echo $fid; ?>" style="display:none;">
                      <?php pms_subtabs_form_state_fields($moduleKey, $subtabState); ?>
                      <?php reservations_render_filter_hiddens($filters); ?>
                      <input type="hidden" name="reservations_action" value="create_sale_item">
                      <input type="hidden" name="sale_catalog_id" id="sale-catalog-<?php echo $fid; ?>" value="">
                      ...
                      <label>Concepto
                        <select name="sale_catalog_id" class="concept-select" data-target="<?php echo $fid; ?>">
                          <option value="" data-price="0">Selecciona concepto</option>
                          <?php
                            $concepts = isset($conceptsByProperty[$propCodeDetail]) ? $conceptsByProperty[$propCodeDetail] : array();
                            foreach ($concepts as $c):
                              $optId = isset($c['id_sale_item_catalog']) ? (int)$c['id_sale_item_catalog'] : 0;
                              $optName = isset($c['item_name']) ? (string)$c['item_name'] : '';
                              $optCat = isset($c['category']) ? (string)$c['category'] : '';
                              $optPrice = isset($c['default_unit_price_cents']) ? (int)$c['default_unit_price_cents'] : 0;
                              $optLabel = $optCat !== '' ? ($optCat . ' / ' . $optName) : $optName;
                          ?>
                      ...
                      <label>Fecha servicio <input type="date" name="sale_service_date"></label>
                      <label>Cantidad <input type="number" step="1" min="1" name="sale_quantity" value="1"></label>
                      <label>Precio unit. <input type="number" step="0.01" name="sale_unit_price" id="sale-price-<?php echo $fid; ?>" value="0"></label>
                  ...
                  <?php if ($items): ?>
                    <div class="table-scroll">
                      <table>
                        ...
                        <tbody>
                          <?php
                            $getDerivedChildIds = function ($parentId, $folioId) use ($childrenByParent, $saleItemsById, $childContributesToParentTotal) {
                                $derivedChildIds = array();
                                if (!isset($childrenByParent[$parentId])) {
                                    return $derivedChildIds;
                                }
                                foreach ($childrenByParent[$parentId] as $childId) {
                                    ...
                                    $childRow = $saleItemsById[$childId];
                                    if (!isset($childRow['id_folio']) || (int)$childRow['id_folio'] !== (int)$folioId) {
                                        continue;
                                    }
                                    if (!$childContributesToParentTotal($parentId, $childId)) {
                                        continue;
                                    }
                                    $derivedChildIds[] = $childId;
                                }
                                return $derivedChildIds;
                            };
                            $renderDerivedRows = function ($parentId, $folioId, $groupId, $parentGroupId, $depth) use (&$renderDerivedRows, $getDerivedChildIds, $saleItemsById, $finalTotalForSaleItem, $folioCurrency, $moduleKey, $subtabState, $filters, $reservationId) {
                                $derivedChildIds = $getDerivedChildIds($parentId, $folioId);
                                if (empty($derivedChildIds)) {
                                    return;
                                }
                                foreach ($derivedChildIds as $childId) {
                                    if (!isset($saleItemsById[$childId])) {
                                        continue;
                                    }
                                    $childRow = $saleItemsById[$childId];
                                    $childSubcat = isset($childRow['subcategory_name']) ? trim((string)$childRow['subcategory_name']) : '';
                                    $childName = isset($childRow['item_name']) ? (string)$childRow['item_name'] : '';
                                    $childLabel = $childSubcat !== '' ? ($childSubcat . ' / ' . $childName) : $childName;
                                    $childLineCents = isset($childRow['amount_cents']) ? (int)$childRow['amount_cents'] : 0;
                                    $childFinalCents = $finalTotalForSaleItem((int)$childId);
                                    $childGroupId = $groupId . '-c' . (int)$childId;
                                    $childDerived = $getDerivedChildIds($childId, $folioId);
                                    $hasChildDerivedRows = !empty($childDerived);
                                    $level = max(0, (int)$depth) + 1;
                                    $indentPx = $level * 28;
                                    $rowAttrs = 'class="tax-item-row" data-tax-group="' . htmlspecialchars($groupId, ENT_QUOTES, 'UTF-8') . '"';
                                    if ($parentGroupId !== '') {
                                        $rowAttrs .= ' data-tax-parent="' . htmlspecialchars($parentGroupId, ENT_QUOTES, 'UTF-8') . '"';
                                    }
                          ?>
                          <tr <?php echo $rowAttrs; ?> style="display:none;">
                            <td class="tax-label">
                              <span style="display:inline-block; padding-left: <?php echo (int)$indentPx; ?>px;">
                                <?php echo htmlspecialchars('Derivado: ' . $childLabel, ENT_QUOTES, 'UTF-8'); ?>
                              </span>
                              <?php if ($hasChildDerivedRows): ?>
                                <button type="button" class="button-secondary folio-tax-toggle" data-tax-toggle="<?php echo htmlspecialchars($childGroupId, ENT_QUOTES, 'UTF-8'); ?>">Ver derivados</button>
                              <?php endif; ?>
                            </td>
                            <td></td>
                            <td></td>
                            ...
                            <td></td>
                            <td class="muted"></td>
                            <td><?php echo reservations_format_money($childLineCents, $folioCurrency); ?></td>
                            <td><?php echo reservations_format_money($childLineCents, $folioCurrency); ?></td>
                            <td><?php echo reservations_format_money($childFinalCents, $folioCurrency); ?></td>
                            <td class="folio-row-actions">
                              <form method="post" class="inline-form" onsubmit="return confirm('Confirmar eliminacion de line item derivado? Si tiene derivados hijos, tambien se eliminaran.');">
                                <?php pms_subtabs_form_state_fields($moduleKey, $subtabState); ?>
                                <?php reservations_render_filter_hiddens($filters); ?>
                                <input type="hidden" name="reservations_action" value="delete_sale_item">
                                <input type="hidden" name="sale_reservation_id" value="<?php echo (int)$reservationId; ?>">
                                <input type="hidden" name="sale_folio_id" value="<?php echo (int)$folioId; ?>">
                                <input type="hidden" name="sale_item_id" value="<?php echo (int)$childId; ?>">
                          ...
                          <?php
                                    if (!empty($childDerived)) {
                                        $renderDerivedRows($childId, $folioId, $childGroupId, $groupId, $depth + 1);
                                    }
                                }
                            };
                          ?>
                          <?php foreach ($items as $si): ?>
                              <?php
                                ...
                                if ($parentSaleId > 0 && !isset($saleItemsById[$parentSaleId]) && ($addToFatherRow === null || $addToFatherRow !== 0)) {
                                    continue;
                                }
                                if ($parentSaleId > 0) {
                                    $parentCatalogId = isset($saleItemsById[$parentSaleId]['id_sale_item_catalog']) ? (int)$saleItemsById[$parentSaleId]['id_sale_item_catalog'] : 0;
                                ...
                                $lineTotalWithTaxCents = $lineTotalCents;
                                $finalTotalCents = $finalTotalForSaleItem((int)$si['id_sale_item']);
                                $taxGroupId = 'tax-' . $fid . '-' . (int)$si['id_sale_item'];
	                                $derivedChildIds = $getDerivedChildIds($saleItemId, $fid);
	                                $hasDerivedRows = !empty($derivedChildIds);
	                                $currentLineItemType = isset($si['item_type']) ? strtolower(trim((string)$si['item_type'])) : 'sale_item';
	                                $isProtectedPrimaryLodging = $primaryLodgingLineItemId > 0 && $saleItemId === $primaryLodgingLineItemId;
                            ...
                            <tr>
                              <td>
                                ...
                                <?php if ($hasDerivedRows): ?>
                                  <button type="button" class="button-secondary folio-tax-toggle" data-tax-toggle="<?php echo htmlspecialchars($taxGroupId, ENT_QUOTES, 'UTF-8'); ?>">Ver derivados</button>
                                <?php endif; ?>
                              </td>
                              <td>
                              ...
                              <td>
                                <input type="number" step="1" min="1" name="sale_quantity" form="<?php echo $updateFormId; ?>" value="<?php echo htmlspecialchars($quantityDisplay, ENT_QUOTES, 'UTF-8'); ?>" style="width:80px">
                              </td>
                              <td>
                              ...
                              <td><?php echo reservations_format_money($lineTotalWithTaxCents, $folioCurrency); ?></td>
                              ...
                              <td class="folio-row-actions">
                                <form method="post" class="inline-form" id="<?php echo $updateFormId; ?>" onsubmit="return confirm('Confirmar actualizacion de line item y recalculo de derivados?');">
                                  ...
                                  <button type="submit" class="button-secondary">Actualizar</button>
	                                ...
	                                <input type="hidden" name="sale_reservation_id" value="<?php echo (int)$reservationId; ?>">
	                                <input type="hidden" name="sale_item_id" value="<?php echo (int)$si['id_sale_item']; ?>">
	                                <button type="submit" class="button-secondary" <?php echo $isProtectedPrimaryLodging ? 'disabled title="Usa Cambiar tipo de hospedaje"' : ''; ?>>Eliminar</button>
	                              </form>
	                            </td>
	                          </tr>
                          <?php
                            if (!empty($derivedChildIds)) {
                                $renderDerivedRows($saleItemId, $fid, $taxGroupId, '', 0);
                            }
                          ?>
                        <?php endforeach; ?>
                      </tbody>
                  ...
                  <div class="folio-section">
                    <div class="folio-section-header">
                      <h5>Pagos</h5>
                      <button type="button" class="button-secondary folio-section-toggle" data-toggle-target="payment-form-<?php echo $fid; ?>" data-toggle-display="flex">Nuevo pago</button>
                    </div>
                    <form method="post" class="form-inline" id="payment-form-<?php echo $fid; ?>" style="display:none;">
                      ...
                      <label>Concepto de pago
                        <?php $detailPaymentCatalogs = reservations_payment_catalogs_for_property($paymentCatalogsByProperty, $propertyCodeDetail); ?>
                            ...
                            <tr>
                              ...
                              <td class="form-inline">
                                <form method="post" class="inline-form">
                                  ...
                                  <?php
                                    $detailPaymentCatalogs = reservations_payment_catalogs_for_property($paymentCatalogsByProperty, $propertyCodeDetail);
                ...
                <div class="folio-summary-total">
                  <span>Cargos: <?php echo reservations_format_money($folioTotal, $folioCurrency); ?></span>
                  <span>Pagos: <?php echo reservations_format_money($folioPaymentsNet, $folioCurrency); ?></span>
                  <span>Saldo: <?php echo reservations_format_money($folioBalanceCents, $folioCurrency); ?></span>
                </div>

                <?php if (!$allItems): ?>
                  <form method="post" class="form-inline folio-inline-action folio-delete-action">
                    <?php pms_subtabs_form_state_fields($moduleKey, $subtabState); ?>
                    <?php reservations_render_filter_hiddens($filters); ?>
                    <input type="hidden" name="reservations_action" value="delete_folio">
                    <input type="hidden" name="folio_id" value="<?php echo $fid; ?>">
                    <input type="hidden" name="folio_reservation_id" value="<?php echo (int)$reservationId; ?>">
                    <button type="submit" class="button-secondary danger">Eliminar folio</button>
                  </form>
                <?php endif; ?>
              </div>
            </div>
          ...
          <tr class="folio-panel" id="folio-<?php echo $fid; ?>" style="display:none;">
            <td colspan="8">
              <div class="subtab-info">
                ...
                <form method="post" class="form-inline sale-form" data-sale-fields="sale-fields-<?php echo $fid; ?>">
                  <?php pms_subtabs_form_state_fields($moduleKey, $subtabState); ?>
                  <?php reservations_render_filter_hiddens($filters); ?>
                  <input type="hidden" name="reservations_action" value="create_sale_item">
                  <input type="hidden" name="sale_catalog_id" id="sale-catalog-<?php echo $fid; ?>" value="">
                  ...
                  <label>Fecha servicio <input type="date" name="sale_service_date"></label>
                  <label>Cantidad <input type="number" step="0.01" name="sale_quantity" value="1"></label>
                  <label>Precio unit. <input type="number" step="0.01" name="sale_unit_price" id="sale-price-<?php echo $fid; ?>" value="0"></label>
                              ...
                              if ($parentSaleId > 0) {
                                  ...
                                  if (isset($saleItemsById[$parentSaleId])) {
                                      ...
                                      if ($childCatalogId > 0 && $parentCatalogId > 0 && isset($addToFatherMap) && is_array($addToFatherMap)) {
                                          ...
                                          $addToFatherLabel = (isset($addToFatherMap[$relationKey]) && (int)$addToFatherMap[$relationKey] === 1) ? 'SÃ­' : 'No';
                                      } else {
                                          $addToFatherLabel = 'No';
                                      }
                                  } else {
                  ...
                  <div class="form-grid grid-3">
                    ...
                    <label>
                      ...
                      <div class="phone-input">
                        ...
                        <input type="text" name="confirm_guest_phone" data-guest-scope="confirm-<?php echo (int)$reservationId; ?>" data-guest-field="phone" value="<?php echo htmlspecialchars((string)$confirmGuestData['phone'], ENT_QUOTES, 'UTF-8'); ?>">
                      </div>
                    </label>
                    <label>
                      Apellido paterno
...
/* Fallback: si la pestaÃ±a activa no existe, ir a General o al primer tab disponible */
...
// Toggle mostrar/ocultar formulario de nuevo folio
(function(){
  var toggleBtn = document.getElementById('btn-new-folio-toggle');
  var form = document.getElementById('new-folio-form');
  ...
  toggleBtn.addEventListener('click', function(){
    ...
    if (show) {
      var input = form.querySelector('input[name=\"folio_name\"]');
...
/* Sincroniza los campos ocultos al elegir un concepto */
document.addEventListener('change', function (ev) {
  var select = ev.target.closest('.concept-select');
  if (!select) return;
  var target = select.getAttribute('data-target');
  var option = select.options[select.selectedIndex];
  var price = option.getAttribute('data-price') || '0';
  var priceField = document.getElementById('sale-price-' + target);
  var catalogField = document.getElementById('sale-catalog-' + target);
...
(function () {
  function setupGuestSuggestions(scope, suggestions) {
    ...
    function fetchGuests(query) {
      ...
      fetch('api/guest_search.php?q=' + encodeURIComponent(query))
...
(function () {
  ...
  function applyBreakdownAverage(avg) {
    var rows = getBreakdownRows();
    if (!rows.length) return;
    rows.forEach(function (row) {
      var input = row.querySelector('.nightly-amount-input');
      if (input) {
        setFieldValue(input, avg.toFixed(2));
      } else {
        row.setAttribute('data-night-amount', avg.toFixed(2));
        var amountEl = row.querySelector('.nightly-amount');
        if (amountEl) {
          amountEl.textContent = avg.toFixed(2);
        }
      }
    });
  }
  ...
  function updateFixedChild() {
    ...
    function updateFinalTotals(nights) {
      ...
      // Impuestos fuera: mostramos igual sin tax mientras no haya reglas aquÃ­.
    ...
    totalInput.addEventListener('input', function () {
      ...
      lastLodgingSource = totalInput.value.trim() !== '' ? 'total' : (nightlyInput.value.trim() !== '' ? 'nightly' : '');
        // ya no sincronizamos con tarifa base: solo desglose
        var nights = computeNights();
        var totalVal = parseNumber(totalInput.value);
        if (nights > 0 && totalVal && totalVal > 0) {
          applyBreakdownAverage(totalVal / nights);
        }
    ...
    nightlyInput.addEventListener('input', function () {
      ...
      lastLodgingSource = nightlyInput.value.trim() !== '' ? 'nightly' : (totalInput.value.trim() !== '' ? 'total' : '');
        ...
        var nights = computeNights();
        var nightlyVal = parseNumber(nightlyInput.value);
        if (nights > 0 && nightlyVal && nightlyVal > 0) {
          applyBreakdownAverage(nightlyVal);
        }
      updateFinalTotals(computeNights());
    });
  if (fixedInput) {
...
(function () {
  document.querySelectorAll('[data-interest-widget]').forEach(function (widget) {
    ...
    function callInterestApi(action, catalogId) {
      ...
      return fetch('api/reservation_interest.php', {
        method: 'POST',
