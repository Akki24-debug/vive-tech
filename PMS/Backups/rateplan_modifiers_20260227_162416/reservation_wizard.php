...
function rw_concept_display_label(array $row)
{
    ...
    foreach ($categoryFields as $field) {
        if (!empty($row[$field])) {
            $category = trim((string)$row[$field]);
            if ($category !== '') {
                break;
            }
        }
    ...
    foreach ($subcategoryFields as $field) {
        if (!empty($row[$field])) {
            $subcategory = trim((string)$row[$field]);
            if ($subcategory !== '') {
                break;
            }
        }
    ...
    if ($subcategory !== '' && strcasecmp($subcategory, $category) !== 0) {
        $parts[] = $subcategory;
    }
    if ($item !== '') {
        $parts[] = $item;
...
function wizard_apply_payments($reservationId, $actorUserId, $propertyCode = '')
{
    ...
    foreach ($methods as $idx => $methodIdRaw) {
        ...
        try {
            ...
            $result['errors'][] = 'Pago no aplicado: ' . $inner->getMessage();
            $created = false;
        }
        if ($created) {
            $paymentsCreated++;
...
if ($step === 2) {
    ...
    // Prefill desglose usando sp_rateplan_calendar si es posible
    if ($step === 2 && $nights > 0) {
        try {
            $calSets = pms_call_procedure('sp_rateplan_calendar', array(
                $wizard['property_code'],
                '', // rateplan opcional
                $wizard['category_code'],
                $wizard['room_code'],
            ...
            $rows = isset($calSets[0]) ? $calSets[0] : array();
            foreach ($rows as $row) {
        ...
        if ($editingReservationId > 0) {
            ...
            if ($guestIdValue > 0) {
                ...
                if (isset($paymentResult['attempted'], $paymentResult['created']) && (int)$paymentResult['attempted'] > 0 && (int)$paymentResult['created'] === 0) {
                    ...
                    if (!empty($paymentResult['errors'])) {
                        $errors[] = 'Detalle pagos: ' . implode(' | ', array_unique($paymentResult['errors']));
                    }
                    $paymentFailed = true;
                }
                if (!$paymentFailed) {
            ...
            if ($newReservationId > 0 && $newFolioId <= 0 && $guestIdValue > 0) {
                try {
                    ...
                    $errors[] = 'No se pudo crear folio para pagos: ' . $e->getMessage();
                }
            }
            if ($newReservationId > 0) {
                if ($newFolioId <= 0) {
...
<div class="subtab-info wizard-lightbox-host" id="wizard-lightbox-host">
  <div class="wizard-lightbox-backdrop" data-wizard-lightbox-close="1"></div>
  ...
  <?php endforeach; ?>
    ...
    <script>window.location.href = "index.php?view=calendar";</script>
  <?php endif; ?>
...
(function () {
  ...
  if (lightboxHost) {
    ...
    var closeWizardLightbox = function () {
      ...
      window.location.href = 'index.php?view=reservations';
    };
    if (closeBtn) {
      closeBtn.addEventListener('click', closeWizardLightbox);
    }
    lightboxHost.addEventListener('click', function (event) {
      var target = event.target;
      if (!(target instanceof HTMLElement)) {
        return;
      }
      if (target.getAttribute('data-wizard-lightbox-close') === '1') {
        closeWizardLightbox();
      }
    });
