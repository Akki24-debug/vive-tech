...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function handleCalendarArrowKeyScroll(event) {
    ...
    const currentLeft = calendarGlobalScrollbarInner ? calendarGlobalScrollbarInner.scrollLeft : primaryScroll.scrollLeft;
    const nextLeft = Math.max(0, Math.min(maxLeft, Math.round(currentLeft + (direction * step))));
    ...
    primaryScroll.scrollLeft = nextLeft;
    syncCalendarScrollFrom(primaryScroll);
  }
  ...
  function resolveCalendarHorizontalWheelDelta(event) {
    ...
    if (!event.shiftKey && Math.abs(fallbackDeltaY) > 0.01 && Math.abs(deltaX) < (Math.abs(fallbackDeltaY) * 0.7)) {
      return 0;
    }
    return deltaX;
  }
  ...
  function handleCalendarHorizontalWheel(event) {
    ...
    const currentLeft = calendarGlobalScrollbarInner ? calendarGlobalScrollbarInner.scrollLeft : primaryScroll.scrollLeft;
    const nextLeft = Math.max(0, Math.min(maxLeft, Math.round(currentLeft + deltaX)));
