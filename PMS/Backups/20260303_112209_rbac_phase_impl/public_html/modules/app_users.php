    ...
    try {
        ...
        $pdo->prepare(
            'DELETE up FROM user_property up
             ...
             WHERE up.id_user = ? AND pr.id_company = ?'
        )->execute(array($userId, $companyId));

        if ($codes) {
            $placeholders = implode(',', array_fill(0, count($codes), '?'));
            $params = array_merge(array($companyId), $codes);
            $stmt = $pdo->prepare(
                "SELECT id_property FROM property
                 WHERE id_company = ? AND deleted_at IS NULL AND code IN ($placeholders)"
            );
            $stmt->execute($params);
            ...
            if ($properties) {
                $insert = $pdo->prepare(
                    'INSERT INTO user_property (id_user, id_property, is_primary, is_active, created_at, updated_at, created_by)
                     VALUES (?, ?, 0, 1, NOW(), NOW(), ?)'
                );
                foreach ($properties as $pid) {
                    $insert->execute(array($userId, (int)$pid, $actorUserId));
                }
            }
        }
    ...
    try {
        ...
        $pdo->prepare(
            'DELETE ur FROM user_role ur
             JOIN role rl ON rl.id_role = ur.id_role
             JOIN property pr ON pr.id_property = rl.id_property
             WHERE ur.id_user = ? AND pr.id_company = ?'
        )->execute(array($userId, $companyId));
            ...
            $stmt = $pdo->prepare(
                "SELECT rl.id_role
                 ...
                 WHERE pr.id_company = ?
                   ...
                   AND rl.is_active = 1
                   AND rl.id_role IN ($placeholders)"
            );
            $params = array_merge(array($companyId), $ids);
            $stmt->execute($params);
            $roles = $stmt->fetchAll(PDO::FETCH_COLUMN);

            if ($roles) {
                $insert = $pdo->prepare(
                    'INSERT INTO user_role (id_user, id_role, is_active, created_at, updated_at, created_by)
                     VALUES (?, ?, 1, NOW(), NOW(), ?)'
                );
                foreach ($roles as $rid) {
                    $insert->execute(array($userId, (int)$rid, $actorUserId));
                }
            }
        }
...
<section class="card detail-card">
  ...
  <form method="post" class="form-grid grid-3">
    ...
    <fieldset class="full related-group">
      ...
      <p class="muted">La logica de permisos se integrara mas adelante.</p>
    </fieldset>
