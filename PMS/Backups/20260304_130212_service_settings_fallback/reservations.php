...
try {
    ...
    $loadSelectedServiceCatalogIds = function ($scopePropertyCode) use ($companyCode) {
        $scopePropertyCode = strtoupper(trim((string)$scopePropertyCode));
        try {
            $sets = pms_call_procedure('sp_pms_settings_data', array(
                $companyCode,
  ...
  function rowsForProperty(propertyCode) {
    ...
    merged.forEach(function (row) {
      ...
      rows.push({
        ...
        label: normalizeText(row && row.label) || normalizeText(row && row.name) || ('Concepto #' + String(id)),
        category_name: normalizeText(row && row.category_name),
        subcategory_name: normalizeText(row && row.subcategory_name),
        default_unit_price_cents: parseInt(row && row.default_unit_price_cents ? row.default_unit_price_cents : 0, 10) || 0
      });
    });
    return rows;
  }
