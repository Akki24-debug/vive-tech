<div class="report-v2-grid">
  <section class="report-v2-card report-v2-stack report-v2-card--full">
    <div>
      <h2>Plantillas</h2>
      <p class="report-v2-muted">Cada plantilla define las columnas visibles del reporte por reservacion.</p>
    </div>
    ...
    <?php if ($canDesign): ?>
      <div class="report-v2-card report-v2-stack">
        ...
        <?php if ($selectedTemplate): ?>
          <div class="report-v2-actions">
            <a class="report-v2-chip" href="<?php echo reports_v2_h('?view=reports&reports_tab=templates'); ?>">Nueva</a>
...
<?php if ($canDesign): ?>
  <div class="report-v2-grid-bottom">
      ...
      <?php else: ?>
        <div class="report-v2-table-wrap">
          <table class="report-v2-table">
            <thead><tr><th>Orden</th><th>Campo</th><th>Tipo</th><th>Origen</th><th>Formato</th><th>Visible</th><th>Activa</th><th></th></tr></thead>
            <tbody>
              <?php if (empty($templateFields)): ?>
                <tr><td colspan="8" class="report-v2-muted">La plantilla no tiene campos.</td></tr>
              <?php else: ?>
                <?php foreach ($templateFields as $fieldRow): ?>
                  ...
                  <tr>
                    ...
                    <td><?php echo reports_v2_h($fieldRow['display_name']); ?></td>
        ...
        <div class="report-v2-card report-v2-stack">
          ...
          <form method="post" class="report-v2-stack">
            ...
            <div class="report-v2-form-grid report-v2-form-grid--4">
              <label><span>Tipo</span><select name="field_type" id="reports-v2-field-type"><?php foreach (reports_v2_field_type_options() as $typeCode => $typeLabel): ?><option value="<?php echo reports_v2_h($typeCode); ?>"><?php echo reports_v2_h($typeLabel); ?></option><?php endforeach; ?></select></label>
              <label><span>Orden</span><input type="number" name="order_index" min="1" value="<?php echo max(1, count($templateFields) + 1); ?>"></label>
              <label><span>Formato</span><select name="field_format_hint"><?php foreach ($formatOptions as $formatCode => $formatLabel): ?><option value="<?php echo reports_v2_h($formatCode); ?>"><?php echo reports_v2_h($formatLabel); ?></option><?php endforeach; ?></select></label>
              <label><span>Visible</span><select name="field_is_visible"><option value="1">Si</option><option value="0">No</option></select></label>
            </div>
            <label><span>Nombre visible</span><input type="text" name="display_name" placeholder="Opcional; si lo dejas vacio toma el nombre base"></label>
            <div class="report-v2-form-grid" data-field-panel="reservation">
              <label><span>Campo de reservacion</span><select name="reservation_field_code"><?php $fieldGroups = array(); foreach ($reservationFieldCatalog as $fieldCode => $fieldMeta) { $groupName = isset($fieldMeta['group']) ? $fieldMeta['group'] : 'Otros'; if (!isset($fieldGroups[$groupName])) { $fieldGroups[$groupName] = array(); } $fieldGroups[$groupName][$fieldCode] = $fieldMeta; } foreach ($fieldGroups as $groupName => $groupFields): ?><optgroup label="<?php echo reports_v2_h($groupName); ?>"><?php foreach ($groupFields as $fieldCode => $fieldMeta): ?><option value="<?php echo reports_v2_h($fieldCode); ?>"><?php echo reports_v2_h($fieldMeta['label']); ?></option><?php endforeach; ?></optgroup><?php endforeach; ?></select></label>
            </div>
            <div class="report-v2-form-grid report-v2-form-grid--3" data-field-panel="line_item" style="display:none;">
              <label><span>Line item</span><select name="line_item_catalog_id"><?php foreach ($lineItemCatalogs as $catalogRow): $parts = array(); if (!empty($catalogRow['property_name'])) { $parts[] = $catalogRow['property_name']; } if (!empty($catalogRow['category_name'])) { $parts[] = $catalogRow['category_name']; } if (!empty($catalogRow['subcategory_name']) && $catalogRow['subcategory_name'] !== $catalogRow['category_name']) { $parts[] = $catalogRow['subcategory_name']; } $parts[] = $catalogRow['item_name']; ?><option value="<?php echo (int)$catalogRow['id_line_item_catalog']; ?>"><?php echo reports_v2_h(implode(' / ', array_filter($parts))); ?></option><?php endforeach; ?></select></label>
              <label><span>Metrica</span><select name="source_metric"><?php foreach ($metricOptions as $metricCode => $metricMeta): ?><option value="<?php echo reports_v2_h($metricCode); ?>"><?php echo reports_v2_h($metricMeta['label']); ?></option><?php endforeach; ?></select></label>
            </div>
            <div class="report-v2-form-grid" data-field-panel="calculated" style="display:none;">
              <label><span>Calculo</span><select name="report_calculation_id"><option value="0">Selecciona</option><?php foreach ($calculations as $calculationRow): ?><option value="<?php echo (int)$calculationRow['id_report_calculation']; ?>"><?php echo reports_v2_h($calculationRow['calc_name']); ?></option><?php endforeach; ?></select></label>
            </div>
            <div class="report-v2-actions"><button type="submit">Agregar campo</button></div>
          </form>
        </div>
      <?php endif; ?>
    </section>
    ...
    <section class="report-v2-card report-v2-stack">
      <div><h2>Calculos</h2><p class="report-v2-muted">Las expresiones usan variables numericas reales. Ejemplo: <code>reservation_total_price_cents - line_item_12_amount_cents</code>.</p></div>
      <div class="report-v2-list">
      ...
      <div class="report-v2-card report-v2-stack">
        ...
        <form method="post" class="report-v2-stack">
          ...
          <div><div class="report-v2-muted">Variables disponibles</div><div class="report-v2-code-list"><?php $variableGroups = array(); foreach ($variableCatalog as $variableCode => $variableMeta) { $groupName = isset($variableMeta['group']) ? $variableMeta['group'] : 'Otros'; if (!isset($variableGroups[$groupName])) { $variableGroups[$groupName] = array(); } $variableGroups[$groupName][$variableCode] = $variableMeta; } foreach ($variableGroups as $groupName => $groupVariables): ?><div style="margin-bottom:10px;"><div class="report-v2-muted" style="margin-bottom:6px;"><?php echo reports_v2_h($groupName); ?></div><?php foreach ($groupVariables as $variableCode => $variableMeta): ?><button type="button" class="report-v2-insert-variable" data-variable="<?php echo reports_v2_h($variableCode); ?>"><?php echo reports_v2_h($variableCode); ?></button><?php endforeach; ?></div><?php endforeach; ?></div></div>
          <div class="report-v2-actions">
...
<section class="report-v2-card report-v2-stack">
  <div>
    <h2>Resultado</h2>
    <p class="report-v2-muted">Primera version sobre reservaciones. Se ejecuta contra reservacion, huesped, propiedad, habitacion, categoria, tarifa y sumas de line items.</p>
  </div>
  <?php if (!$canRun): ?>
  ...
  <?php else: ?>
    <form method="get" class="report-v2-stack">
      ...
      <div class="report-v2-form-grid report-v2-form-grid--4">
        ...
        <label><span>Estatus</span><select name="report_status"><option value="">Todos</option><?php foreach (array('apartado', 'confirmado', 'en casa', 'salida', 'no-show', 'cancelada') as $statusOption): ?><option value="<?php echo reports_v2_h($statusOption); ?>" <?php echo $runFilters['status'] === $statusOption ? 'selected' : ''; ?>><?php echo reports_v2_h($statusOption); ?></option><?php endforeach; ?></select></label>
        <label><span>Desde</span><input type="date" name="report_date_from" value="<?php echo reports_v2_h($runFilters['date_from']); ?>"></label>
        <label><span>Hasta</span><input type="date" name="report_date_to" value="<?php echo reports_v2_h($runFilters['date_to']); ?>"></label>
      </div>
      <div class="report-v2-form-grid">
        <label><span>Busqueda</span><input type="text" name="report_search" value="<?php echo reports_v2_h($runFilters['search']); ?>" placeholder="Codigo reserva, huesped, habitacion"></label>
      ...
      <div class="report-v2-actions"><button type="submit">Ejecutar</button><span class="report-v2-chip">Maximo 500 reservaciones por corrida</span><?php if ($runTemplate): ?><span class="report-v2-chip"><?php echo reports_v2_h($runTemplate['report_name']); ?></span><?php endif; ?></div>
    </form>

    <div class="report-v2-table-wrap">
      <table class="report-v2-table">
        <thead><tr><?php foreach ($templateFields as $fieldRow): ?><th><?php echo reports_v2_h($fieldRow['display_name']); ?></th><?php endforeach; ?></tr></thead>
        <tbody>
          <?php if (empty($templateFields)): ?>
            <tr><td class="report-v2-muted">La plantilla seleccionada todavia no tiene campos.</td></tr>
          <?php elseif (empty($reportRows)): ?>
            <tr><td colspan="<?php echo count($templateFields); ?>" class="report-v2-muted">Sin resultados para los filtros actuales.</td></tr>
          <?php else: ?>
            <?php foreach ($reportRows as $reportRow): ?><tr><?php foreach ($templateFields as $fieldRow): $fieldId = isset($fieldRow['id_report_template_field']) ? (int)$fieldRow['id_report_template_field'] : 0; $cell = isset($reportRow['cells'][$fieldId]) ? $reportRow['cells'][$fieldId] : array('display' => ''); ?><td title="<?php echo reports_v2_h(isset($cell['error']) ? $cell['error'] : ''); ?>"><?php echo reports_v2_h(isset($cell['display']) ? $cell['display'] : ''); ?></td><?php endforeach; ?></tr><?php endforeach; ?>
          <?php endif; ?>
        </tbody>
      </table>
    </div>
