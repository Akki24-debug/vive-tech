...
if (!function_exists('reports_fetch_sale_item_catalogs')) {
    function reports_fetch_sale_item_catalogs($companyCode, $propertyCode)
    {
        ...
        try {
            ...
            $rateCol = 'percent_value';
            $stmtRate = $pdo->query("SHOW COLUMNS FROM line_item_catalog LIKE 'rate_percent'");
            if ($stmtRate && $stmtRate->fetch(PDO::FETCH_ASSOC)) {
                $rateCol = 'rate_percent';
            }
            $sql = 'SELECT
                        sic.id_line_item_catalog,
                        sic.catalog_type,
                        sic.item_name,
                        sic.' . $rateCol . ' AS rate_percent,
