            ...
            $stmt = $pdo->prepare(
                'SELECT
                 ...
                 JOIN sale_item_category child_cat
                  ...
                  AND COALESCE(child_cat.is_active, 1) = 1
                  AND child_cat.id_company = ?
        ...
        try {
            ...
            if ($createdId) {
                ...
                if ($needsFolio) {
                    ...
                    if ($folioId <= 0) {
                        try {
                            ...
                            $financeErrors[$createdId] = 'No se pudo obtener el folio para registrar servicios/pagos: ' . $e->getMessage();
                        }
                    }
                    if ($folioId <= 0 && !isset($financeErrors[$createdId])) {
                        try {
                            ...
                            $financeErrors[$createdId] = 'No se pudo crear el folio para registrar servicios/pagos: ' . $e->getMessage();
                        }
                    }
                }
                ...
                if ($folioId > 0 && !empty($filteredExtraConcepts)) {
                    ...
                    $financeErrors[$createdId] = 'No se encontro un folio para registrar servicios/pagos.';
                }
    ...
    if ($reservationId > 0 && ($folioId > 0 || $saleId > 0)) {
        try {
            ...
            if ($actionCode === 'create') {
                ...
                if ($actionCode === 'create' && $targetCatalogId > 0 && $folioId > 0) {
                    $fixedPath = array();
                    reservations_upsert_fixed_children_tree(
                        $reservationId,
                        $folioId,
...
foreach ($openReservationIds as $reservationId) {
    ...
    if ($isHold) {
        ...
        $headerAlertButton = 'Agregar servicios';
        $headerAlertClass = '';
    ...
    <?php else: ?>
      ...
      <?php
        ...
        foreach ($saleItems as $nightSaleRow) {
            ...
            $nightFinalCents = (int)$finalTotalForSaleItem($nightSaleId);
            $nightlyAmountsByDate[$nightDate] += $nightFinalCents;
            if ($nightFinalCents > 0) {
                $hasLodgingNightlyCharges = true;
            }
        }
      ...
      <div class="reservation-side-stack">
        ...
        <summary>
          <div class="reservation-folios-head">
            ...
            <div class="reservation-folios-metrics">
              ...
              <span>Servicios: <?php echo htmlspecialchars(reservations_format_money($foliosChargesTotalCents, $summaryCurrency), ENT_QUOTES, 'UTF-8'); ?></span>
        ...
        <div class="folio-topbar">
          <h4>Folios, servicios y pagos</h4>
        ...
        <?php if ($folios): ?>
          <div class="folio-accordion">
            ...
            ?>
            <details class="folio-item" id="wrap-<?php echo $toggleId; ?>" open>
              ...
              <div class="folio-body" id="<?php echo $toggleId; ?>">
                ...
                <div class="folio-summary-total">
                  <span>Servicios: <?php echo reservations_format_money($folioTotal, $folioCurrency); ?></span>
...
function pmsReservationOpenServiceLightbox(context) {
  var existing = document.querySelector('.calendar-quickres-lightbox');
  if (existing) {
    existing.remove();
  }

  var serviceMap = (window.pmsReservationServiceCatalogMap && typeof window.pmsReservationServiceCatalogMap === 'object')
  ...
  function formatCentsToInput(cents) {
    var value = Number.isFinite(cents) ? Math.max(0, cents) : 0;
    return (value / 100).toFixed(2);
  }
  function rowsForProperty(propertyCode) {
    var prop = normalizeCode(propertyCode);
    var rows = [];
    var seen = {};
    var globalRows = Array.isArray(serviceMap['*']) ? serviceMap['*'] : [];
    var localRows = prop && Array.isArray(serviceMap[prop]) ? serviceMap[prop] : [];
    var merged = globalRows.concat(localRows);
    merged.forEach(function (row) {
      var id = parseInt(row && row.id_service_catalog ? row.id_service_catalog : 0, 10) || 0;
      if (id <= 0 || seen[id]) return;
      seen[id] = true;
      var defaultUnitPriceCents = parseInt(row && row.default_unit_price_cents ? row.default_unit_price_cents : 0, 10) || 0;
      var conceptName = normalizeText(row && row.name) || normalizeText(row && row.label) || ('Concepto #' + String(id));
      rows.push({
        id_service_catalog: id,
        name: conceptName,
        label: conceptName + (defaultUnitPriceCents > 0 ? (' ($' + formatCentsToInput(defaultUnitPriceCents) + ')') : ''),
  ...
  var serviceRows = rowsForProperty(context.propertyCode || '');
  if (!serviceRows.length) {
    alert('No hay conceptos de servicio configurados para esta propiedad.');
    return;
  }
  ...
  overlay.innerHTML = `
    <div class="calendar-quickres-window" role="dialog" aria-modal="true" aria-label="Agregar servicio">
      ...
      <p class="calendar-quickres-context">
        ${(context.propertyCode || '').toString()} / ${(context.reservationCode || '').toString()} / ${(context.guestName || '').toString()}
      </p>
      <form class="calendar-quickres-form" novalidate>
        ...
        <label class="full">
          Concepto
          <select name="service_catalog"></select>
        </label>
        <label>
          Cantidad
        ...
        <label>
          Fecha de servicio
          <input type="date" name="service_date" value="${(new Date()).toISOString().slice(0, 10)}">
        </label>
        <label class="full">
          Descripcion
        ...
        <div class="calendar-quickres-actions calendar-payment-actions">
          <button type="submit" class="button-primary">Confirmar y agregar servicio</button>
          <button type="button" class="button-secondary calendar-quickres-cancel calendar-quickres-cancel-danger">
            <span class="calendar-quickres-cancel-icon" aria-hidden="true">X</span> Cancelar
          </button>
        </div>
      </form>
    </div>
  ...
  var closeButton = overlay.querySelector('.calendar-quickres-close');
  var cancelButton = overlay.querySelector('.calendar-quickres-cancel');
  var form = overlay.querySelector('.calendar-quickres-form');
  var categorySelect = overlay.querySelector('select[name="service_category"]');
  var subcategorySelect = overlay.querySelector('select[name="service_subcategory"]');
  var conceptSelect = overlay.querySelector('select[name="service_catalog"]');
  var qtyInput = overlay.querySelector('input[name="service_quantity"]');
  var unitPriceInput = overlay.querySelector('input[name="service_unit_price"]');
  var dateInput = overlay.querySelector('input[name="service_date"]');
  var descriptionInput = overlay.querySelector('input[name="service_description"]');
  ...
  if (cancelButton) cancelButton.addEventListener('click', closeLightbox);
  ...
  function fillSubcategories() {
    ...
    var selectedCategory = normalizeText(categorySelect && categorySelect.value);
    var seen = {};
    serviceRows.forEach(function (row) {
      if (selectedCategory && row.category_name !== selectedCategory) return;
      var key = row.subcategory_name || '';
      if (!key || seen[key]) return;
      seen[key] = true;
  ...
  function filteredRows() {
    var selectedCategory = normalizeText(categorySelect && categorySelect.value);
    var selectedSubcategory = normalizeText(subcategorySelect && subcategorySelect.value);
  ...
  function updateUnitPrice() {
    ...
    var selectedId = parseInt(conceptSelect.value || '0', 10) || 0;
    var selectedRow = null;
    serviceRows.forEach(function (row) {
      if (selectedRow) return;
      if (row.id_service_catalog === selectedId) {
        selectedRow = row;
      }
    });
    if (selectedRow) {
      unitPriceInput.value = formatCentsToInput(selectedRow.default_unit_price_cents || 0);
    }
  }
  function fillConcepts() {
    ...
    rows.forEach(function (row) {
      appendOption(conceptSelect, row.id_service_catalog, row.label);
    });
  ...
  if (conceptSelect) {
    conceptSelect.addEventListener('change', updateUnitPrice);
  }

  if (form) {
    form.addEventListener('submit', function (event) {
      event.preventDefault();
      var conceptId = conceptSelect ? (parseInt(conceptSelect.value || '0', 10) || 0) : 0;
      var qtyRaw = qtyInput && qtyInput.value ? qtyInput.value.trim() : '';
      var unitRaw = unitPriceInput && unitPriceInput.value ? unitPriceInput.value.trim() : '';
      var qtyValue = parseFloat((qtyRaw || '0').replace(',', '.'));
      var unitValue = parseFloat((unitRaw || '0').replace(',', '.'));
      var serviceDate = dateInput && dateInput.value ? dateInput.value.trim() : '';
      ...
      var totalValue = qtyValue * unitValue;
      if (!confirm('Confirmar servicio por ' + totalValue.toFixed(2) + ' ' + (context.currency || 'MXN') + '?')) {
        return;
      }
      ...
      pmsReservationBuildHiddenInput(postForm, 'sale_catalog_id', String(conceptId));
      pmsReservationBuildHiddenInput(postForm, 'sale_service_date', serviceDate);
      pmsReservationBuildHiddenInput(postForm, 'sale_quantity', qtyValue.toString());
      pmsReservationBuildHiddenInput(postForm, 'sale_unit_price', unitValue.toFixed(2));
      pmsReservationBuildHiddenInput(postForm, 'sale_item_description', description);
