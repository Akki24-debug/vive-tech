...
if (!$currentUser) {
    echo '<p class="error">SesiÃ³n invÃ¡lida.</p>';
    return;
}
...
function rw_fetch_lodging_options_with_children($companyCode, $propertyCode, $companyId = 0)
{
                ...
                foreach ($configuredRows as $row) {
                    ...
                    foreach ($children as $childRow) {
                        ...
                        $childrenById[$childId] = array(
                            ...
                            'default_cents' => isset($childRow['default_unit_price_cents']) ? (int)$childRow['default_unit_price_cents'] : 0,
                            'tax_rule_ids' => ''
                        );
                    }
    ...
    if (!$lodgingOptions && $allowedIds) {
        try {
            ...
            $stmt = $pdo->prepare(
                'SELECT id_line_item_catalog AS id_sale_item_catalog, item_name, default_unit_price_cents
            ...
            foreach ($rawRows as $r) {
                ...
                $lodgingOptions[] = array(
                    ...
                    'default_unit_price_cents' => isset($r['default_unit_price_cents']) ? (int)$r['default_unit_price_cents'] : 0,
                    'child_id' => 0,
                    'child_name' => '',
                    'child_label' => '',
    ...
    try {
        ...
        $sql = 'SELECT
                ...
                WHERE lcp.deleted_at IS NULL
                  ...
                  AND (lcp.percent_value IS NULL OR lcp.percent_value = 0)';
        $params = array();
        // No recortar por propiedad aquÃ­: el wizard debe poder renderizar
        // todo el Ã¡rbol no porcentual bajo cada padre seleccionado.
        // El control de "quÃ© raÃ­z puede elegirse" ya lo hace settings.
        ...
        foreach ($rows as $row) {
            ...
            $map[$parentId][$childId] = array(
                ...
                'label' => $label,
                'default_cents' => isset($row['default_unit_price_cents']) ? (int)$row['default_unit_price_cents'] : 0,
                'tax_rule_ids' => ''
            );
        }
...
// CategorÃ­as desde sp_portal_property_data (RS4)
$categoryRows = array();
try {
...
function wizard_apply_payments($reservationId, $actorUserId, $propertyCode = '')
{
    ...
    $fallbackCatalogs = rw_payment_catalogs_for_property($GLOBALS['paymentCatalogsByProperty'], $propertyCode);
    $fallbackCatalogId = 0;
    ...
    foreach ($methods as $idx => $methodIdRaw) {
        ...
        try {
            $catalog = null;
            $allMethods = rw_payment_catalogs_for_property($GLOBALS['paymentCatalogsByProperty'], $propertyCode);
            if ($methodId > 0) {
...
)
{
    ...
    foreach ($directChildrenByPair as $childSpec) {
        ...
        // Los nietos/bisnietos se crean desde recursiÃ³n; aquÃ­ solo se atienden hijos directos del padre principal.
        ...
        if ($childTotalCents <= 0) {
            // Aun sin total directo, disparar recursiÃ³n para respetar posibles overrides en descendientes.
            $visitPath = array();
            rw_upsert_fixed_children_tree(
                $reservationId,
                $folioId,
...
function rw_sync_reservation_origin_from_lodging($companyCode, $companyId, $reservationId, $lodgingCatalogId, $actorUserId)
{
    ...
    try {
        ...
        $stmt->execute(array($lodgingCatalogId, $reservationId, $companyId));
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($row) {
            $otaId = isset($row['id_ota_account']) ? (int)$row['id_ota_account'] : 0;
            $platform = strtolower(trim((string)(isset($row['platform']) ? $row['platform'] : '')));
            if (function_exists('pms_reservation_source_from_ota_platform')) {
                $sourceName = (string)pms_reservation_source_from_ota_platform($platform);
            } else {
                $sourceName = $platform === 'booking'
                    ? 'Booking'
                    : (($platform === 'airbnb' || $platform === 'abb')
                        ? 'Airbnb'
                        : ($platform === 'expedia' ? 'Expedia' : 'OTA'));
            }
            $sourceInput = $sourceName !== '' ? $sourceName : 'OTA';
        }
    } catch (Exception $e) {
        $otaId = 0;
        $sourceName = 'Directo';
...
function wizard_find_guest_id($companyCode, $email, $phone, $names, $last, $maiden) {
  $query = '';
...
$lockReservationFields = ($wizard['property_code'] !== ''
...
// Compatibilidad hacia atrÃ¡s: aceptar payload legado (un solo derivado).
...
$wizardPaymentCatalogOptions = rw_payment_catalogs_for_property($paymentCatalogsByProperty, $wizard['property_code']);
...
if ($wizard['property_code'] !== '' && $wizard['room_code'] !== '') {
    ...
    if ($roomMatch) {
        ...
        if ($wizard['category_code'] === '' && $roomCategoryCode !== '') {
            $wizard['category_code'] = $roomCategoryCode;
        } elseif ($wizard['category_code'] !== '') {
            $selectedCategoryCode = strtoupper((string)$wizard['category_code']);
            if ($roomCategoryCode === '' || $roomCategoryCode !== $selectedCategoryCode) {
                $wizard['room_code'] = '';
            }
        }
    } else {
...
// Cuando el usuario envÃ­a el paso 1, avanzamos a paso 2
// El paso 2 solo debe mostrarse cuando el usuario presiona "Siguiente"
// (seÃ±alado por el parÃ¡metro wizard_next desde el formulario).
...
if ($step === 2) {
    if ($wizard['property_code'] === '' || $wizard['room_code'] === '' || $wizard['check_in'] === '' || $wizard['check_out'] === '' || $wizard['category_code'] === '') {
        $errors[] = 'Completa propiedad, categoria, habitacion y fechas.';
        $step = 1;
    ...
    if (!$errors) {
        ...
        if ($editingReservationId > 0) {
            ...
            try {
                ...
                $resultSets = pms_call_procedure('sp_create_reservation_hold', array(
                    $wizard['property_code'],
                    $wizard['room_code'],
                    $wizard['check_in'],
                    $wizard['check_out'],
...
if ($step === 3 && isset($_POST['wizard_confirm']) && $_POST['wizard_confirm'] === '1') {
    ...
    try {
        ...
        // Compatibilidad con SP actual: solo admite un derivado explÃ­cito.
        $childCatalogId = 0;
        $childAmountCents = 0;
        $childTotalCents = 0;
        ...
        if ($editingReservationId > 0) {
            ...
            if ($guestIdValue > 0) {
	            ...
	            if ($replaceLodgingMode && !in_array($editingReservationStatus, array('confirmado', 'en casa', 'encasa'), true)) {
                ...
                if (!$paymentFailed) {
	                ...
	                if ($replaceLodgingMode) {
	                    ...
	                    $oldPrimaryLineItemId = isset($primaryLodging['id_line_item']) ? (int)$primaryLodging['id_line_item'] : 0;
	                    if ($oldPrimaryLineItemId <= 0) {
	                        throw new Exception('No se encontro el hospedaje principal actual para reemplazar.');
	                    }
	                    if ($targetFolioId <= 0 && isset($primaryLodging['id_folio'])) {
	                        $targetFolioId = (int)$primaryLodging['id_folio'];
	                    }
	                    if ($targetFolioId <= 0) {
	                        $targetFolioId = rw_get_latest_folio_id($editingReservationId);
	                    }
	                    if ($targetFolioId <= 0) {
	                        throw new Exception('No se encontro un folio activo para aplicar el cambio de hospedaje.');
	                    }
	                    rw_delete_line_item_tree($editingReservationId, $oldPrimaryLineItemId, $actorUserId);
	                    $serviceDateForLodging = trim((string)$wizard['check_in']) !== ''
	                        ? (string)$wizard['check_in']
	                        : (isset($primaryLodging['service_date']) && (string)$primaryLodging['service_date'] !== ''
	                            ? (string)$primaryLodging['service_date']
	                            : date('Y-m-d'));
            ...
            if ($newReservationId > 0 && $newFolioId <= 0 && $guestIdValue > 0) {
                try {
                    pms_call_procedure('sp_reservation_add_folio', array(
                        $companyCode,
                        $newReservationId,
...
<div class="subtab-info wizard-lightbox-host" id="wizard-lightbox-host">
  ...
  <?php endif; ?>
    ...
    <?php if ($step === 1): ?>
      ...
      <?php if (!empty($wizardExtraLodgingConcepts)): ?>
        <?php foreach ($wizardExtraLodgingConcepts as $extraIdx => $extraConceptId): ?>
          <input type="hidden" name="wizard_lodging_concept_extra[]" value="<?php echo (int)$extraConceptId; ?>">
          <input type="hidden" name="wizard_lodging_total_extra[]" value="<?php echo htmlspecialchars(isset($wizardExtraLodgingTotals[$extraIdx]) ? (string)$wizardExtraLodgingTotals[$extraIdx] : '', ENT_QUOTES, 'UTF-8'); ?>">
          <input type="hidden" name="wizard_lodging_children_json_extra[]" value="<?php echo htmlspecialchars(isset($wizardExtraChildrenJson[$extraIdx]) ? (string)$wizardExtraChildrenJson[$extraIdx] : '[]', ENT_QUOTES, 'UTF-8'); ?>">
        <?php endforeach; ?>
      <?php endif; ?>
      ...
      <div class="form-section">
        ...
        <div class="form-grid grid-3">
          ...
          <label class="<?php echo wizard_field_class($wizard['category_code'], true) . ($lockReservationFields ? ' is-locked' : ''); ?>">Categoria *
            <select name="wizard_category" required>
      ...
      <div class="form-section">
        <h4>InformaciÃ³n del huÃ©sped</h4>
        <div class="form-grid grid-3">
        ...
        <div class="form-actions wizard-actions wizard-actions-step1">
          ...
          <?php if (!$replaceLodgingMode): ?>
            <button type="submit" class="button-primary wizard-create-no-price" name="wizard_create_now" value="1">
              <?php echo $editingReservationId > 0 ? 'Confirmar reservacion' : 'Crear reservaciÃ³n sin precio'; ?>
            </button>
          <?php endif; ?>
        </div>
    <?php elseif ($step === 2): ?>
      ...
      <div class="form-section" style="padding:0;border:none;background:transparent;">
        ...
        <?php $selectedExtraLodgings = $wizardExtraLodgingConcepts; ?>
        <?php $selectedExtraTotals = $wizardExtraLodgingTotals; ?>
        <div style="margin-bottom:10px;">
          <div class="payment-row-title" style="display:flex;align-items:center;justify-content:space-between;gap:8px;">
            <span style="font-size:1.05rem;font-weight:800;letter-spacing:.02em;">Precios de hospedaje</span>
            <button type="button" class="button-secondary" id="wizard-add-lodging-concept">Agregar precio de hospedaje</button>
          </div>
        ...
        <div id="wizard-extra-lodging-list"></div>
        <template id="wizard-extra-lodging-template">
          <div class="payment-block wizard-extra-lodging-row" style="margin-top:10px; border:1px solid rgba(59,130,246,.25); border-radius:12px; padding:16px 18px; background:rgba(6,24,48,.45);">
            <div class="form-grid grid-2">
              <label>
                Precio de hospedaje
                <select name="wizard_lodging_concept_extra[]" class="wizard-extra-lodging-select">
              ...
              <div style="display:flex;gap:8px;align-items:flex-end;">
                <label style="flex:1;">
                  Total
                  <input type="number" step="0.01" min="0" name="wizard_lodging_total_extra[]" class="wizard-extra-lodging-total" placeholder="0.00">
                </label>
                <button type="button" class="button-secondary js-remove-extra-lodging">Quitar</button>
              </div>
            </div>
            <div class="payment-block wizard-extra-child-wrap" style="display:none; margin-top:10px; border:1px solid rgba(59,130,246,.25); border-radius:10px; padding:10px 12px; background:rgba(8,24,46,.35);">
              <div class="wizard-extra-child-list"></div>
              <input type="hidden" name="wizard_lodging_children_json_extra[]" class="wizard-extra-children-json" value="[]">
              <div class="wizard-extra-child-info muted" style="margin-top:8px; display:none;"></div>
            </div>
          </div>
        </template>
      ...
      <div class="form-actions">
        ...
        <button type="submit" class="button-primary" name="wizard_confirm" value="1">
          <?php echo ($replaceLodgingMode && $editingReservationId > 0) ? 'Guardar cambio de hospedaje' : 'Crear reserva'; ?>
        </button>
      </div>
    <?php endif; ?>
  </form>
  ...
  function initWizard() {
    ...
    function populateDependentFields(forceReset) {
      ...
      cats.forEach(function (cat) {
        var ccode = normalizeCode(cat.code || '');
        if (!ccode) return;
        appendOption(categorySelect, ccode, buildCategoryLabel(cat), ccode === currentCategory);
      });
      ...
      rooms.forEach(function (room) {
        var rcode = normalizeCode(room.code || '');
        if (!rcode) return;
        if (currentCategory && !roomMatchesSelectedCategory(room, currentCategory, selectedCategoryId)) {
          return;
        }
        var roomCategoryCode = normalizeCode(
    ...
    function refreshTotals(forceUpdate) {
      ...
      if (extraLodgingList) {
        extraLodgingList.querySelectorAll('.wizard-extra-lodging-total').forEach(function (input) {
          grandTotalCents += toCents(parseVal(input));
        });
        extraLodgingList.querySelectorAll('.wizard-extra-child-total-field').forEach(function (input) {
          grandTotalCents += toCents(parseVal(input));
        });
      }
      ...
      if (extraLodgingList) {
        extraLodgingList.querySelectorAll('.wizard-extra-lodging-select').forEach(function (select) {
    ...
    var addExtraLodgingBtn = document.getElementById('wizard-add-lodging-concept');
    var extraLodgingList = document.getElementById('wizard-extra-lodging-list');
    var extraLodgingTpl = document.getElementById('wizard-extra-lodging-template');
    var initialExtraConcepts = <?php echo json_encode(array_values($wizardExtraLodgingConcepts), JSON_UNESCAPED_UNICODE); ?>;
    var initialExtraTotals = <?php echo json_encode(array_values($wizardExtraLodgingTotals), JSON_UNESCAPED_UNICODE); ?>;
    var initialExtraChildrenJson = <?php echo json_encode(array_values($wizardExtraChildrenJson), JSON_UNESCAPED_UNICODE); ?>;

    function getAllLodgingSelects() {
      var selects = [];
      if (lodgingSelect) selects.push(lodgingSelect);
      if (extraLodgingList) {
        extraLodgingList.querySelectorAll('.wizard-extra-lodging-select').forEach(function (select) {
          selects.push(select);
        });
    ...
    function isConceptUsedElsewhere(currentSelect, value) {
      if (!value) return false;
      var used = false;
      getAllLodgingSelects().forEach(function (select) {
        if (used || select === currentSelect) return;
    ...
    function refreshUniqueLodgingOptions() {
      var selectedMap = {};
      getAllLodgingSelects().forEach(function (select) {
      ...
      getAllLodgingSelects().forEach(function (select) {
        var ownValue = String(select.value || '');
        Array.prototype.forEach.call(select.options, function (opt) {
          ...
          opt.disabled = !!selectedMap[optVal] && optVal !== ownValue;
        });
      });
    }
    ...
    function bindExtraRow(row) {
      if (!row) return;
      var select = row.querySelector('.wizard-extra-lodging-select');
      var totalInput = row.querySelector('.wizard-extra-lodging-total');
      var childWrap = row.querySelector('.wizard-extra-child-wrap');
      var childList = row.querySelector('.wizard-extra-child-list');
      var childJsonInput = row.querySelector('.wizard-extra-children-json');
      var childInfo = row.querySelector('.wizard-extra-child-info');
      var extraChildStateByCatalog = {};
      if (childJsonInput && childJsonInput.value) {
        try {
          var parsedState = JSON.parse(childJsonInput.value);
          if (Array.isArray(parsedState)) {
            parsedState.forEach(function (child) {
              var childId = parseInt(child && child.catalog_id ? child.catalog_id : 0, 10) || 0;
              if (!childId) return;
              var parentId = parseInt(child && child.parent_catalog_id ? child.parent_catalog_id : 0, 10) || 0;
              var pairKey = String(parentId) + ':' + String(childId);
              extraChildStateByCatalog[pairKey] = {
                total: child && child.total_cents != null ? (Number(child.total_cents) / 100).toFixed(2) : ''
              };
            });
          }
        } catch (e) {}
      ...
      function snapshotExtraChildState() {
        if (!childList) return;
        childList.querySelectorAll('.wizard-extra-child-item').forEach(function (item) {
          var childIdInput = item.querySelector('.wizard-extra-child-catalog-id');
          var childParentIdInput = item.querySelector('.wizard-extra-child-parent-catalog-id');
          var childTotalInput = item.querySelector('.wizard-extra-child-total-field');
          var childId = childIdInput ? (parseInt(childIdInput.value || '0', 10) || 0) : 0;
          var parentId = childParentIdInput ? (parseInt(childParentIdInput.value || '0', 10) || 0) : 0;
          if (!childId) return;
          var pairKey = String(parentId) + ':' + String(childId);
          extraChildStateByCatalog[pairKey] = {
            total: childTotalInput ? String(childTotalInput.value || '') : ''
          };
        });
      }

      function updateExtraChildrenJson() {
        if (!childJsonInput || !childList) return;
        var payload = [];
        childList.querySelectorAll('.wizard-extra-child-item').forEach(function (item) {
          var childIdInput = item.querySelector('.wizard-extra-child-catalog-id');
          var childParentIdInput = item.querySelector('.wizard-extra-child-parent-catalog-id');
          ...
          var parentId = childParentIdInput ? (parseInt(childParentIdInput.value || '0', 10) || 0) : 0;
          if (!childId) return;
          var totalVal = parseExtraValue(childTotalInput ? childTotalInput.value : '');
          var totalCents = Math.round(totalVal * 100);
          payload.push({
            catalog_id: childId,
            parent_catalog_id: parentId,
            total_cents: totalCents > 0 ? totalCents : 0
          });
        });
        childJsonInput.value = JSON.stringify(payload);
      }

      function renderExtraChildRows() {
        if (!select || !childWrap || !childList) return;
        var opt = select.options[select.selectedIndex];
        var rootCatalogId = parseInt(select.value || '0', 10) || 0;
        var fixedTree = buildFixedChildrenTree(rootCatalogId, opt);
        var fixedChildren = flattenFixedChildrenTree(fixedTree, []).filter(function (child) {
          var childId = parseInt(child && child.id ? child.id : 0, 10) || 0;
          return childId > 0;
        ...
        if (!fixedChildren.length) {
          ...
          if (childInfo) childInfo.textContent = '';
          if (childJsonInput) childJsonInput.value = '[]';
          return;
        }
        ...
        function renderNode(child) {
          var childId = parseInt(child.id || '0', 10) || 0;
          var childName = String((child.label || child.name || '')).trim();
          var defaultTotal = (parseInt(child.default_cents || '0', 10) || 0) / 100;
          var depth = parseInt(child.depth || 0, 10) || 0;
          var parentId = parseInt(child.parent_catalog_id || '0', 10) || 0;
          var pairKey = String(parentId) + ':' + String(childId);
          var saved = extraChildStateByCatalog[pairKey] || extraChildStateByCatalog[childId] || null;
          var total = saved && String(saved.total || '').trim() !== '' ? parseExtraValue(saved.total) : defaultTotal;
          var block = document.createElement('div');
          block.className = 'payment-block wizard-extra-child-item';
          block.style.marginTop = '10px';
          block.style.marginLeft = String(depth * 16) + 'px';
          block.innerHTML =
            '<div class="payment-row-title">' + (childName !== '' ? ('Derivados ' + (depth > 0 ? new Array(depth + 1).join('\u2014 ') : '') + childName) : 'Derivados') + '</div>' +
            '<div class="form-grid grid-1 wizard-extra-child-grid">' +
              '<label>Total' +
                '<input type="text" inputmode="decimal" autocomplete="off" class="wizard-extra-child-total-field" placeholder="0.00">' +
              '</label>' +
            '</div>' +
            '<input type="hidden" class="wizard-extra-child-catalog-id" value="' + String(childId) + '">' +
            '<input type="hidden" class="wizard-extra-child-parent-catalog-id" value="' + String(parseInt(child.parent_catalog_id || '0', 10) || 0) + '">';
          var totalInput = block.querySelector('.wizard-extra-child-total-field');
          if (totalInput) totalInput.value = total.toFixed(2);
          if (totalInput) {
            totalInput.addEventListener('input', function () {
              updateExtraChildrenJson();
              refreshTotals(true);
            });
            totalInput.addEventListener('blur', function () {
              var val = parseExtraValue(totalInput.value);
              totalInput.value = val.toFixed(2);
              updateExtraChildrenJson();
              refreshTotals(true);
            });
          }
          childList.appendChild(block);
          if (Array.isArray(child.children) && child.children.length) {
            child.children.forEach(renderNode);
          }
        }
      ...
      function update() {
        if (!select) return;
        snapshotExtraChildState();
        renderExtraChildRows();
        refreshUniqueLodgingOptions();
        refreshTotals(true);
      }
      ...
      if (select) {
        select.addEventListener('change', function () {
          ...
          if (selected && isConceptUsedElsewhere(select, selected)) {
            select.value = '';
          }
          update();
        });
    ...
    function addExtraRow(conceptId, totalValue, childrenJsonValue) {
      if (!extraLodgingList || !extraLodgingTpl) return;
      var frag = extraLodgingTpl.content.cloneNode(true);
      var row = frag.querySelector('.wizard-extra-lodging-row');
      if (row) {
        var select = row.querySelector('.wizard-extra-lodging-select');
        var totalInput = row.querySelector('.wizard-extra-lodging-total');
        var childrenJsonInput = row.querySelector('.wizard-extra-children-json');
        if (select && conceptId) {
          var desiredConcept = String(conceptId);
          if (!isConceptUsedElsewhere(select, desiredConcept)) {
            select.value = desiredConcept;
          }
        }
        if (totalInput && totalValue !== undefined && totalValue !== null && String(totalValue) !== '') {
          totalInput.value = String(totalValue);
        }
        if (childrenJsonInput) {
          childrenJsonInput.value = (childrenJsonValue !== undefined && childrenJsonValue !== null && String(childrenJsonValue) !== '')
            ? String(childrenJsonValue)
            : '[]';
        }
      }
      extraLodgingList.appendChild(frag);
      var appendedRows = extraLodgingList.querySelectorAll('.wizard-extra-lodging-row');
      var lastRow = appendedRows.length ? appendedRows[appendedRows.length - 1] : null;
      bindExtraRow(lastRow);
      refreshUniqueLodgingOptions();
    }

    if (lodgingSelect) {
      lodgingSelect.addEventListener('change', function () {
        var selected = String(lodgingSelect.value || '');
        if (selected && isConceptUsedElsewhere(lodgingSelect, selected)) {
          lodgingSelect.value = '';
        }
    ...
    if (addExtraLodgingBtn && extraLodgingList && extraLodgingTpl) {
      if (Array.isArray(initialExtraConcepts) && initialExtraConcepts.length) {
        initialExtraConcepts.forEach(function (conceptId, idx) {
          var totalVal = Array.isArray(initialExtraTotals) ? initialExtraTotals[idx] : '';
          var childrenJsonVal = Array.isArray(initialExtraChildrenJson) ? initialExtraChildrenJson[idx] : '[]';
          addExtraRow(conceptId, totalVal, childrenJsonVal);
        });
      }
      addExtraLodgingBtn.addEventListener('click', function () {
        addExtraRow('', '', '[]');
        bindWizardFieldStates();
      });
      extraLodgingList.addEventListener('click', function (ev) {
        var btn = ev.target.closest('.js-remove-extra-lodging');
        if (!btn) return;
        var row = btn.closest('.wizard-extra-lodging-row');
        if (row) row.remove();
        refreshUniqueLodgingOptions();
        refreshTotals(true);
        bindWizardFieldStates();
