...
foreach ($targetProperties as $propCode) {
    ...
    if ($orderMode === 'category_availability') {
        ...
        $propertyOtaRows = isset($otaAccountsByProperty[$propCode]) && is_array($otaAccountsByProperty[$propCode])
            ? $otaAccountsByProperty[$propCode]
            : array();
        ...
        foreach ($categoryIndex as $cid => $cat) {
            ...
            foreach ($propertyOtaRows as $otaRow) {
                ...
                if ($otaMode === 'none' || abs($otaValue) < 0.0001) {
                    continue;
                }
    ...
    <?php foreach ($calendarBundles as $bundleIndex => $bundle): ?>
      ...
      <?php endif; ?>
        ...
        <?php if ($categoryView && $calendarDays): ?>
          ...
          <table class="calendar-table calendar-category-availability" data-property-code="<?php echo htmlspecialchars((string)$propertyCodeRow, ENT_QUOTES, 'UTF-8'); ?>" data-owner-copy-enabled="<?php echo $calendarOwnerBulkCopyEnabled ? '1' : '0'; ?>" style="--calendar-day-count: <?php echo (int)$dayCount; ?>; --calendar-room-width: <?php echo (int)$categoryRoomWidth; ?>px; --calendar-day-width: <?php echo (int)$categoryDayWidth; ?>px; width: <?php echo (int)$categoryTableWidth; ?>px; min-width: <?php echo (int)$categoryTableWidth; ?>px;">
              <thead>
                <?php
                  $monthNames = array(
                      1 => 'Enero',
              ...
              <tbody>
                ...
                ?>
                  ...
                  <?php foreach ((isset($cat['ota_price_rows']) && is_array($cat['ota_price_rows'])) ? $cat['ota_price_rows'] : array() as $otaPriceRow): ?>
                    ...
                    ?>
                    <tr class="calendar-rateplan-row calendar-rateplan-row-ota<?php echo $otaColorHex !== '' ? ' has-ota-accent' : ''; ?>"<?php echo $otaRowStyle !== '' ? ' style="' . htmlspecialchars($otaRowStyle, ENT_QUOTES, 'UTF-8') . '"' : ''; ?>>
                      <th class="room-cell rateplan-cell">
                        <div class="rateplan-name"><?php echo htmlspecialchars((string)$otaPriceRow['ota_name'], ENT_QUOTES, 'UTF-8'); ?></div>
                      </th>
                      <?php for ($i = 0; $i < $dayCount; $i++):
                        $dayInfo = isset($calendarDayMap[$i]) ? $calendarDayMap[$i] : null;
                        $dateKey = $dayInfo && isset($dayInfo['date_key']) ? (string)$dayInfo['date_key'] : '';
      ...
      <?php elseif ($rooms && $calendarDays): ?>
        ...
        ?>
        <div class="calendar-scroll" data-scroll-date="<?php echo htmlspecialchars($baseDate, ENT_QUOTES, 'UTF-8'); ?>" data-today="<?php echo htmlspecialchars(date('Y-m-d'), ENT_QUOTES, 'UTF-8'); ?>">
          <table class="calendar-table" data-property-code="<?php echo htmlspecialchars((string)$propertyCodeRow, ENT_QUOTES, 'UTF-8'); ?>" data-owner-copy-enabled="<?php echo $calendarOwnerBulkCopyEnabled ? '1' : '0'; ?>" style="--calendar-day-count: <?php echo (int)$dayCount; ?>; --calendar-room-width: <?php echo (int)$roomLabelWidth; ?>px; --calendar-day-width: <?php echo (int)$roomDayWidth; ?>px; width: <?php echo (int)$roomTableWidth; ?>px; min-width: <?php echo (int)$roomTableWidth; ?>px;">
            <thead>
              <?php
                $monthNames = array(
                    1 => 'Enero',
