...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function buildCalendarReturnState() {
    return {
      savedAt: Date.now(),
  ...
  function saveCalendarReturnState() {
    const state = buildCalendarReturnState();
    try {
    ...
    overlay.innerHTML = `
      <div class="calendar-quickres-window" role="dialog" aria-modal="true" aria-label="Reserva rapida">
        ...
        <form class="calendar-quickres-form" novalidate>
          ...
          <label>
            Apellido (opcional)
            <input type="text" name="guest_last_name" autocomplete="family-name" placeholder="Apellido">
          </label>
          <label>
            Precio total (MXN)
    ...
    if (form) {
      form.addEventListener('submit', function (event) {
        ...
        if (!priceRaw || !Number.isFinite(priceValue) || priceValue <= 0) {
          alert('Captura un monto de hospedaje mayor a 0.');
          return;
        }
