...
document.addEventListener('DOMContentLoaded', function () {
  ...
  actionBar.innerHTML = `
    ...
    <div class="calendar-action-group calendar-action-reservations">
      ...
      <button type="button" class="calendar-action-btn button-secondary js-calendar-add-charges" hidden>Agregar cargos</button>
  ...
  function enterMoveMode(sourceReservation) {
    ...
    document.querySelectorAll('.calendar-cell.is-empty').forEach(function (cell) {
      ...
      if (propCode !== sourcePropertyCode) {
        return;
      }
    ...
    overlay.innerHTML = `
      <div class="calendar-quickres-window" role="dialog" aria-modal="true" aria-label="Registrar pago">
        ...
        <form class="calendar-quickres-form" novalidate>
          ...
          <div class="calendar-quickres-actions">
            <button type="button" class="button-secondary calendar-quickres-cancel">Cancelar</button>
            <button type="submit" class="button-primary">Confirmar y registrar pago</button>
          </div>
        </form>
      </div>
    `;
    ...
    function paymentRowsForProperty(propertyCode) {
      ...
      merged.forEach(function (row) {
        ...
        rows.push({
          ...
          label: (row && (row.label || row.name)) ? String(row.label || row.name) : ('Concepto #' + String(id))
        });
      });
      return rows;
    }
  ...
  function updateActionBar() {
    ...
    const canAddCharges = singleReservation
      ...
      && target.reservationId > 0
      && !hasFolio
      && (targetStatus === 'confirmado' || targetStatus === 'en casa');
  ...
  document.addEventListener('click', function (event) {
    ...
    if (moveModeActive) {
      ...
      if (moveTargetCell && calendarTables.some(function (t) { return t.contains(moveTargetCell); }) && moveModeSource) {
        ...
        const sourceLabel = (moveModeSource.roomCode || '') + ' | ' + (moveModeSource.checkIn || '') + ' -> ' + (moveModeSource.checkOut || '');
        const targetLabel = targetRoomCode + ' | ' + targetCheckIn + ' -> ' + targetCheckOut;
        ...
        addInput('property_code', propertyCode || targetPropertyCode);
  ...
  if (quickResButton) {
    quickResButton.addEventListener('click', function () {
      ...
      openQuickReservationLightbox(
        ...
        function (payload) {
          const notesParts = [];
          if (payload.guestName) notesParts.push(payload.guestName);
          if (payload.priceRaw) notesParts.push(payload.priceRaw);
          if (payload.notesRaw) notesParts.push(payload.notesRaw);
          ...
          addInput('quick_property_code', range.propertyCode || '');
          addInput('quick_room_code', range.roomCode || '');
          addInput('quick_check_in', range.startDate);
          addInput('quick_check_out', addDays(range.endDateInclusive, 1));
          addInput('quick_price', payload.priceRaw || '');
          addInput('quick_source_id', payload.sourceId || 0);
          addInput('quick_source', '');
          addInput('quick_ota_account_id', payload.otaAccountId || 0);
          addInput('quick_notes', combinedNotes);
          if (range.propertyCode) {
            addInput('property_code', range.propertyCode);
          }
  ...
  if (payResButton) {
    payResButton.addEventListener('click', function () {
      ...
      openPaymentLightbox({
        ...
        const currentPropertyFilter = document.querySelector('.calendar-filters select[name="property_code"]');
        const propertyCode = currentPropertyFilter ? (currentPropertyFilter.value || '') : '';
        addInput('property_code', propertyCode || (res.propertyCode || ''));
        const currentStart = document.querySelector('input[name="start_date"]');
        if (currentStart && currentStart.value) addInput('start_date', currentStart.value);
        const viewMode = document.querySelector('select[name="view_mode"]');
        if (viewMode && viewMode.value) addInput('view_mode', viewMode.value);
  ...
  if (addChargesButton) {
    addChargesButton.addEventListener('click', function () {
      ...
      if ((normalizedStatus !== 'confirmado' && normalizedStatus !== 'en casa') || res.folioCount > 0) {
        return;
      }
      const form = document.createElement('form');
      form.method = 'post';
      ...
      addInput('wizard_step', '2');
      addInput('wizard_force_step', '2');
      appendWizardReturnContextInputs(addInput, returnContext);
      document.body.appendChild(form);
      form.submit();
    });
  ...
  if (advanceStatusButton) {
    advanceStatusButton.addEventListener('click', function () {
      ...
      if (normalizedStatus === 'apartado') {
        ...
        if (res.propertyCode) addInput('property_code', res.propertyCode);
      }
