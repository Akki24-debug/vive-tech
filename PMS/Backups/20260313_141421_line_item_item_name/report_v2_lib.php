    ...
    function reports_v2_fetch_report_base_rows(PDO $pdo, $companyId, array $filters, $limit = 500, $rowSource = 'reservation', $lineItemTypeScope = '')
    {
        ...
        if ($search !== '') {
            ...
            if ($rowSource === 'line_item') {
                $searchParts[] = 'lic.item_name LIKE ?';
                $searchParts[] = 'li.description LIKE ?';
            }
        ...
        if ($rowSource === 'line_item') {
            ...
            $sql = 'SELECT
                        ...
                        COALESCE(lic.item_name, \'\') AS base_line_item_name,
                        CASE
                            WHEN COALESCE(lic.item_name, \'\') <> \'\' THEN TRIM(CONCAT(
                                lic.item_name,
                                CASE
                                    WHEN COALESCE((
                                        SELECT parent_lic.item_name
...
if (!function_exists('reports_v2_fetch_line_item_metrics')) {
    function reports_v2_fetch_line_item_metrics(PDO $pdo, array $reservationIds)
    {
        ...
        $sql = 'SELECT
                    ...
                    MAX(COALESCE(parent_lic.item_name, \'\')) AS item_parent_name,
                    CASE
