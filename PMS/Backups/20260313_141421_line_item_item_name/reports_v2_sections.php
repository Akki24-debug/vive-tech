            ...
            <th data-field-id="<?php echo $fieldId; ?>" data-calculate-total="<?php echo !empty($fieldRow['calculate_total']) ? '1' : '0'; ?>" data-format-hint="<?php echo reports_v2_h(isset($fieldRow['format_hint']) ? (string)$fieldRow['format_hint'] : 'auto'); ?>">
              <div class="report-v2-th-inner">
                <div class="report-v2-th-main">
                  <button type="button" class="report-v2-sort-button" data-sort-field-id="<?php echo $fieldId; ?>" aria-label="Ordenar columna <?php echo reports_v2_h($fieldRow['display_name']); ?>">
                  ...
                  <span class="report-v2-th-label"><?php echo reports_v2_h($fieldRow['display_name']); ?></span>
                </div>
                <div class="report-v2-th-actions">
                  <button type="button" class="report-v2-filter-button" data-filter-field-id="<?php echo $fieldId; ?>" aria-label="Filtrar columna <?php echo reports_v2_h($fieldRow['display_name']); ?>">
        ...
        <tr <?php echo implode(' ', $rowAttributes); ?>>
          ...
          <?php if (!empty($ctx['hasEditableReservationFields'])): ?>
            <td class="report-v2-actions-cell">
              <?php if ($rowCanEdit && !$rowIsEditing): ?>
                <form method="get" class="report-v2-inline-form">
                  <?php reports_v2_render_run_state_hidden_inputs($ctx); ?>
                  <input type="hidden" name="report_edit_reservation_id" value="<?php echo $rowReservationId; ?>">
                  <button type="submit">Editar</button>
                </form>
              <?php elseif ($rowIsEditing): ?>
                <form method="post" class="report-v2-inline-form" id="<?php echo reports_v2_h($rowFormId); ?>">
          ...
          <tr class="report-v2-group-row report-v2-group-row--level-<?php echo $level; ?>" data-group-node-id="<?php echo reports_v2_h($nodeId); ?>" data-group-level="<?php echo $level; ?>">
            <td colspan="<?php echo $colspan; ?>">
              <div class="report-v2-group-row-inner">
                <span class="report-v2-group-row-kicker"><?php echo $level === 2 ? 'Subseccion 2' : 'Subseccion 3'; ?></span>
        ...
        <form method="post" class="report-v2-stack">
          ...
          <?php
            ...
            $selectedTemplateSubdivideByFieldIdLevel2 = $selectedTemplate && isset($selectedTemplate['subdivide_by_field_id_level_2'])
                ...
                : 0;
            $selectedTemplateSubdivideByFieldIdLevel3 = $selectedTemplate && isset($selectedTemplate['subdivide_by_field_id_level_3'])
                ? (int)$selectedTemplate['subdivide_by_field_id_level_3']
                : 0;
            ...
            $selectedTemplateSubdivideShowTotalsLevel3 = !$selectedTemplate || !isset($selectedTemplate['subdivide_show_totals_level_3']) || !empty($selectedTemplate['subdivide_show_totals_level_3']) ? 1 : 0;
          ?>
          ...
          <div class="report-v2-form-grid report-v2-form-grid--2">
            <label><span>Subseccion 3</span><select name="template_subdivide_by_field_id_level_3"><option value="0">Sin subseccion</option><?php foreach ($templateFields as $templateFieldOption): $templateFieldOptionId = isset($templateFieldOption['id_report_template_field']) ? (int)$templateFieldOption['id_report_template_field'] : 0; ?><option value="<?php echo $templateFieldOptionId; ?>" <?php echo $selectedTemplateSubdivideByFieldIdLevel3 === $templateFieldOptionId ? 'selected' : ''; ?>><?php echo reports_v2_h($templateFieldOption['display_name']); ?></option><?php endforeach; ?></select></label>
            <label><span>Totales seccion 3</span><select name="template_subdivide_show_totals_level_3"><option value="1" <?php echo $selectedTemplateSubdivideShowTotalsLevel3 ? 'selected' : ''; ?>>Si</option><option value="0" <?php echo !$selectedTemplateSubdivideShowTotalsLevel3 ? 'selected' : ''; ?>>No</option></select></label>
          </div>
        ...
        <div class="report-v2-card report-v2-stack">
          ...
          <form method="post" class="report-v2-stack">
            ...
            <div class="report-v2-form-grid">
              <label><span>Alias de columna</span><input type="text" name="display_name" value="<?php echo reports_v2_h($selectedField ? $selectedField['display_name'] : ''); ?>" placeholder="Ejemplo: Total OTA, Tipo de pago, Huesped. Si lo dejas vacio toma el nombre base"></label>
            ...
            <div class="report-v2-form-grid report-v2-form-grid--3" data-field-panel="line_item" style="display:none;">
              ...
              if (
                  ...
                  foreach ($lineItemCatalogs as $catalogRow) {
                      ...
                      } elseif ($selectedDisplayName !== '' && $selectedDisplayName === ($itemName . ' - nombre padre')) {
            ...
            <div class="report-v2-form-grid" data-field-panel="line_item" style="display:none;">
              <label><span>Conceptos adicionales (Ctrl/Cmd para varios)</span><select name="line_item_catalog_ids[]" multiple size="8"><?php foreach ($lineItemCatalogs as $catalogRow): $parts = array(); if (!empty($catalogRow['property_name'])) { $parts[] = $catalogRow['property_name']; } if (!empty($catalogRow['category_name'])) { $parts[] = $catalogRow['category_name']; } if (!empty($catalogRow['subcategory_name']) && $catalogRow['subcategory_name'] !== $catalogRow['category_name']) { $parts[] = $catalogRow['subcategory_name']; } $parts[] = $catalogRow['item_name']; $catalogId = (int)$catalogRow['id_line_item_catalog']; ?><option value="<?php echo $catalogId; ?>" <?php echo in_array($catalogId, $selectedLineItemCatalogIds, true) ? 'selected' : ''; ?>><?php echo reports_v2_h(implode(' / ', array_filter($parts))); ?></option><?php endforeach; ?></select></label>
            </div>
...
<section class="report-v2-card report-v2-stack">
  ...
  <?php else: ?>
          ...
          <?php if ($reportHasCalculatedTotals): ?>
            <div class="report-v2-stack">
              ...
              <table class="report-v2-table report-v2-result-totals-table" data-result-scope="global-totals">
                <thead><tr><?php foreach ($templateFields as $fieldRow): ?><th><?php echo reports_v2_h($fieldRow['display_name']); ?></th><?php endforeach; ?><?php if ($hasEditableReservationFields): ?><th></th><?php endif; ?></tr></thead>
          ...
          <tbody>
            <?php foreach ($reportRows as $reportRow): ?>
              <?php
                ...
                $rowCheckOutDate = isset($rowBase['reservation_check_out_date']) ? (string)$rowBase['reservation_check_out_date'] : '';
              ?>
              <tr class="report-v2-result-row" data-row-created-date="<?php echo reports_v2_h($rowCreatedDate); ?>" data-row-service-date="<?php echo reports_v2_h($rowServiceDate); ?>" data-row-check-in-date="<?php echo reports_v2_h($rowCheckInDate); ?>" data-row-check-out-date="<?php echo reports_v2_h($rowCheckOutDate); ?>">
                <?php foreach ($templateFields as $fieldRow): ?>
                  <?php
                    $fieldId = isset($fieldRow['id_report_template_field']) ? (int)$fieldRow['id_report_template_field'] : 0;
                    $cell = isset($reportRow['cells'][$fieldId]) ? $reportRow['cells'][$fieldId] : array('display' => '');
    ...
    <div class="report-v2-context-menu" id="report-v2-column-context-menu" aria-hidden="true">
      <button type="button" id="report-v2-column-context-subdivide">Dividir reporte por columna</button>
      <button type="button" id="report-v2-column-context-advanced-filter">Filtro especializado</button>
    </div>
  <?php endif; ?>
</section>
<?php endif; ?>
