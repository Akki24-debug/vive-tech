/* ============================================================================
   ROLLBACK / CLEANUP IMPORT MASIVO (ULTIMOS MINUTOS)
   - Objetivo principal: reservas insertadas registradas en tmp_reservation_import_log
   - Fallback: reservas recientes con code SP-* / IMP-*
   - Limpia:
     reservation, folio, line_item, reservation_note, reservation_interest,
     reservation_group_member, reservation_message_log, obligation_payment_log
   - Guest: soft-delete solo si queda huerfano (sin reservas activas)
   ============================================================================ */

SET @lookback_minutes := 180; -- ajusta: 60, 180, 1440, etc.
...
SELECT COUNT(*) AS target_folios FROM tmp_cleanup_target_folio;
SELECT COUNT(*) AS target_line_items FROM tmp_cleanup_target_line_item;
SELECT COUNT(*) AS target_guests FROM tmp_cleanup_target_guest;
...
ORDER BY r.created_at DESC, r.id_reservation DESC
LIMIT 500;
...
SELECT
  COUNT(*) AS still_active_folios
FROM folio f
JOIN tmp_cleanup_target_res t ON t.id_reservation = f.id_reservation
WHERE f.deleted_at IS NULL;

SELECT
  COUNT(*) AS still_active_line_items
FROM line_item li
