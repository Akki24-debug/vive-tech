...
proc:BEGIN
  ...
  SELECT p.id_property, p.id_company
    INTO v_id_property, v_id_company
  FROM property p
  ...
  SET v_last_name = TRIM(CONCAT(COALESCE(p_guest_last_name,''), ' ', COALESCE(p_guest_maiden_name,'')));
  ...
  IF p_guest_email IS NOT NULL AND p_guest_email <> '' THEN
    ...
    IF v_id_guest IS NULL THEN
      INSERT INTO guest (email, phone, names, last_name, language, is_active, created_at, updated_at)
      VALUES (p_guest_email, p_guest_phone, NULLIF(p_guest_names,''), NULLIF(v_last_name,''), 'es', 1, NOW(), NOW());
  ...
  ELSE
    ...
    IF (p_guest_names IS NOT NULL AND p_guest_names <> '')
      ...
      INSERT INTO guest (email, phone, names, last_name, language, is_active, created_at, updated_at)
      VALUES (NULL, NULLIF(p_guest_phone,''), NULLIF(p_guest_names,''), NULLIF(v_last_name,''), 'es', 1, NOW(), NOW());
      SET v_id_guest = LAST_INSERT_ID();
    END IF;
  END IF;
