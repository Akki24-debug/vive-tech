...
if (!function_exists('settings_rate_column')) {
    ...
    {
        try {
            $stmt = $pdo->query("SHOW COLUMNS FROM line_item_catalog LIKE 'rate_percent'");
...
if (isset($_POST['settings_action']) && $_POST['settings_action'] === 'save_timezone') {
    ...
    if (!function_exists('pms_timezone_is_valid') || !pms_timezone_is_valid($timezoneInput)) {
        ...
        try {
            ...
            try {
                $stmtColumn = $pdo->query("SHOW COLUMNS FROM pms_settings LIKE 'timezone'");
                $hasTimezoneColumn = (bool)$stmtColumn->fetch(PDO::FETCH_ASSOC);
            } catch (Exception $e) {
                $hasTimezoneColumn = false;
            }
            if (!$hasTimezoneColumn) {
                throw new Exception('Falta columna timezone en pms_settings. Ejecuta la migracion migrate_pms_settings_timezone.sql');
            }
...
if (isset($_POST['settings_action']) && $_POST['settings_action'] === 'recalc_folio_nodes') {
    ...
    if (!$selectedReservationIds) {
        ...
        try {
            ...
            $validReservationIds = array_values(array_unique(array_map('intval', $stmtValidReservations->fetchAll(PDO::FETCH_COLUMN))));
