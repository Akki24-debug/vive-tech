...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function ensureCalendarCopyMenu() {
    ...
    calendarCopyMenuButton.textContent = 'Copiar';
    ...
    calendarCopyMenuButton.addEventListener('click', function () {
      ...
      copyCalendarTextToClipboard(calendarCopyPayload.text)
        .then(function () {
          showCalendarCopyFeedback('Copiado: ' + calendarCopyPayload.count + ' valores.');
        })
        .catch(function () {
  ...
  function buildCalendarCopyPayload(rowHeaderCell) {
    ...
    const row = rowHeaderCell.closest('tr');
    const table = rowHeaderCell.closest('table.calendar-table');
    if (!row || !table) {
      return null;
    }
    const dataCells = Array.prototype.slice.call(row.querySelectorAll('td'));
    const rowType = detectCalendarCopyRowType(dataCells);
    if (!rowType) {
      return null;
    }
    ...
    if (startPosition < 0 || startPosition >= valuesByPosition.length) {
      startPosition = 0;
    }

    const copiedValues = valuesByPosition.slice(startPosition, startPosition + 30);
    if (!copiedValues.some(function (value) { return value !== ''; })) {
      return null;
    }

    return {
      count: copiedValues.length,
      text: copiedValues.join(',')
    };
  }
  ...
  function openCalendarCopyMenu(clientX, clientY, payload) {
    ...
    if (calendarCopyMenuButton) {
      calendarCopyMenuButton.textContent = payload.count === 30
        ? 'Copiar 30 valores'
        : 'Copiar ' + payload.count + ' valores';
    }
    menu.hidden = false;
    menu.style.left = '0px';
    menu.style.top = '0px';
  ...
  function bindCalendarToolbarFormOwner() {
    ...
    if (todayBtn && todayBtn.dataset.calendarTodayBound !== '1') {
      ...
      todayBtn.addEventListener('click', function (event) {
        event.preventDefault();
        markCalendarContextRestoreSkip();
        saveCalendarReturnState(buildCalendarSubmitContext(calendarControlsForm, todayBtn));
        if (typeof calendarControlsForm.requestSubmit === 'function') {
  ...
  function updateActionBar() {
    ...
    if (isMobileCalendar()) {
      ...
      const anyVisible = [blockButton, createButton, quickResButton, priceSetButton, priceClearButton, priceMinButton, priceMidButton, priceMaxButton, viewResButton, moveResButton, duplicateResButton, payResButton, addChargesButton, advanceStatusButton, noShowButton, cancelResButton, groupResButton, deleteBlocksButton]
        .some(function (btn) { return !!btn && !btn.hidden; });
      actionBar.classList.toggle('is-visible', anyVisible);
    } else {
      [blockButton, createButton, quickResButton, priceSetButton, priceClearButton, priceMinButton, priceMidButton, priceMaxButton, viewResButton, moveResButton, duplicateResButton, payResButton, addChargesButton, advanceStatusButton, noShowButton, cancelResButton, groupResButton, deleteBlocksButton].forEach(function (btn) {
        if (!btn) return;
  ...
  function submitCustomSelectedPrice() {
    ...
    submitBulkPriceSelection('bulk_rateplan_override_set', buildSelectedPricePayload('custom', normalized), notes);
  }
  ...
  if (calendarCopyEnabled) {
    document.addEventListener('contextmenu', function (event) {
      const headerCell = event.target.closest('tbody th.room-cell, tbody th.calendar-category-cell');
      if (!headerCell) {
        closeCalendarCopyMenu();
        return;
      }
