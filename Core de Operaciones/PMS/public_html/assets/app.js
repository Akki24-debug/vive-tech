...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const calendarCopyEnabled = calendarTables.some(function (table) {
    return (table.getAttribute('data-owner-copy-enabled') || '') === '1';
  });
  let calendarCopyMenu = null;
  let calendarCopyMenuButton = null;
  let calendarCopyMenuUsdButton = null;
  let calendarCopyPayload = null;
  let calendarCopyFeedback = null;
  let calendarCopyFeedbackTimer = 0;
  let calendarOtaSelectionMenu = null;
  let calendarOtaSelectionPayload = null;
  ...
  function getCalendarFabIconSvg(actionKey) {
    const icons = {
      ...
      'price-set': '<svg viewBox="0 0 24 24" focusable="false"><path d="M12 3.5v17M3.5 12h17"></path><rect x="4.5" y="4.5" width="15" height="15" rx="3"></rect></svg>',
      'price-clear': '<svg viewBox="0 0 24 24" focusable="false"><path d="M7 7l10 10M17 7 7 17"></path><rect x="4.5" y="4.5" width="15" height="15" rx="3"></rect></svg>',
      'price-min': '<svg viewBox="0 0 24 24" focusable="false"><path d="M6 16.5h12M6 12.5h8M6 8.5h4"></path></svg>',
      'price-mid': '<svg viewBox="0 0 24 24" focusable="false"><path d="M6 15.5h12M6 12h12M6 8.5h12"></path></svg>',
      'price-max': '<svg viewBox="0 0 24 24" focusable="false"><path d="M6 16.5h4M6 12.5h8M6 8.5h12"></path></svg>',
      ...
      duplicate: '<svg viewBox="0 0 24 24" focusable="false"><rect x="8" y="8" width="11" height="11" rx="2"></rect><path d="M6 15H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v1"></path></svg>',
  ...
  function isCalendarPreferredControl(field) {
  ...
  function readCalendarControlValue(selector, scope) {
    const root = scope && typeof scope.querySelectorAll === 'function' ? scope : document;
    const fields = Array.prototype.slice.call(root.querySelectorAll(selector));
    if (!fields.length) {
      return '';
    }
    const preferred = fields.filter(isCalendarPreferredControl);
    const field = (preferred.length ? preferred : fields)[(preferred.length ? preferred : fields).length - 1];
    if (!field) {
      return '';
    }
  ...
  function readCalendarNamedControlValue(name, scope) {
    if (!name) {
      return '';
    }
    return readCalendarControlValue(`[name="${name}"]`, scope);
  }
  ...
  function ensureCalendarCopyMenu() {
    if (calendarCopyMenu) {
      return calendarCopyMenu;
    }
    calendarCopyMenu = document.createElement('div');
    calendarCopyMenu.className = 'calendar-copy-menu';
    calendarCopyMenu.hidden = true;

    calendarCopyMenuButton = document.createElement('button');
    calendarCopyMenuButton.type = 'button';
    calendarCopyMenuButton.className = 'calendar-copy-menu-button';
    calendarCopyMenuButton.textContent = 'Copiar datos de 45 dias';
    calendarCopyMenu.appendChild(calendarCopyMenuButton);

    calendarCopyMenuButton.addEventListener('click', function () {
      if (!calendarCopyPayload || !calendarCopyPayload.text) {
        closeCalendarCopyMenu();
        return;
      }
      if (!window.confirm('Copiar al portapapeles los datos de 45 dias de esta propiedad?')) {
        closeCalendarCopyMenu();
        return;
      }
      copyCalendarTextToClipboard(calendarCopyPayload.text)
        .then(function () {
          showCalendarCopyFeedback('Copiadas ' + calendarCopyPayload.categoryCount + ' categorias.');
        })
        .catch(function () {
          showCalendarCopyFeedback('No se pudo copiar al portapapeles.', true);
        })
        .finally(function () {
          closeCalendarCopyMenu();
        });
    ...
    document.body.appendChild(calendarCopyMenu);
    return calendarCopyMenu;
  }

  function closeCalendarCopyMenu() {
    calendarCopyPayload = null;
    if (!calendarCopyMenu) {
      return;
    }
    calendarCopyMenu.hidden = true;
    calendarCopyMenu.style.left = '';
    calendarCopyMenu.style.top = '';
  }
  ...
  function ensureCalendarOtaSelectionMenu() {
    if (calendarOtaSelectionMenu) {
      return calendarOtaSelectionMenu;
    }
    calendarOtaSelectionMenu = document.createElement('div');
    calendarOtaSelectionMenu.className = 'calendar-copy-menu';
    calendarOtaSelectionMenu.hidden = true;

    [
      { scope: 'row', label: 'Seleccionar 45 dias de esta fila' },
      { scope: 'property', label: 'Seleccionar 45 dias de toda la propiedad' },
      { scope: 'all', label: 'Seleccionar 45 dias de todas las propiedades' }
    ].forEach(function (option, index) {
      const button = document.createElement('button');
      button.type = 'button';
      button.className = 'calendar-copy-menu-button' + (index > 0 ? ' is-secondary' : '');
      button.textContent = option.label;
      button.addEventListener('click', function () {
        if (!calendarOtaSelectionPayload) {
          closeCalendarOtaSelectionMenu();
          return;
        }
        const cells = getOtaSelectionCells(calendarOtaSelectionPayload, option.scope);
        if (cells.length) {
          selectPriceCells(cells, cells[0]);
        }
        closeCalendarOtaSelectionMenu();
      });
      calendarOtaSelectionMenu.appendChild(button);
    });

    document.body.appendChild(calendarOtaSelectionMenu);
    return calendarOtaSelectionMenu;
  }

  function closeCalendarOtaSelectionMenu() {
    calendarOtaSelectionPayload = null;
    if (!calendarOtaSelectionMenu) {
      return;
    }
    calendarOtaSelectionMenu.hidden = true;
    calendarOtaSelectionMenu.style.left = '';
    calendarOtaSelectionMenu.style.top = '';
  }

  function openCalendarOtaSelectionMenu(clientX, clientY, payload) {
    if (!payload || !(Number(payload.otaAccountId) > 0)) {
      closeCalendarOtaSelectionMenu();
      return;
    }
    closeCalendarCopyMenu();
    const menu = ensureCalendarOtaSelectionMenu();
    calendarOtaSelectionPayload = payload;
    menu.hidden = false;
    menu.style.left = '0px';
    menu.style.top = '0px';
    ...
    menu.style.left = Math.max(8, Math.min(Math.round(clientX), maxLeft)) + 'px';
    menu.style.top = Math.max(8, Math.min(Math.round(clientY), maxTop)) + 'px';
  }

  function ensureCalendarCopyFeedback() {
    if (calendarCopyFeedback) {
      return calendarCopyFeedback;
    }
    calendarCopyFeedback = document.createElement('div');
    calendarCopyFeedback.className = 'calendar-copy-feedback';
    calendarCopyFeedback.hidden = true;
    document.body.appendChild(calendarCopyFeedback);
    return calendarCopyFeedback;
  }

  function showCalendarCopyFeedback(message, isError) {
    const feedback = ensureCalendarCopyFeedback();
    if (calendarCopyFeedbackTimer) {
      window.clearTimeout(calendarCopyFeedbackTimer);
      calendarCopyFeedbackTimer = 0;
    }
    feedback.textContent = message || '';
    feedback.classList.toggle('is-error', !!isError);
    feedback.hidden = false;
    calendarCopyFeedbackTimer = window.setTimeout(function () {
  ...
  function fallbackCalendarClipboardCopy(text) {
  ...
  function copyCalendarTextToClipboard(text) {
    const payload = (text || '').toString();
    if (!payload) {
      return Promise.reject(new Error('empty_copy_payload'));
    }
    if (navigator.clipboard && typeof navigator.clipboard.writeText === 'function') {
      return navigator.clipboard.writeText(payload).catch(function () {
        if (fallbackCalendarClipboardCopy(payload)) {
          return true;
        }
    ...
    return Promise.reject(new Error('clipboard_copy_failed'));
  }

  function normalizeCalendarCopyValue(rawValue) {
    const compact = (rawValue || '').toString().replace(/\s+/g, '').trim();
  ...
  function normalizeCalendarUsdCopyValue(rawValue, usdRate) {
    const compact = (rawValue || '').toString().replace(/\s+/g, '').trim();
    const rate = Number(usdRate || 0);
    if (!compact || !/^-?\d+(?:\.\d+)?$/.test(compact) || !Number.isFinite(rate) || rate <= 0) {
      return '';
    }
    const converted = Number(compact) * rate;
  ...
  function extractCalendarCopyCellValue(cell, rowType) {
    if (!(cell instanceof HTMLElement)) {
      return '';
    }
    if (rowType === 'availability') {
      const countNode = cell.querySelector('.availability-count');
      return normalizeCalendarCopyValue(countNode ? countNode.textContent : cell.textContent);
    }
    if (rowType === 'rateplan') {
      return normalizeCalendarCopyValue(cell.getAttribute('data-price') || cell.textContent);
    }
    return '';
  }

  function detectCalendarCopyRowType(cells) {
    if (!Array.isArray(cells) || !cells.length) {
      return '';
    }
    if (cells.some(function (cell) { return cell.classList.contains('availability'); })) {
      return 'availability';
    }
    if (cells.some(function (cell) { return cell.classList.contains('rateplan-price'); })) {
      return 'rateplan';
    }
    return '';
  }

  function extractCalendarCopySeriesFromRow(row, rowType, startPosition, maxItems) {
    if (!(row instanceof HTMLElement)) {
      return [];
    }
    const dataCells = Array.prototype.slice.call(row.querySelectorAll('td'));
    if (!dataCells.length) {
      return [];
    }
    ...
    dataCells.forEach(function (cell) {
      ...
      const value = extractCalendarCopyCellValue(cell, rowType);
      for (let offset = 0; offset < span; offset += 1) {
        valuesByPosition[position + offset] = value;
      }
      position += span;
    });
    const safeStart = Math.max(0, startPosition || 0);
    const safeLength = Math.max(0, maxItems || 0);
    const values = valuesByPosition.slice(safeStart, safeStart + safeLength);
    while (values.length < safeLength) {
      values.push('');
    }
    return values;
  }

  function buildCalendarCategoryCopyBlock(table, availabilityRow, startPosition, maxItems) {
    if (!(table instanceof HTMLElement) || !(availabilityRow instanceof HTMLElement)) {
      return null;
    }
    const headerCell = availabilityRow.querySelector('th.category-cell');
    if (!(headerCell instanceof HTMLElement)) {
      return null;
    }
    const categoryNameNode = headerCell.querySelector('strong');
    const roomCodesNode = headerCell.querySelector('.category-room-list');
    const categoryName = ((categoryNameNode ? categoryNameNode.textContent : '') || '').trim();
    const roomCodes = ((roomCodesNode ? roomCodesNode.textContent : '') || '').trim();
    if (!categoryName) {
      return null;
    }

    const availabilityValues = extractCalendarCopySeriesFromRow(availabilityRow, 'availability', startPosition, maxItems);
    let bookingValues = [];
    let sibling = availabilityRow.nextElementSibling;
    while (sibling && !sibling.classList.contains('calendar-category-availability-row')) {
      if (
        sibling.classList.contains('calendar-rateplan-row-ota')
        // Herramienta interna oculta: este export usa Booking como OTA fija.
        && ((sibling.querySelector('.rateplan-name') ? sibling.querySelector('.rateplan-name').textContent : '') || '').trim() === 'Booking'
      ) {
        bookingValues = extractCalendarCopySeriesFromRow(sibling, 'rateplan', startPosition, maxItems);
        break;
      }
      sibling = sibling.nextElementSibling;
    }
    while (bookingValues.length < maxItems) {
      bookingValues.push('');
    }

    const usdRate = parseFloat(table.getAttribute('data-usd-rate') || '0');
    const bookingUsdValues = bookingValues.map(function (value) {
      return normalizeCalendarUsdCopyValue(value, usdRate);
    });
    ...
    return {
      ...
      roomCodes,
      text: [
        `${categoryName} (${roomCodes}):`,
        `- Disponibilidad: ${availabilityValues.join(',')}`,
        `- Precio en pesos: ${bookingValues.join(',')}`,
        `- Precio USD: ${bookingUsdValues.join(',')}`
      ].join('\n')
    };
  }

  function buildCalendarCopyPayloadForTable(table) {
    if (!(table instanceof HTMLElement)) {
      return null;
    }
    const todayIso = (
      (table.closest('.calendar-scroll') && table.closest('.calendar-scroll').getAttribute('data-today'))
      || getTodayIsoDate()
    ).trim();
    const dayHeaders = Array.prototype.slice.call(table.querySelectorAll('thead .calendar-day-row th[data-date]'));
    let startPosition = dayHeaders.findIndex(function (header) {
      return (header.getAttribute('data-date') || '').trim() === todayIso;
    });
    if (startPosition < 0) {
      startPosition = 0;
    }
    const maxItems = 45;
    const propertyNameNode = table.querySelector('thead .calendar-property-header .calendar-property-name');
    const propertyName = ((propertyNameNode ? propertyNameNode.textContent : '') || '').trim() || 'Sin propiedad';
    const categoryRows = Array.prototype.slice.call(table.querySelectorAll('tbody tr.calendar-category-availability-row'));
    const blocks = categoryRows.map(function (row) {
      return buildCalendarCategoryCopyBlock(table, row, startPosition, maxItems);
    ...
    return {
      ...
      text: [
        ...
        blocks.map(function (block) { return block.text; }).join('\n\n')
      ].join('\n')
    };
  }

  function buildCalendarCopyPayload(rowHeaderCell) {
    if (!(rowHeaderCell instanceof HTMLElement)) {
      return null;
    }
    const sourceTable = rowHeaderCell.closest('table.calendar-table.calendar-category-availability');
    if (!sourceTable) {
      return null;
    }
    const tables = Array.prototype.slice.call(
      document.querySelectorAll('table.calendar-table.calendar-category-availability[data-owner-copy-enabled="1"]')
    ).filter(function (table) {
      return table instanceof HTMLElement && !table.hidden;
    });
    const tablePayloads = (tables.length ? tables : [sourceTable]).map(function (table) {
      return buildCalendarCopyPayloadForTable(table);
    ...
    return {
      categoryCount: tablePayloads.reduce(function (total, payload) {
        return total + (payload.categoryCount || 0);
      }, 0),
      propertyCount: tablePayloads.length,
      text: tablePayloads.map(function (payload) {
        return payload.text;
      }).join('\n\n')
    };
  }

  function openCalendarCopyMenu(clientX, clientY, payload) {
    if (!payload || !payload.text) {
      return;
    }
    closeCalendarOtaSelectionMenu();
    const menu = ensureCalendarCopyMenu();
    calendarCopyPayload = payload;
    if (calendarCopyMenuButton) {
      calendarCopyMenuButton.textContent = 'Copiar datos de 45 dias';
    }
    menu.hidden = false;
    menu.style.left = '0px';
  ...
  function currentCalendarReturnContext() {
    return {
      propertyCode: readCalendarNamedControlValue('property_code', calendarControlsForm || document),
      startDate: readCalendarNamedControlValue('start_date', calendarControlsForm || document),
      viewMode: readCalendarNamedControlValue('view_mode', calendarControlsForm || document),
      orderMode: readCalendarNamedControlValue('order_mode', calendarControlsForm || document),
      currentSubtab: readCalendarNamedControlValue('calendar_current_subtab', calendarControlsForm || document),
      dirtyTabs: readCalendarNamedControlValue('calendar_dirty_tabs', calendarControlsForm || document)
    };
  }
  ...
  function readCalendarContextFromForm(form) {
    ...
    return {
      propertyCode: readCalendarNamedControlValue('property_code', form),
      startDate: readCalendarNamedControlValue('start_date', form),
      viewMode: readCalendarNamedControlValue('view_mode', form),
      orderMode: readCalendarNamedControlValue('order_mode', form),
      currentSubtab: readCalendarNamedControlValue('calendar_current_subtab', form),
      dirtyTabs: readCalendarNamedControlValue('calendar_dirty_tabs', form)
    };
  }

  function buildCalendarSubmitContext(form, submitter) {
    const context = readCalendarContextFromForm(form);
    const trigger = submitter || null;
    if (
      trigger
      && (trigger.name || '') === 'nav_action'
      && (trigger.value || '') === 'today'
    ) {
      context.startDate = getTodayIsoDate();
    }
    return context;
  }
  ...
  function buildCalendarReturnState(contextOverride) {
    return {
      ...
      scrollers: calendarScrollers.map(function (scroll, index) {
        const shouldLockScrollDate = (scroll.getAttribute('data-lock-scroll-date') || '0') === '1';
        const maxLeft = Math.max(0, scroll.scrollWidth - scroll.clientWidth);
        const leftRatio = shouldLockScrollDate
          ? 0
          : (maxLeft > 0 ? (scroll.scrollLeft / maxLeft) : 0);
        return {
          index: index,
  ...
  function resolveCalendarScrollLeftForMetrics(targetScroll, sourceMetrics) {
    ...
    const desiredLeft = sourceMetrics.dateOffset * targetMetrics.dayWidth;
  ...
  function ensureCalendarGlobalScrollbar() {
    ...
    calendarGlobalScrollbarRoot.addEventListener('wheel', handleCalendarHorizontalWheel, { passive: false, capture: true });
  }
  ...
  function getCalendarHorizontalScrollState() {
    ...
    const maxLeft = Math.max(0, primaryScroll.scrollWidth - primaryScroll.clientWidth);
    const currentLeft = calendarGlobalScrollbarInner
      ? calendarGlobalScrollbarInner.scrollLeft
      : primaryScroll.scrollLeft;
    return {
      primaryScroll,
      maxLeft,
      currentLeft
  ...
  function setCalendarHorizontalScrollLeft(nextLeft) {
    const scrollState = getCalendarHorizontalScrollState();
    if (!scrollState) {
      return false;
    }
    const clampedLeft = Math.max(0, Math.min(scrollState.maxLeft, Math.round(nextLeft)));
    if (Math.abs(clampedLeft - scrollState.currentLeft) <= 0.5) {
      return false;
    }
    if (calendarGlobalScrollbarInner) {
      calendarGlobalScrollbarInner.scrollLeft = clampedLeft;
      return true;
    }
    scrollState.primaryScroll.scrollLeft = clampedLeft;
    syncCalendarScrollFrom(scrollState.primaryScroll);
    return true;
  }
  ...
  function handleCalendarArrowKeyScroll(event) {
    ...
    const scrollState = getCalendarHorizontalScrollState();
    if (!scrollState || scrollState.maxLeft <= 0) {
      return;
    }
    const metrics = getCalendarScrollMetrics(scrollState.primaryScroll);
    const step = metrics
      ? Math.max(96, Math.round(metrics.dayWidth * 2))
      : Math.max(96, Math.round(scrollState.primaryScroll.clientWidth * 0.18));
    const direction = isRight ? 1 : -1;
    const nextLeft = scrollState.currentLeft + (direction * step);
    event.preventDefault();
    setCalendarHorizontalScrollLeft(nextLeft);
  }
  ...
  function resolveCalendarHorizontalWheelDelta(event) {
    ...
    if (Math.abs(deltaX) <= 0.01 && typeof event.wheelDeltaX === 'number' && Math.abs(event.wheelDeltaX) > 0.01) {
    ...
    const fallbackDeltaY = Math.abs(deltaY) > 0.01 ? deltaY : wheelDeltaY;
    if (Math.abs(deltaX) <= 0.01 && event.shiftKey && Math.abs(deltaY) > 0.01) {
      deltaX = deltaY;
    }
    if (Math.abs(deltaX) <= 0.01 && event.shiftKey && Math.abs(fallbackDeltaY) > 0.01) {
      deltaX = fallbackDeltaY;
    }
    if (Math.abs(deltaX) <= 0.01) {
      return 0;
    }
    if (!event.shiftKey && Math.abs(deltaX) < 3 && Math.abs(fallbackDeltaY) > Math.abs(deltaX) * 4) {
      return 0;
    }
    return deltaX;
  }
  ...
  function handleCalendarHorizontalWheel(event) {
    ...
    if (scrollState.maxLeft <= 0) {
      return;
    }
    const nextLeft = scrollState.currentLeft + deltaX;
    event.preventDefault();
    setCalendarHorizontalScrollLeft(nextLeft);
  }
  ...
  function syncCalendarTableWidths() {
    calendarTables.forEach((table) => {
      ...
      let measuredRoomWidth = isCategoryAvailability ? 98 : 118;
      ...
      if (!table.dataset.baseCellHeight) {
        ...
        const fallbackCellHeight = isCategoryAvailability ? 42 : 76;
      ...
      const desktopRoomMin = isCategoryAvailability ? 96 : 112;
      const desktopRoomMax = isCategoryAvailability ? 172 : 168;
      ...
      const baseCellHeight = parseInt(table.dataset.baseCellHeight || '', 10) || (isCategoryAvailability ? 42 : 76);
      if (isMobileCalendar()) {
        const minCellHeight = isCategoryAvailability ? 20 : 28;
        ...
        const minCellHeight = isCategoryAvailability ? 22 : 36;
  ...
  function setCalendarStartDateControlValue(targetDate) {
    if (!targetDate) {
      return;
    }
    const selectors = [
      '#calendar-controls-form input[name="start_date"]',
      '.calendar-action-bar input[name="start_date"]'
    ];
  ...
  function scrollAllCalendarsToDate(targetDate) {
    if (!targetDate || !calendarScrollers.length) {
      return false;
    }
    let didScroll = false;
    calendarScrollers.forEach(function (scroll) {
      if (scrollCalendarToDate(scroll, targetDate)) {
        didScroll = true;
      }
    });
    if (!didScroll) {
  ...
  function initCalendarScrollPosition() {
    calendarScrollers.forEach((scroll) => {
      ...
      const applyInitialScroll = function () {
        if (shouldLockScrollDate) {
          scroll.scrollLeft = 0;
          return;
        }
  ...
  function getCalendarScrollMetrics(scroll) {
    ...
    const dateOffset = Math.max(0, scroll.scrollLeft) / dayWidth;
    return {
      roomWidth: roomWidth,
      dayWidth: dayWidth,
  ...
  function bindCalendarScrollSync() {
    calendarScrollers.forEach(function (scroll) {
      ...
      scroll.addEventListener('wheel', handleCalendarHorizontalWheel, { passive: false, capture: true });
    });
  }
  ...
  updateCalendarScrollbarState();
  window.addEventListener('keydown', handleCalendarArrowKeyScroll, { capture: true });
  window.addEventListener('wheel', handleCalendarHorizontalWheel, { passive: false, capture: true });
  ...
  function bindCalendarToolbarFormOwner() {
    ...
    if (todayBtn && todayBtn.dataset.calendarTodayBound !== '1') {
      ...
      todayBtn.addEventListener('click', function (event) {
        ...
        setCalendarStartDateControlValue(todayIso);
        if (scrollAllCalendarsToDate(todayIso)) {
    ...
    if (calendarControlsForm.dataset.calendarSubmitStateBound !== '1') {
      ...
      calendarControlsForm.addEventListener('submit', function (event) {
        saveCalendarReturnState(buildCalendarSubmitContext(calendarControlsForm, event.submitter || null));
      });
    }
  }
  ...
  actionBar.innerHTML = `
    <div class="calendar-action-group calendar-action-cells">
      ...
      <button type="button" class="calendar-action-btn button-secondary js-calendar-create-block" hidden>Crear reservaciones en bloque</button>
      <button type="button" class="calendar-action-btn button-secondary js-calendar-quick-res" hidden>Reserva r&aacute;pida</button>
    </div>
    <div class="calendar-action-group calendar-action-prices">
      <button type="button" class="calendar-action-btn button-secondary js-calendar-price-set" hidden>Fijar precio</button>
      <button type="button" class="calendar-action-btn button-secondary js-calendar-price-percent" hidden>Fijar %</button>
      <button type="button" class="calendar-action-btn button-secondary js-calendar-price-clear" hidden>Quitar precio fijo</button>
      <button type="button" class="calendar-action-btn button-secondary js-calendar-price-min" hidden>Fijar en m&iacute;nimo</button>
      <button type="button" class="calendar-action-btn button-secondary js-calendar-price-mid" hidden>Fijar en medio</button>
      <button type="button" class="calendar-action-btn button-secondary js-calendar-price-max" hidden>Fijar precio m&aacute;ximo</button>
    </div>
    <div class="calendar-action-group calendar-action-reservations">
      ...
      <button type="button" class="calendar-action-btn button-secondary js-calendar-duplicate-res" hidden>Duplicar</button>
  ...
  const createBlockButton = actionBar.querySelector('.js-calendar-create-block');
  const quickResButton = actionBar.querySelector('.js-calendar-quick-res');
  const priceSetButton = actionBar.querySelector('.js-calendar-price-set');
  const pricePercentButton = actionBar.querySelector('.js-calendar-price-percent');
  const priceClearButton = actionBar.querySelector('.js-calendar-price-clear');
  const priceMinButton = actionBar.querySelector('.js-calendar-price-min');
  const priceMidButton = actionBar.querySelector('.js-calendar-price-mid');
  const priceMaxButton = actionBar.querySelector('.js-calendar-price-max');
  ...
  const duplicateResButton = actionBar.querySelector('.js-calendar-duplicate-res');
  ...
  let reservationData = [];
  let selectionType = null; // 'cell' | 'availability' | 'price' | 'reservation' | 'block'
  let selectionProperty = null;
  let calendarDeveloperMenu = null;
  let calendarDeveloperSelect = null;
  const lastSelectionAnchorByType = {
    cell: null,
    availability: null,
    price: null
  };
  ...
  function enterMoveMode(sourceReservation, modeKind) {
    ...
    const effectiveModeKind = modeKind === 'duplicate' ? 'duplicate' : 'move';
    if (!sourcePropertyCode || !sourceRoomCode || nights <= 0) {
      alert(effectiveModeKind === 'duplicate'
        ? 'No se pudo determinar fechas/habitacion para duplicar la reservacion.'
        : 'No se pudo determinar fechas/habitacion para mover la reservacion.');
      return;
    }
    ...
    moveModeKind = effectiveModeKind;
    ...
    if (!eligibleCells.length) {
      ...
      moveModeKind = '';
      alert(effectiveModeKind === 'duplicate'
        ? 'No hay espacios disponibles en calendario para duplicar esta reservacion.'
        : 'No hay espacios disponibles en calendario para mover esta reservacion.');
      return;
    }
  ...
  function clearSelection() {
    ...
    lastSelectionAnchorByType.cell = null;
    lastSelectionAnchorByType.availability = null;
    lastSelectionAnchorByType.price = null;
    window.pmsCalendarSelection = selectionData;
    updateEmptySelectionPriceVisuals();
    updateActionBar();
  }

  function setSelectionAnchor(type, cell) {
    if (!Object.prototype.hasOwnProperty.call(lastSelectionAnchorByType, type)) {
      return;
    }
    lastSelectionAnchorByType[type] = cell && cell.isConnected ? cell : null;
  }

  function getSelectionAnchor(type) {
    if (!Object.prototype.hasOwnProperty.call(lastSelectionAnchorByType, type)) {
      return null;
    }
    const anchor = lastSelectionAnchorByType[type];
    return anchor && anchor.isConnected ? anchor : null;
  }

  function getSelectionSelectorByType(type) {
    if (type === 'cell') return '.calendar-cell.is-empty';
    if (type === 'price') return '.js-rateplan-price';
    if (type === 'availability') return '.js-calendar-availability-select';
    return '';
  }

  function isSelectableCellForType(cell, type) {
    const selector = getSelectionSelectorByType(type);
    if (!cell || !selector || !cell.matches(selector)) {
      return false;
    }
    if (type === 'availability') {
      const availability = parseInt(cell.getAttribute('data-availability') || '0', 10);
      return Number.isFinite(availability) && availability > 0;
    }
    return true;
  }

  function getSelectableCellMeta(cell, type) {
    if (!isSelectableCellForType(cell, type)) {
      return null;
    }
    const row = cell.closest('tr');
    const section = row && row.parentElement ? row.parentElement : null;
    const table = cell.closest('.calendar-table');
    const propertyCode = (cell.getAttribute('data-property-code') || '').toUpperCase();
    const roomCode = (cell.getAttribute('data-room-code') || '').toUpperCase();
    const rowKey = cell.getAttribute('data-row-key') || '';
    const dayIndex = parseInt(cell.getAttribute('data-day-index') || '-1', 10);
    let rowIndex = -1;
    if (row && section) {
      rowIndex = Array.prototype.indexOf.call(section.children, row);
    }
    return {
      cell: cell,
      row: row,
  ...
  function getSelectionScopeForType(targetCell, type) {
    const anchorMeta = getSelectableCellMeta(getSelectionAnchor(type), type);
    const targetMeta = getSelectableCellMeta(targetCell, type);
    if (!anchorMeta || !targetMeta) {
      return null;
    }
    if (!anchorMeta.table || anchorMeta.table !== targetMeta.table) {
      return null;
    }
    if (!anchorMeta.propertyCode || anchorMeta.propertyCode !== targetMeta.propertyCode) {
      return null;
    }
    if (!Number.isFinite(anchorMeta.dayIndex) || anchorMeta.dayIndex < 0 || !Number.isFinite(targetMeta.dayIndex) || targetMeta.dayIndex < 0) {
      return null;
    }
    if (!Number.isFinite(anchorMeta.rowIndex) || anchorMeta.rowIndex < 0 || !Number.isFinite(targetMeta.rowIndex) || targetMeta.rowIndex < 0) {
      return null;
    }
    return {
      anchor: anchorMeta,
  ...
  function sortSelectionCells(cells, type) {
    const unique = [];
    cells.forEach(function (cell) {
      if (!isSelectableCellForType(cell, type)) {
        return;
      }
      if (unique.indexOf(cell) === -1) {
        unique.push(cell);
      }
    });
    unique.sort(function (a, b) {
      const aMeta = getSelectableCellMeta(a, type);
      const bMeta = getSelectableCellMeta(b, type);
      if (!aMeta || !bMeta) {
        return 0;
      }
      ...
      return aMeta.dayIndex - bMeta.dayIndex;
    });
    return unique;
  }

  function mergeSelectionCells(cellsToAdd, type, anchorCell) {
    const nextCells = selectionCells.slice();
    cellsToAdd.forEach(function (cell) {
      if (nextCells.indexOf(cell) === -1) {
        nextCells.push(cell);
      }
    });
    applyGenericSelection(sortSelectionCells(nextCells, type), type);
    setSelectionAnchor(type, anchorCell || null);
  }

  function resolveRowAnchorCell(rowCells, targetDayIndex, fallbackDayIndex) {
    if (!rowCells.length) {
      return null;
    }
    const exact = rowCells.find(function (cell) {
      return parseInt(cell.getAttribute('data-day-index') || '-1', 10) === targetDayIndex;
    });
    if (exact) {
      return exact;
    }
    const movingForward = targetDayIndex >= fallbackDayIndex;
    const sorted = rowCells.slice().sort(function (a, b) {
      const aDay = parseInt(a.getAttribute('data-day-index') || '-1', 10);
      const bDay = parseInt(b.getAttribute('data-day-index') || '-1', 10);
      if (movingForward) {
        return bDay - aDay;
      }
      return aDay - bDay;
  ...
  function selectShiftRangeForType(targetCell, type) {
    const scope = getSelectionScopeForType(targetCell, type);
    if (!scope) {
      return false;
    }
    const anchorMeta = scope.anchor;
    const targetMeta = scope.target;
    const minDay = Math.min(anchorMeta.dayIndex, targetMeta.dayIndex);
    const maxDay = Math.max(anchorMeta.dayIndex, targetMeta.dayIndex);
    const rowCells = Array.prototype.slice.call(anchorMeta.row.querySelectorAll(getSelectionSelectorByType(type)))
      .filter(function (cell) { return isSelectableCellForType(cell, type); });
    const rangeCells = rowCells.filter(function (cell) {
      const dayIndex = parseInt(cell.getAttribute('data-day-index') || '-1', 10);
      return Number.isFinite(dayIndex) && dayIndex >= minDay && dayIndex <= maxDay;
    });
    if (!rangeCells.length) {
      return false;
    }
    const nextAnchorCell = resolveRowAnchorCell(rangeCells, targetMeta.dayIndex, anchorMeta.dayIndex) || anchorMeta.cell;
    mergeSelectionCells(rangeCells, type, nextAnchorCell);
    return true;
  }

  function selectCtrlRectangleForType(targetCell, type) {
    const scope = getSelectionScopeForType(targetCell, type);
    if (!scope) {
      return false;
    }
    ...
    const minDay = Math.min(anchorMeta.dayIndex, targetMeta.dayIndex);
    const maxDay = Math.max(anchorMeta.dayIndex, targetMeta.dayIndex);
    const minRow = Math.min(anchorMeta.rowIndex, targetMeta.rowIndex);
    const maxRow = Math.max(anchorMeta.rowIndex, targetMeta.rowIndex);
    const tableCells = Array.prototype.slice.call(anchorMeta.table.querySelectorAll(getSelectionSelectorByType(type)))
      .filter(function (cell) { return isSelectableCellForType(cell, type); });
    const rectCells = tableCells.filter(function (cell) {
      const meta = getSelectableCellMeta(cell, type);
      if (!meta) {
        return false;
      }
      return meta.propertyCode === anchorMeta.propertyCode
        && meta.rowIndex >= minRow
        && meta.rowIndex <= maxRow
        && meta.dayIndex >= minDay
        && meta.dayIndex <= maxDay;
    });
    if (!rectCells.length) {
      return false;
    }
    mergeSelectionCells(rectCells, type, targetCell);
    return true;
  }
  ...
  function getCellData(cell) {
    return {
      ...
      categoryId: parseInt(cell.getAttribute('data-category-id') || '0', 10),
      roomId: parseInt(cell.getAttribute('data-room-id') || '0', 10),
      selectionKind: cell.getAttribute('data-selection-kind') || '',
      rowKey: cell.getAttribute('data-row-key') || '',
      availability: parseInt(cell.getAttribute('data-availability') || '0', 10),
      price: cell.getAttribute('data-price') || '',
      priceCents: parseInt(cell.getAttribute('data-price-cents') || '0', 10),
      hasFixedPrice: (cell.getAttribute('data-has-fixed-price') || '0') === '1',
      otaAccountId: parseInt(cell.getAttribute('data-ota-account-id') || '0', 10),
      otaName: cell.getAttribute('data-ota-name') || '',
      presetMinCents: parseInt(cell.getAttribute('data-min-price-cents') || '0', 10),
      presetMidCents: parseInt(cell.getAttribute('data-mid-price-cents') || '0', 10),
      presetMaxCents: parseInt(cell.getAttribute('data-max-price-cents') || '0', 10)
    };
  }
  ...
  function applyGenericSelection(nextCells, nextType) {
    document.querySelectorAll('.calendar-cell.is-range-selected').forEach(function (cell) {
      cell.classList.remove('is-range-selected');
    });
    selectionCells = Array.isArray(nextCells) ? nextCells.slice() : [];
    ...
    window.pmsCalendarSelection = selectionData;
    selectionType = selectionData.length ? nextType : null;
    if (selectionData.length) {
      selectionProperty = ((selectionData[0].propertyCode || '') + '').toUpperCase();
    } else {
      selectionProperty = null;
    }
    updateEmptySelectionPriceVisuals();
  ...
  function formatCalendarSelectionAmount(cents) {
    const normalized = Number.isFinite(cents) ? Math.max(0, Math.round(cents)) : 0;
    return String(Math.round(normalized / 100));
  }

  function resetEmptySelectionPriceVisuals() {
    document.querySelectorAll('.calendar-cell.is-empty').forEach(function (cell) {
      cell.classList.remove('is-selection-price-block', 'is-selection-price-start', 'is-selection-price-middle', 'is-selection-price-end', 'is-selection-price-solo', 'is-selection-price-label-host');
      cell.removeAttribute('data-selection-price-block-size');
      cell.style.removeProperty('--selection-price-block-size');
      const label = cell.querySelector('.calendar-empty-selection-label');
      if (label) {
        label.textContent = '';
      }
    });
  ...
  function updateEmptySelectionPriceVisuals() {
    resetEmptySelectionPriceVisuals();
    if (selectionType !== 'cell' || !selectionCells.length) {
      return;
    }
    ...
    selectionCells.forEach(function (cell) {
      ...
      const propCode = (data.propertyCode || '').toUpperCase();
      const roomCode = (data.roomCode || '').toUpperCase();
      if (!propCode || !roomCode) {
        return;
      }
      const key = propCode + '::' + roomCode;
      if (!grouped.has(key)) {
    ...
    grouped.forEach(function (items) {
      items.sort(function (a, b) {
        if (Number.isFinite(a.dayIndex) && Number.isFinite(b.dayIndex) && a.dayIndex !== b.dayIndex) {
      ...
      items.forEach(function (item) {
        ...
        const consecutiveByIndex = Number.isFinite(prev.dayIndex) && Number.isFinite(item.dayIndex)
          ? (item.dayIndex - prev.dayIndex === 1)
          : false;
        const consecutiveByDate = prev.date && item.date
          ? (addDays(prev.date, 1) === item.date)
          : false;
        if (consecutiveByIndex || consecutiveByDate) {
          current.push(item);
        } else {
          blocks.push([item]);
        }
      ...
      blocks.forEach(function (block) {
        const totalCents = block.reduce(function (sum, item) {
          return sum + (Number.isFinite(item.priceCents) ? item.priceCents : 0);
        }, 0);
        const blockSize = block.length;
        block.forEach(function (item, index) {
          const cell = item.cell;
          if (!cell) return;
          cell.classList.add('is-selection-price-block');
          cell.setAttribute('data-selection-price-block-size', String(blockSize));
          cell.style.setProperty('--selection-price-block-size', String(blockSize));
          if (blockSize === 1) {
            cell.classList.add('is-selection-price-solo');
          } else if (index === 0) {
            cell.classList.add('is-selection-price-start');
          } else if (index === blockSize - 1) {
            cell.classList.add('is-selection-price-end');
          } else {
            cell.classList.add('is-selection-price-middle');
          }

          const label = cell.querySelector('.calendar-empty-selection-label');
          if (!label) return;
          if (index === 0) {
            cell.classList.add('is-selection-price-label-host');
            label.textContent = formatCalendarSelectionAmount(totalCents);
          } else {
            label.textContent = '';
          }
        });
  ...
  function toggleCellSelection(cell) {
    ...
    applyGenericSelection(sortSelectionCells(selectionCells, 'cell'), 'cell');
    setSelectionAnchor('cell', cell);
  }

  function togglePriceSelection(cell) {
    if (selectionType && selectionType !== 'price') return;
    const propCode = (cell.getAttribute('data-property-code') || '').toUpperCase();
    if (selectionProperty && selectionProperty !== propCode) return;
    if (!selectionProperty) selectionProperty = propCode;

    let nextCells = selectionCells.slice();
    const alreadySelected = cell.classList.contains('is-range-selected');
    if (alreadySelected) {
      nextCells = nextCells.filter(function (item) { return item !== cell; });
    } else {
      nextCells.push(cell);
    }
    applyGenericSelection(sortSelectionCells(nextCells, 'price'), 'price');
    setSelectionAnchor('price', cell);
  }

  function getOperationalPriceColumnCells(dayHeaderCell) {
    if (!(dayHeaderCell instanceof HTMLElement)) {
      return [];
    }
    const table = dayHeaderCell.closest('.calendar-table');
    const dayIndex = parseInt(dayHeaderCell.getAttribute('data-day-index') || '-1', 10);
    if (!table || !Number.isFinite(dayIndex) || dayIndex < 0) {
      return [];
    }
    return Array.prototype.slice.call(
      table.querySelectorAll('.js-rateplan-price[data-day-index="' + dayIndex + '"]')
    ).filter(function (cell) {
      const otaAccountId = parseInt(cell.getAttribute('data-ota-account-id') || '0', 10);
      return isSelectableCellForType(cell, 'price') && (!Number.isFinite(otaAccountId) || otaAccountId <= 0);
    });
  }

  function getOperationalPriceCellsForDateAcrossCalendars(dateValue) {
    const normalizedDate = String(dateValue || '').trim();
    if (!normalizedDate) {
      return [];
    }
    return Array.prototype.slice.call(
      document.querySelectorAll('.calendar-table .js-rateplan-price[data-date="' + normalizedDate + '"]')
    ).filter(function (cell) {
      const otaAccountId = parseInt(cell.getAttribute('data-ota-account-id') || '0', 10);
      return isSelectableCellForType(cell, 'price') && (!Number.isFinite(otaAccountId) || otaAccountId <= 0);
    });
  ...
  function selectOperationalPriceColumn(dayHeaderCell) {
    const cells = getOperationalPriceColumnCells(dayHeaderCell);
    return toggleOperationalPriceCells(cells, cells[0] || null);
  }

  function toggleOperationalPriceCells(cells, anchorCell) {
    if (!Array.isArray(cells) || !cells.length) {
      return false;
    }
    if ((selectionType && selectionType !== 'price') || reservationData.length) {
      clearSelection();
    }
    const anchorTable = anchorCell && typeof anchorCell.closest === 'function'
      ? anchorCell.closest('.calendar-table')
      : null;
    if (anchorTable) {
      attachActionBar(anchorTable);
    }
    if (selectionType === 'price' && selectionData.length) {
      var allSelected = cells.every(function (cell) {
        return selectionCells.indexOf(cell) !== -1;
      });
      if (allSelected) {
        var remainingCells = selectionCells.filter(function (cell) {
          return cells.indexOf(cell) === -1;
        });
        applyGenericSelection(sortSelectionCells(remainingCells, 'price'), 'price');
        setSelectionAnchor('price', remainingCells.length ? remainingCells[0] : null);
      } else {
        mergeSelectionCells(cells, 'price', anchorCell || cells[0]);
      }
    } else {
      applyGenericSelection(sortSelectionCells(cells, 'price'), 'price');
      setSelectionAnchor('price', anchorCell || cells[0]);
    }
    return true;
  }

  function selectPriceCells(cells, anchorCell) {
    if (!Array.isArray(cells) || !cells.length) {
      return false;
    }
    if ((selectionType && selectionType !== 'price') || reservationData.length) {
    ...
    const effectiveAnchor = anchorCell || cells[0] || null;
    const anchorTable = effectiveAnchor && typeof effectiveAnchor.closest === 'function'
      ? effectiveAnchor.closest('.calendar-table')
      : null;
    if (anchorTable) {
      attachActionBar(anchorTable);
    }
    applyGenericSelection(sortSelectionCells(cells, 'price'), 'price');
    setSelectionAnchor('price', effectiveAnchor);
    return true;
  }

  function getOtaSelectionCells(payload, scope) {
    if (!payload || !(Number(payload.otaAccountId) > 0)) {
      return [];
    }
    const otaAccountId = String(payload.otaAccountId);
    const todayIso = getTodayIsoDate();
    const endIso = addDaysToIsoDate(todayIso, 44);
    return Array.prototype.slice.call(
      document.querySelectorAll('.calendar-table .js-rateplan-price[data-ota-account-id="' + otaAccountId + '"]')
    ).filter(function (cell) {
      if (!isSelectableCellForType(cell, 'price')) {
        return false;
      }
      const cellDate = (cell.getAttribute('data-date') || '').trim();
      if (!cellDate || cellDate < todayIso || (endIso && cellDate > endIso)) {
        return false;
      }
      if (scope === 'row') {
        return (cell.getAttribute('data-row-key') || '') === (payload.rowKey || '');
      }
      if (scope === 'property') {
        return ((cell.getAttribute('data-property-code') || '').trim().toUpperCase()) === (payload.propertyCode || '');
      }
      return true;
    });
  }

  function selectOperationalPriceColumnsAcrossCalendars(dayHeaderCell) {
    if (!(dayHeaderCell instanceof HTMLElement)) {
      return false;
    }
    const dateValue = dayHeaderCell.getAttribute('data-date') || '';
    const cells = getOperationalPriceCellsForDateAcrossCalendars(dateValue);
    return toggleOperationalPriceCells(cells, cells[0] || null);
  }

  function toggleAvailabilitySelection(cell) {
    if (selectionType && selectionType !== 'availability') return;
    const availability = parseInt(cell.getAttribute('data-availability') || '0', 10);
    ...
    if (!nextCells.length) {
      applyGenericSelection([], 'availability');
      setSelectionAnchor('availability', cell);
      return;
    }

    const nextData = nextCells.map(function (item) { return getCellData(item); });
    const sameRow = nextData.every(function (item) {
      return item.rowKey === rowKey && item.propertyCode.toUpperCase() === propCode && item.availability > 0;
    });
    const indices = nextData
      .map(function (item) { return item.dayIndex; })
      .filter(function (index) { return Number.isFinite(index) && index >= 0; })
      .sort(function (a, b) { return a - b; });
    if (!sameRow || indices.length !== nextData.length || !isConsecutiveIndices(indices)) {
      return;
    }

    applyGenericSelection(sortSelectionCells(nextCells, 'availability'), 'availability');
    setSelectionAnchor('availability', cell);
  }
  ...
  function getSelectionSummary() {
    ...
    if (selectionType === 'price') {
      const fixedCount = selectionData.filter(function (item) { return !!item.hasFixedPrice; }).length;
      const presetValues = { min: [], mid: [], max: [] };
      selectionData.forEach(function (item) {
        if (item.presetMinCents > 0) presetValues.min.push(item.presetMinCents);
        if (item.presetMidCents > 0) presetValues.mid.push(item.presetMidCents);
        if (item.presetMaxCents > 0) presetValues.max.push(item.presetMaxCents);
      });
      function resolvePresetValue(values) {
        if (!values.length) {
          return null;
        }
        const first = values[0];
        const allSame = values.every(function (value) { return value === first; });
        return allSame ? first : null;
      }
      return {
        hasSelection: true,
        selectionCount: selectionData.length,
        hasFixedPrice: fixedCount > 0,
        fixedCount: fixedCount,
        presetMinCents: resolvePresetValue(presetValues.min),
        presetMidCents: resolvePresetValue(presetValues.mid),
        presetMaxCents: resolvePresetValue(presetValues.max)
      };
    }
    if (selectionType === 'availability') {
      const firstAvailability = selectionData[0];
      const rowKey = firstAvailability.rowKey || '';
      const indices = selectionData
        .map(function (item) { return item.dayIndex; })
        .filter(function (index) { return Number.isFinite(index) && index >= 0; })
        .sort(function (a, b) { return a - b; });
      const sameRow = selectionData.every(function (item) {
        return item.rowKey === rowKey && item.availability > 0;
      });
      return {
        hasSelection: true,
        sameRow: sameRow,
        contiguous: sameRow && indices.length === selectionData.length && isConsecutiveIndices(indices),
        nights: indices.length || selectionData.length,
        categoryCode: firstAvailability.categoryCode || '',
        propertyCode: firstAvailability.propertyCode || ''
      };
    }
  ...
  function openQuickReservationLightbox(context, onConfirm) {
    ...
    overlay.innerHTML = `
      <div class="calendar-quickres-window" role="dialog" aria-modal="true" aria-label="Reserva rapida">
        ...
        <form class="calendar-quickres-form" novalidate>
          ...
          <label>
            Telefono (opcional)
            <input type="tel" name="guest_phone" autocomplete="tel" inputmode="tel" placeholder="Telefono / WhatsApp">
          </label>
          <label>
            Precio total (MXN)
    ...
    const guestPhoneInput = overlay.querySelector('input[name="guest_phone"]');
    ...
    if (form) {
      form.addEventListener('submit', function (event) {
        ...
        const guestPhoneRaw = guestPhoneInput && guestPhoneInput.value ? guestPhoneInput.value.trim() : '';
  ...
  function submitQuickReservation(context, payload) {
    ...
    addInput('quick_guest_phone', data.guestPhone || '');
  ...
  function updateActionBar() {
    ...
    const isAvailability = selectionType === 'availability';
    const isPrice = selectionType === 'price';
    const isRes = selectionType === 'reservation' || selectionType === 'block';
    const onlyBlocks = (selectionType === 'block') || (isRes && summary.allBlocks);
    const selectedChunks = isCell && summary.hasSelection ? buildBlockChunks() : [];
    if (blockButton) blockButton.hidden = !(isCell && summary.hasSelection);
    if (createButton) {
      const showCellSelection = isCell && summary.hasSelection && summary.sameRoom && summary.contiguous;
      const showAvailabilitySelection = isAvailability && summary.hasSelection && summary.sameRow && summary.contiguous;
      const show = showCellSelection || showAvailabilitySelection;
      createButton.hidden = !show;
      if (show) {
        const nights = summary.nights;
        const label = showAvailabilitySelection && summary.categoryCode
          ? ('Crear reserva en ' + summary.categoryCode)
          : 'Crear reserva para fechas';
        createButton.textContent = label + ' (' + nights + ' noche' + (nights === 1 ? '' : 's') + ')';
      }
    }
    if (createBlockButton) {
      const blockCount = selectedChunks.length;
      const showBlockCreation = isCell && blockCount > 1;
      createBlockButton.hidden = !showBlockCreation;
      if (showBlockCreation) {
        createBlockButton.textContent = 'Crear reservaciones en bloque (' + blockCount + ' bloque' + (blockCount === 1 ? '' : 's') + ')';
      }
    }
    if (quickResButton) {
      quickResButton.hidden = !(isCell && summary.hasSelection && summary.sameRoom && summary.contiguous);
    }
    function setPriceActionLabel(button, baseLabel, cents) {
      if (!button) return;
      button.textContent = cents && Number.isFinite(cents)
        ? (baseLabel + ' (' + formatAmountInputFromCents(cents) + ')')
        : baseLabel;
    }
    if (priceSetButton) {
      priceSetButton.hidden = !isPrice;
      if (!priceSetButton.hidden) {
        priceSetButton.textContent = 'Fijar precio';
      }
    }
    if (pricePercentButton) {
      pricePercentButton.hidden = !isPrice;
      if (!pricePercentButton.hidden) {
        pricePercentButton.textContent = 'Fijar %';
      }
    }
    if (priceClearButton) {
      priceClearButton.hidden = !(isPrice && summary.hasFixedPrice);
      if (!priceClearButton.hidden) {
        priceClearButton.textContent = summary.fixedCount > 1 ? 'Quitar precios fijos' : 'Quitar precio fijo';
      }
    }
    if (priceMinButton) {
      priceMinButton.hidden = !isPrice;
      setPriceActionLabel(priceMinButton, 'Fijar en minimo', summary.presetMinCents);
    }
    if (priceMidButton) {
      priceMidButton.hidden = !isPrice;
      setPriceActionLabel(priceMidButton, 'Fijar en medio', summary.presetMidCents);
    }
    if (priceMaxButton) {
      priceMaxButton.hidden = !isPrice;
      setPriceActionLabel(priceMaxButton, 'Fijar precio maximo', summary.presetMaxCents);
    }
    ...
    const canDuplicateReservation = canMoveReservation;
    ...
    if (moveResButton) {
      ...
      moveResButton.textContent = moveModeActive && moveModeKind === 'move' ? 'Salir modo mover' : 'Mover';
    }
    if (duplicateResButton) {
      duplicateResButton.hidden = !canDuplicateReservation;
      duplicateResButton.textContent = moveModeActive && moveModeKind === 'duplicate' ? 'Salir modo duplicar' : 'Duplicar';
    }
    ...
    if (moveModeActive) {
      ...
      if (moveResButton) moveResButton.hidden = moveModeKind !== 'move';
      if (duplicateResButton) duplicateResButton.hidden = moveModeKind !== 'duplicate';
      ...
      if (createBlockButton) createBlockButton.hidden = true;
      if (quickResButton) quickResButton.hidden = true;
      if (priceSetButton) priceSetButton.hidden = true;
      if (priceClearButton) priceClearButton.hidden = true;
      if (priceMinButton) priceMinButton.hidden = true;
      if (priceMidButton) priceMidButton.hidden = true;
      if (priceMaxButton) priceMaxButton.hidden = true;
    }
    if (mobileSelectionClearButton) {
      const showMobileClear = isMobileCalendar() && summary.hasSelection && !isRes;
    ...
    if (isMobileCalendar()) {
      const iconMap = new Map([
        [blockButton, 'block'],
        [createButton, 'create'],
        [createBlockButton, 'create'],
        [quickResButton, 'quick'],
        [priceSetButton, 'price-set'],
        [priceClearButton, 'price-clear'],
        [priceMinButton, 'price-min'],
        [priceMidButton, 'price-mid'],
        [priceMaxButton, 'price-max'],
        [viewResButton, 'view'],
        [moveResButton, 'move'],
        [duplicateResButton, 'duplicate'],
        [payResButton, 'pay'],
      ...
      const anyVisible = [blockButton, createButton, createBlockButton, quickResButton, priceSetButton, priceClearButton, priceMinButton, priceMidButton, priceMaxButton, viewResButton, moveResButton, duplicateResButton, payResButton, addChargesButton, advanceStatusButton, noShowButton, cancelResButton, groupResButton, deleteBlocksButton]
        .some(function (btn) { return !!btn && !btn.hidden; });
      actionBar.classList.toggle('is-visible', anyVisible);
    } else {
      [blockButton, createButton, createBlockButton, quickResButton, priceSetButton, priceClearButton, priceMinButton, priceMidButton, priceMaxButton, viewResButton, moveResButton, duplicateResButton, payResButton, addChargesButton, advanceStatusButton, noShowButton, cancelResButton, groupResButton, deleteBlocksButton].forEach(function (btn) {
        if (!btn) return;
  ...
  document.addEventListener('click', function (event) {
    ...
    if (moveModeActive) {
      ...
      if (moveTargetCell && calendarTables.some(function (t) { return t.contains(moveTargetCell); }) && moveModeSource) {
        ...
        const isDuplicateMode = moveModeKind === 'duplicate';
        const confirmMessage = isDuplicateMode
          ? ('Duplicar reservacion #' + moveModeSource.reservationId + '?\n\nOrigen: ' + sourceLabel + '\nNuevo destino: ' + targetLabel)
          : ('Mover reservacion #' + moveModeSource.reservationId + '?\n\nOrigen: ' + sourceLabel + '\nDestino: ' + targetLabel);
        ...
        addInput('calendar_action', isDuplicateMode ? 'duplicate_reservation' : 'move_reservation');
    ...
    const dayHeaderCell = event.target.closest('.calendar-day-row th.day-header[data-day-index]');
    if (dayHeaderCell && calendarTables.some(function (t) { return t.contains(dayHeaderCell); })) {
      event.preventDefault();
      if (selectOperationalPriceColumn(dayHeaderCell)) {
        return;
      }
    }
    const otaToggleButton = event.target.closest('.calendar-ota-toggle[data-ota-toggle-for]');
    if (otaToggleButton && calendarTables.some(function (t) { return t.contains(otaToggleButton); })) {
      event.preventDefault();
      toggleCalendarOtaRows(otaToggleButton.getAttribute('data-ota-toggle-for') || '', otaToggleButton);
      return;
    }
    ...
    if (priceCell && calendarTables.some(function (t) { return t.contains(priceCell); })) {
      event.preventDefault();
      const propCode = (priceCell.getAttribute('data-property-code') || '').toUpperCase();
      if (selectionProperty && selectionProperty !== propCode) return;
      if (selectionType && selectionType !== 'price') return;
      const table = priceCell.closest('.calendar-table');
      if (selectionProperty && activeCalendarTable && table !== activeCalendarTable) return;
      if (!selectionProperty || selectionProperty === propCode) {
        attachActionBar(table);
      }
      if (event.shiftKey && selectShiftRangeForType(priceCell, 'price')) {
        return;
      }
      if ((event.ctrlKey || event.metaKey) && selectCtrlRectangleForType(priceCell, 'price')) {
        return;
      }
      togglePriceSelection(priceCell);
      return;
    }
    const availabilityCell = event.target.closest('.js-calendar-availability-select');
    if (availabilityCell && calendarTables.some(function (t) { return t.contains(availabilityCell); })) {
      event.preventDefault();
      const propCode = (availabilityCell.getAttribute('data-property-code') || '').toUpperCase();
      if (selectionProperty && selectionProperty !== propCode) return;
      if (selectionType && selectionType !== 'availability') return;
      const table = availabilityCell.closest('.calendar-table');
      if (selectionProperty && activeCalendarTable && table !== activeCalendarTable) return;
      if (!selectionProperty || selectionProperty === propCode) {
        attachActionBar(table);
      }
      if (event.shiftKey && selectShiftRangeForType(availabilityCell, 'availability')) {
        return;
      }
      if ((event.ctrlKey || event.metaKey) && selectCtrlRectangleForType(availabilityCell, 'availability')) {
        return;
      }
      toggleAvailabilitySelection(availabilityCell);
      return;
    }
    ...
    if (cell && calendarTables.some(function (t) { return t.contains(cell); })) {
      ...
      if (event.shiftKey && selectShiftRangeForType(cell, 'cell')) {
        return;
      }
      if ((event.ctrlKey || event.metaKey) && selectCtrlRectangleForType(cell, 'cell')) {
        return;
      }
  ...
  document.addEventListener('dblclick', function (event) {
    const dayHeaderCell = event.target.closest('.calendar-day-row th.day-header[data-date]');
    if (!dayHeaderCell || !calendarTables.some(function (t) { return t.contains(dayHeaderCell); })) {
      return;
    }
    event.preventDefault();
    event.stopPropagation();
    selectOperationalPriceColumnsAcrossCalendars(dayHeaderCell);
  });
  ...
  if (createButton) {
    createButton.addEventListener('click', function () {
      const summary = getSelectionSummary();
      const canCreateFromEmptyCells = selectionType === 'cell' && summary.sameRoom && summary.contiguous;
      const canCreateFromAvailability = selectionType === 'availability' && summary.sameRow && summary.contiguous;
      if (!summary.hasSelection || (!canCreateFromEmptyCells && !canCreateFromAvailability)) {
        return;
      }
      ...
      openReservationWizardPrefill({
        ...
        roomCode: canCreateFromEmptyCells ? (range.roomCode || '') : '',
  ...
  if (createBlockButton) {
    createBlockButton.addEventListener('click', function () {
      const chunks = buildBlockChunks();
      if (!chunks.length || chunks.length < 2) {
        return;
      }
      openReservationWizardBlockPrefill(chunks);
    });
  }

  function appendCalendarPostContextInputs(addInput) {
    const currentPropertyFilter = document.querySelector('select[name="property_code"]');
    if (currentPropertyFilter) {
      addInput('property_code', currentPropertyFilter.value || '');
    }
    const currentStart = document.querySelector('input[name="start_date"]');
    if (currentStart && currentStart.value) addInput('start_date', currentStart.value);
    const viewMode = document.querySelector('select[name="view_mode"]');
  ...
  function buildSelectedPricePayload(mode, customPriceRaw) {
    return selectionData
      .filter(function (item) { return item.selectionKind === 'price' && item.date && item.propertyCode; })
      .map(function (item) {
        let priceRaw = '';
        if (mode === 'custom') {
          priceRaw = customPriceRaw;
        } else if (mode === 'min' && item.presetMinCents > 0) {
          priceRaw = formatAmountInputFromCents(item.presetMinCents);
        } else if (mode === 'mid' && item.presetMidCents > 0) {
          priceRaw = formatAmountInputFromCents(item.presetMidCents);
        } else if (mode === 'max' && item.presetMaxCents > 0) {
          priceRaw = formatAmountInputFromCents(item.presetMaxCents);
        }
        if (priceRaw !== '') {
          priceRaw = clampPriceValueToSelectionMinimum([item], priceRaw);
        }
        return {
          property_code: item.propertyCode || '',
          category_id: item.categoryId > 0 ? item.categoryId : '',
          room_id: item.roomId > 0 ? item.roomId : '',
          ota_account_id: item.otaAccountId > 0 ? item.otaAccountId : '',
          date: item.date || '',
          price: priceRaw
        };
      })
      .filter(function (item) { return item.date && item.property_code && (mode === 'clear' || item.price); });
  }

  function clampPriceValueToSelectionMinimum(selectionItems, priceRaw) {
    const numericPrice = Number(String(priceRaw || '').replace(',', '.'));
    if (!Number.isFinite(numericPrice) || numericPrice <= 0) {
      return '';
    }
    const priceCents = Math.round(numericPrice * 100);
    let clampedCents = priceCents;
    (Array.isArray(selectionItems) ? selectionItems : []).forEach(function (item) {
      if (!item) {
        return;
      }
      const minCents = Number.isFinite(item.presetMinCents) ? item.presetMinCents : 0;
      if (minCents > 0 && clampedCents < minCents) {
        clampedCents = minCents;
      }
    });
    return formatAmountInputFromCents(clampedCents);
  }

  function toggleCalendarOtaRows(categoryId, toggleButton) {
    const normalizedCategoryId = String(categoryId || '').trim();
    if (!normalizedCategoryId) {
      return;
    }
    const otaRows = Array.prototype.slice.call(document.querySelectorAll('.calendar-rateplan-row-ota[data-ota-group="' + normalizedCategoryId + '"]'));
    if (!otaRows.length) {
      return;
    }
    const shouldCollapse = otaRows.some(function (row) {
    ...
    if (toggleButton) {
      ...
      toggleButton.setAttribute('title', shouldCollapse ? 'Expandir filas OTA' : 'Contraer filas OTA');
      var icon = toggleButton.querySelector('.calendar-ota-toggle-icon');
      if (icon) {
  ...
  function submitBulkPriceSelection(actionName, payload, notes) {
    if (!payload.length) {
      return;
    }
    const form = document.createElement('form');
    ...
    addInput('calendar_action', actionName);
    addInput('bulk_rateplan_override_payload', JSON.stringify(payload));
    addInput('override_notes', notes || '');
    appendCalendarPostContextInputs(addInput);
    document.body.appendChild(form);
    form.submit();
  }

  function collectUniformDisplayedPrice() {
    if (!selectionData.length) {
      return '';
    }
    const values = selectionData
      .map(function (item) { return (item.price || '').trim(); })
  ...
  function submitCustomSelectedPrice() {
    if (selectionType !== 'price' || !selectionData.length) {
      return;
    }
    const suggested = collectUniformDisplayedPrice();
    const priceValue = prompt('Nuevo precio para las celdas seleccionadas:', suggested);
    if (priceValue === null) {
      return;
    }
    ...
    const normalizedClamped = clampPriceValueToSelectionMinimum(selectionData, normalized);
    if (normalizedClamped === '') {
    ...
    submitBulkPriceSelection('bulk_rateplan_override_set', buildSelectedPricePayload('custom', normalizedClamped), notes);
  }

  function submitPresetSelectedPrice(mode) {
    if (selectionType !== 'price' || !selectionData.length) {
      return;
    }
    submitBulkPriceSelection('bulk_rateplan_override_set', buildSelectedPricePayload(mode, ''), '');
  }

  function buildSelectedPercentPricePayload(percentValue) {
    const normalizedPercent = Number(String(percentValue || '').replace(',', '.'));
    ...
    return selectionData
      .filter(function (item) { return item.selectionKind === 'price' && item.date && item.propertyCode; })
      .map(function (item) {
        const basePriceCents = Number.isFinite(item.priceCents) ? item.priceCents : 0;
        if (basePriceCents <= 0) {
          return null;
        }
        const adjustedPriceCents = Math.round(basePriceCents * (normalizedPercent / 100));
        if (!Number.isFinite(adjustedPriceCents) || adjustedPriceCents <= 0) {
          return null;
        }
        let priceRaw = formatAmountInputFromCents(adjustedPriceCents);
        priceRaw = clampPriceValueToSelectionMinimum([item], priceRaw);
        return {
          property_code: item.propertyCode || '',
          category_id: item.categoryId > 0 ? item.categoryId : '',
      ...
      .filter(function (item) { return !!item && item.date && item.property_code && item.price; });
  }

  function submitPercentSelectedPrice() {
    if (selectionType !== 'price' || !selectionData.length) {
      return;
    }
    const percentValue = prompt(
      'Porcentaje a aplicar sobre el precio actual.\nEjemplos: 100 = igual, 110 = +10%, 90 = -10%.',
      '100'
    );
    if (percentValue === null) {
      return;
    }
    const normalized = percentValue.trim();
    if (normalized === '') {
      return;
    }
    const payload = buildSelectedPercentPricePayload(normalized);
    if (!payload.length) {
      alert('No se pudieron generar overrides validos con ese porcentaje.');
      return;
    }
    const notes = prompt('Notas (opcional):', '');
    if (notes === null) {
    ...
    submitBulkPriceSelection('bulk_rateplan_override_set', payload, notes);
  }

  function clearSelectedPriceOverrides() {
    if (selectionType !== 'price' || !selectionData.length) {
      return;
    }
    const payload = buildSelectedPricePayload('clear', '');
    if (!payload.length) {
      return;
    }
    submitBulkPriceSelection('bulk_rateplan_override_clear', payload, '');
  }

  if (priceSetButton) {
    priceSetButton.addEventListener('click', submitCustomSelectedPrice);
  }
  if (pricePercentButton) {
    pricePercentButton.addEventListener('click', submitPercentSelectedPrice);
  }
  if (priceClearButton) {
    priceClearButton.addEventListener('click', clearSelectedPriceOverrides);
  }
  if (priceMinButton) {
    priceMinButton.addEventListener('click', function () {
      submitPresetSelectedPrice('min');
    });
  }
  if (priceMidButton) {
    priceMidButton.addEventListener('click', function () {
      submitPresetSelectedPrice('mid');
    });
  }
  if (priceMaxButton) {
    priceMaxButton.addEventListener('click', function () {
      submitPresetSelectedPrice('max');
    });
  }
  ...
  function openReservationWizardBlockPrefill(chunks) {
    const normalizedChunks = Array.isArray(chunks) ? chunks.filter(function (chunk) {
      return chunk && chunk.property_code && chunk.room_code && chunk.start_date && chunk.end_date;
    }) : [];
    ...
    addInput('wizard_step', '1');
    addInput('wizard_force_step', '1');
    addInput('wizard_property', String(firstChunk.property_code || '').toUpperCase());
    addInput('wizard_block_payload', JSON.stringify(normalizedChunks));
    appendWizardReturnContextInputs(addInput, returnContext);
  ...
  if (moveResButton) {
    moveResButton.addEventListener('click', function () {
      if (moveModeActive && moveModeKind === 'move') {
        exitMoveMode();
        return;
      }
      if (selectionType !== 'reservation' || reservationData.length !== 1) return;
      ...
      enterMoveMode(res, 'move');
    });
  }

  if (duplicateResButton) {
    duplicateResButton.addEventListener('click', function () {
      if (moveModeActive && moveModeKind === 'duplicate') {
        exitMoveMode();
        return;
      }
      if (selectionType !== 'reservation' || reservationData.length !== 1) return;
  ...
  if (calendarCopyEnabled) {
    document.addEventListener('contextmenu', function (event) {
      const headerCell = event.target.closest('tbody tr.calendar-category-availability-row th.category-cell');
      if (!headerCell) {
        closeCalendarCopyMenu();
        return;
      }
      const table = headerCell.closest('table.calendar-table');
      if (!table || (table.getAttribute('data-owner-copy-enabled') || '') !== '1') {
        closeCalendarCopyMenu();
        return;
      }
      const payload = buildCalendarCopyPayload(headerCell);
      if (!payload) {
        closeCalendarCopyMenu();
        return;
      }
      event.preventDefault();
      openCalendarCopyMenu(event.clientX, event.clientY, payload);
    });

    document.addEventListener('click', function (event) {
      if (calendarCopyMenu && !calendarCopyMenu.hidden && !calendarCopyMenu.contains(event.target)) {
        closeCalendarCopyMenu();
      }
    });
    ...
    window.addEventListener('resize', closeCalendarCopyMenu);
    window.addEventListener('blur', closeCalendarCopyMenu);
    document.addEventListener('scroll', closeCalendarCopyMenu, true);
  }

  function getCalendarDeveloperOtaOptions() {
    const source = (window.pmsOtaMap && typeof window.pmsOtaMap === 'object') ? window.pmsOtaMap : {};
    const seen = {};
    const options = [];
    Object.keys(source).forEach(function (propertyCode) {
      const rows = Array.isArray(source[propertyCode]) ? source[propertyCode] : [];
      rows.forEach(function (row) {
        const otaAccountId = parseInt(row && row.id_ota_account ? row.id_ota_account : '0', 10);
        if (!(otaAccountId > 0) || seen[otaAccountId]) {
          return;
        }
        const mode = String(row && row.price_adjustment_mode ? row.price_adjustment_mode : 'none').trim().toLowerCase();
        const value = Number(row && row.price_adjustment_value ? row.price_adjustment_value : 0);
        const secondaryPct = Number(row && row.secondary_price_adjustment_pct ? row.secondary_price_adjustment_pct : 0);
        if (mode === 'none' && Math.abs(value) < 0.0001 && Math.abs(secondaryPct) < 0.0001) {
          return;
        }
        seen[otaAccountId] = true;
        options.push({
          id: otaAccountId,
          propertyCode: String(row && row.property_code ? row.property_code : propertyCode).trim().toUpperCase(),
          name: String(row && row.ota_name ? row.ota_name : ('OTA #' + otaAccountId)).trim() || ('OTA #' + otaAccountId)
        });
      });
    });
  ...
  function closeCalendarDeveloperMenu() {
    if (!calendarDeveloperMenu) {
      return;
    }
    calendarDeveloperMenu.hidden = true;
  }

  function submitCalendarDeveloperClearOtaOverrides() {
    if (!calendarDeveloperSelect) {
      return;
    }
    const selectedValue = String(calendarDeveloperSelect.value || '').trim();
    if (!selectedValue) {
      alert('Selecciona una OTA.');
      return;
    }
    const option = calendarDeveloperSelect.selectedOptions && calendarDeveloperSelect.selectedOptions[0]
      ? calendarDeveloperSelect.selectedOptions[0]
      : null;
    const label = option ? option.textContent.trim() : ('OTA #' + selectedValue);
    if (!window.confirm('Eliminar todos los overrides activos de ' + label + '?')) {
      return;
    }
    const form = document.createElement('form');
    form.method = 'post';
    ...
    addInput('calendar_action', 'developer_clear_ota_overrides');
    addInput('developer_ota_account_id', selectedValue);
    appendCalendarPostContextInputs(addInput);
    document.body.appendChild(form);
    form.submit();
  }

  function ensureCalendarDeveloperMenu() {
    if (calendarDeveloperMenu) {
      return calendarDeveloperMenu;
    }
    const menu = document.createElement('div');
    menu.className = 'calendar-developer-menu';
    menu.hidden = true;
    menu.innerHTML = ''
      + '<div class="calendar-developer-menu-card">'
      + '  <div class="calendar-developer-menu-header">'
      + '    <strong>Developer</strong>'
      + '    <button type="button" class="calendar-developer-close" aria-label="Cerrar">&times;</button>'
      + '  </div>'
      + '  <div class="calendar-developer-menu-body">'
      + '    <label class="calendar-developer-field">'
      + '      <span>Limpiar overrides OTA</span>'
      + '      <select class="calendar-developer-ota-select"></select>'
      + '    </label>'
      + '    <button type="button" class="button-secondary calendar-developer-run">Eliminar todos los overrides</button>'
      + '  </div>'
      + '</div>';
    document.body.appendChild(menu);
    calendarDeveloperMenu = menu;
    calendarDeveloperSelect = menu.querySelector('.calendar-developer-ota-select');
    const closeBtn = menu.querySelector('.calendar-developer-close');
    const runBtn = menu.querySelector('.calendar-developer-run');
    if (closeBtn) {
      closeBtn.addEventListener('click', closeCalendarDeveloperMenu);
    }
    if (runBtn) {
      runBtn.addEventListener('click', submitCalendarDeveloperClearOtaOverrides);
    }
    menu.addEventListener('click', function (event) {
      if (event.target === menu) {
        closeCalendarDeveloperMenu();
      }
    });
    return calendarDeveloperMenu;
  }

  function openCalendarDeveloperMenu() {
    if (String(window.pmsCalendarDeveloperEnabled || '0') !== '1') {
      return;
    }
    const menu = ensureCalendarDeveloperMenu();
    if (!calendarDeveloperSelect) {
      return;
    }
    const options = getCalendarDeveloperOtaOptions();
    calendarDeveloperSelect.innerHTML = '';
    if (!options.length) {
      const emptyOption = document.createElement('option');
      emptyOption.value = '';
      emptyOption.textContent = 'No hay OTAs con precio activo';
      calendarDeveloperSelect.appendChild(emptyOption);
      calendarDeveloperSelect.disabled = true;
    } else {
      options.forEach(function (item) {
        const option = document.createElement('option');
        option.value = String(item.id);
        option.textContent = (item.propertyCode ? item.propertyCode + ' - ' : '') + item.name;
        calendarDeveloperSelect.appendChild(option);
      });
      calendarDeveloperSelect.disabled = false;
    }
    menu.hidden = false;
  }
  ...
  document.addEventListener('keydown', function (event) {
    ...
    if (event.ctrlKey && event.altKey && event.shiftKey && key === 'd') {
      event.preventDefault();
      if (calendarDeveloperMenu && !calendarDeveloperMenu.hidden) {
        closeCalendarDeveloperMenu();
      } else {
        openCalendarDeveloperMenu();
      }
      return;
    }
    if (key === 'escape') {
  ...
  document.addEventListener('contextmenu', function (event) {
    const otaName = event.target.closest('.js-calendar-ota-menu');
    if (!otaName) {
      closeCalendarOtaSelectionMenu();
      return;
    }
    const otaAccountId = parseInt(otaName.getAttribute('data-ota-account-id') || '0', 10);
    const rowKey = otaName.getAttribute('data-row-key') || '';
    const propertyCode = (otaName.getAttribute('data-property-code') || '').trim().toUpperCase();
    if (!(otaAccountId > 0) || !rowKey) {
      closeCalendarOtaSelectionMenu();
      return;
    }
    event.preventDefault();
    openCalendarOtaSelectionMenu(event.clientX, event.clientY, {
      otaAccountId: otaAccountId,
      otaName: otaName.getAttribute('data-ota-name') || '',
      rowKey: rowKey,
      propertyCode: propertyCode
    });
  });
  ...
  document.addEventListener('click', function (event) {
    if (calendarOtaSelectionMenu && !calendarOtaSelectionMenu.hidden && !calendarOtaSelectionMenu.contains(event.target)) {
      closeCalendarOtaSelectionMenu();
    }
  });
  ...
  window.addEventListener('resize', closeCalendarOtaSelectionMenu);
  window.addEventListener('blur', closeCalendarOtaSelectionMenu);
  document.addEventListener('scroll', closeCalendarOtaSelectionMenu, true);
