...
class RateplanPricingService
{
    ...
    /** @var array<string,?int> */
    private $operationalOverrideCache = array();
    /** @var array<string,?int> */
    private $otaOverrideCache = array();
    /** @var array<string,?array<string,mixed>> */
    private $legacyClampCache = array();
    /** @var array<int,int> */
    private $legacyOperationalRateplanCache = array();
    /** @var array<int,string> */
    private $pricingStrategyCache = array();
    /** @var array<int,array<string,mixed>> */
    private $otaAccountPricingCache = array();
    /** @var bool|null */
    private $pricingStrategyColumnReady = null;
    /** @var bool|null */
    private $otaPriceAdjustmentColumnsReady = null;
    /** @var bool|null */
    private $otaSecondaryAdjustmentColumnReady = null;
    /** @var bool|null */
    private $otaPriceOverrideTableReady = null;
    ...
    public function getNightlyBreakdown(
        ...
        $pricingStrategy = $this->pricingStrategyForProperty($idProperty);
        $legacyRateplanId = $pricingStrategy === 'use_bases' ? 0 : (int)$idRateplan;
        ...
        // Legacy dynamic rateplan engine: kept for explicit rateplan requests only.
        $mods = $legacyRateplanId > 0 ? $this->applicableModifiers($legacyRateplanId, $date, $idCategory, $idRoom) : array();
        if ($mods) {
            $groups = array();
            foreach ($mods as $m) {
        ...
        $legacyClamp = $this->legacyClamp($legacyRateplanId);
        if ($legacyClamp !== null) {
        ...
        $override = $this->operationalOverridePrice($idProperty, $date, $idCategory, $idRoom, $legacyRateplanId);
        $final = $override !== null ? max((int)$override, (int)$categoryMin) : (int)$price;
    ...
    public function buildOtaPriceBreakdown($idOtaAccount, $idProperty, $date, $idCategory = null, $idRoom = null, array $operationalRow = array())
    {
        $idOtaAccount = (int)$idOtaAccount;
        $idProperty = (int)$idProperty;
        $date = $this->dateYmd($date);
        $idCategory = $idCategory !== null ? (int)$idCategory : 0;
        $idRoom = $idRoom !== null ? (int)$idRoom : 0;
        ...
        if ($idOtaAccount <= 0) {
            throw new RuntimeException('OTA account is required.');
        }
        if ($idProperty <= 0 || $date === null) {
            throw new RuntimeException('Invalid OTA pricing arguments.');
        }
        ...
        if ($idCategory <= 0) {
            throw new RuntimeException('Category is required for OTA pricing.');
        }

        $operational = $operationalRow;
        if (!$operational || !isset($operational['final_price_cents'])) {
            $operational = $this->getNightlyBreakdown(0, $idProperty, $date, $idCategory, $idRoom > 0 ? $idRoom : null);
        }

        $operationalFinal = isset($operational['final_price_cents']) ? (int)$operational['final_price_cents'] : 0;
        $otaMinimum = $this->resolveOtaMinimumPriceCents($idOtaAccount, $idProperty, $idCategory, $idRoom);
        $config = $this->otaAccountPricingConfig($idOtaAccount);
        $mode = isset($config['price_adjustment_mode']) ? (string)$config['price_adjustment_mode'] : 'none';
        $value = isset($config['price_adjustment_value']) ? (float)$config['price_adjustment_value'] : 0.0;
        $secondaryPercent = isset($config['secondary_price_adjustment_pct']) ? (float)$config['secondary_price_adjustment_pct'] : 0.0;
        $override = $this->otaOverridePrice($idOtaAccount, $date, $idCategory, $idRoom);

        if ($override !== null) {
            $final = (int)$override;
        } elseif ($mode === 'percent') {
            $final = (int)round($operationalFinal * (1 + ($value / 100.0)));
        } elseif ($mode === 'fixed') {
            $final = $operationalFinal + (int)round($value * 100);
        } else {
            $final = $operationalFinal;
        }
        $final = $this->applyPercentAdjustmentToCents($final, $secondaryPercent);
        $final = $this->roundUpToPesoCents($final);
        if ($otaMinimum > 0 && $final < $otaMinimum) {
            $final = $otaMinimum;
        }

        return array(
            'calendar_date' => $date,
            'operational_price_cents' => $operationalFinal,
            'min_price_cents' => (int)$otaMinimum,
            'price_adjustment_mode' => $mode,
            'price_adjustment_value' => $value,
            'secondary_price_adjustment_pct' => $secondaryPercent,
            'ota_override_price_cents' => $override !== null ? (int)$override : null,
            'final_price_cents' => max(0, (int)$final)
        );
    }

    public function upsertOperationalOverride($propertyCode, $overrideDate, $priceCents, $idCategory = null, $idRoom = null, $notes = null, $isActive = 1)
    {
        $propertyCode = strtoupper(trim((string)$propertyCode));
        $overrideDate = $this->dateYmd($overrideDate);
        ...
        if ($overrideDate === null) {
            throw new RuntimeException('Override date is required.');
        }
        if ($priceCents <= 0) {
            throw new RuntimeException('Override price must be greater than zero.');
        }

        $property = $this->fetchOneRow(
            'SELECT id_property, COALESCE(currency, "MXN") AS currency
             FROM property
        ...
        if ($idRoom > 0) {
            $room = $this->fetchOneRow(
                'SELECT id_room, id_category
                 FROM room
                 WHERE id_room = ? AND id_property = ? AND deleted_at IS NULL
                 LIMIT 1',
                array($idRoom, $idProperty)
            );
            if (!$room) {
                throw new RuntimeException('Room not found for property.');
            }
            $idRoom = (int)$room['id_room'];
            $idCategory = isset($room['id_category']) ? (int)$room['id_category'] : 0;
        } elseif ($idCategory > 0) {
            $idCategory = $this->fetchOneInt(
                'SELECT id_category
                 FROM roomcategory
                 WHERE id_category = ? AND id_property = ? AND deleted_at IS NULL
                 LIMIT 1',
                array($idCategory, $idProperty),
                true
            );
            if ($idCategory === null || $idCategory <= 0) {
                throw new RuntimeException('Category not found for property.');
            }
        }

        $minimumPriceCents = $this->resolveOperationalMinimumPriceCents($idProperty, $idCategory, $idRoom);
        if ($minimumPriceCents > 0 && $priceCents < $minimumPriceCents) {
            $priceCents = $minimumPriceCents;
        }

        $existingOverrideId = $this->findOperationalOverrideId($idProperty, $overrideDate, $idCategory, $idRoom);
        $legacyRateplanId = $this->ensureLegacyOperationalRateplan($idProperty, isset($property['currency']) ? (string)$property['currency'] : 'MXN');
        ...
        if ($existingOverrideId > 0) {
            $stmt = $this->pdo->prepare(
                'UPDATE rateplan_override
                 SET id_rateplan = ?,
                     ...
                     is_active = ?,
                     updated_at = NOW()
                 WHERE id_rateplan_override = ?'
            );
            $stmt->execute(array(
                $legacyRateplanId,
                $idRoom > 0 ? null : ($idCategory > 0 ? $idCategory : null),
                $idRoom > 0 ? $idRoom : null,
                $overrideDate,
                $priceCents,
                $notes !== '' ? $notes : null,
                $isActive,
            ...
            $overrideId = (int)$this->pdo->lastInsertId();
        }

        $this->operationalOverrideCache = array();
        $this->overrideCache = array();
        ...
        $row = $this->fetchOneRow(
            'SELECT
             ...
             WHERE ro.id_rateplan_override = ?
             LIMIT 1',
            array($overrideId)
        );

        return $row ?: array(
            'id_rateplan_override' => $overrideId,
            'id_rateplan' => $legacyRateplanId,
            'id_category' => $idRoom > 0 ? null : ($idCategory > 0 ? $idCategory : null),
            'id_room' => $idRoom > 0 ? $idRoom : null,
    ...
    public function clearOperationalOverride($propertyCode, $overrideDate, $idCategory = null, $idRoom = null)
    {
        $propertyCode = strtoupper(trim((string)$propertyCode));
        $overrideDate = $this->dateYmd($overrideDate);
        $idCategory = $idCategory !== null ? (int)$idCategory : 0;
        ...
        if ($propertyCode === '' || $overrideDate === null) {
            throw new RuntimeException('Property code and override date are required.');
        }
        ...
        if (!$property || !isset($property['id_property'])) {
            throw new RuntimeException('Unknown property.');
        }
        $idProperty = (int)$property['id_property'];
        ...
        $stmt->execute(array($existingOverrideId));
    ...
    private function resolveOperationalMinimumPriceCents($idProperty, $idCategory = 0, $idRoom = 0)
    {
        $idProperty = (int)$idProperty;
        ...
        $categoryRow = $this->fetchOneRow(
            'SELECT default_base_price_cents, min_price_cents
             FROM roomcategory
             WHERE id_category = ? AND id_property = ? AND deleted_at IS NULL
             LIMIT 1',
            array($idCategory, $idProperty)
        ...
        $baseCents = isset($categoryRow['default_base_price_cents']) ? (int)$categoryRow['default_base_price_cents'] : 0;
        $minimumCents = isset($categoryRow['min_price_cents']) ? (int)$categoryRow['min_price_cents'] : 0;
        if ($minimumCents <= 0) {
            $minimumCents = $baseCents;
        }

        return max(0, $minimumCents);
    }

    private function resolveOtaMinimumPriceCents($idOtaAccount, $idProperty, $idCategory = 0, $idRoom = 0)
    {
        $minimumCents = $this->resolveOperationalMinimumPriceCents($idProperty, $idCategory, $idRoom);
        if ($minimumCents <= 0) {
            return 0;
        }

        $config = $this->otaAccountPricingConfig($idOtaAccount);
        return $this->applyOtaAdjustmentToBaseCents(
            $minimumCents,
            isset($config['price_adjustment_mode']) ? (string)$config['price_adjustment_mode'] : 'none',
            isset($config['price_adjustment_value']) ? (float)$config['price_adjustment_value'] : 0.0
        );
    }

    public function upsertOtaOverride($propertyCode, $idOtaAccount, $overrideDate, $priceCents, $idCategory = null, $idRoom = null, $notes = null, $isActive = 1)
    {
        if (!$this->otaPriceOverrideTableExists()) {
            throw new RuntimeException('La tabla de overrides OTA aun no existe. Aplica la migracion OTA.');
        }
        ...
        $priceCents = $this->roundUpToPesoCents($priceCents);
        ...
        $otaRow = $this->fetchOneRow(
            'SELECT id_ota_account, id_property
             FROM ota_account
             WHERE id_ota_account = ? AND deleted_at IS NULL
             LIMIT 1',
            array($idOtaAccount)
        );
        if (!$otaRow) {
            throw new RuntimeException('OTA account not found.');
        }
        ...
        $minimumPriceCents = $this->resolveOtaMinimumPriceCents($idOtaAccount, $idProperty, $idCategory, $idRoom);
        if ($minimumPriceCents > 0 && $priceCents < $minimumPriceCents) {
            $priceCents = $minimumPriceCents;
        }

        $existingOverrideId = $this->findOtaOverrideId($idOtaAccount, $overrideDate, $idCategory, $idRoom);
        if ($existingOverrideId > 0) {
            $stmt = $this->pdo->prepare(
                'UPDATE ota_price_override
                 SET id_category = ?,
                     id_room = ?,
                     override_date = ?,
                     price_cents = ?,
            ...
            $stmt = $this->pdo->prepare(
                'INSERT INTO ota_price_override (
                    id_ota_account,
                    id_category,
                    id_room,
                    override_date,
        ...
        $this->otaOverrideCache = array();

        return $this->fetchOneRow(
            'SELECT
                id_ota_price_override,
                id_ota_account,
                id_category,
                id_room,
                override_date,
    ...
    public function clearOtaOverride($propertyCode, $idOtaAccount, $overrideDate, $idCategory = null, $idRoom = null)
    {
        if (!$this->otaPriceOverrideTableExists()) {
            return false;
        }
        ...
        if ($propertyCode === '' || $idOtaAccount <= 0 || $overrideDate === null) {
            throw new RuntimeException('Property code, OTA account and override date are required.');
        }
    ...
    public function clearAllOtaOverridesForAccount($idOtaAccount)
    {
        if (!$this->otaPriceOverrideTableExists()) {
            return 0;
        }
        ...
        $stmt->execute(array($idOtaAccount));
        $this->otaOverrideCache = array();
    ...
    private function operationalOverridePrice($idProperty, $date, $idCategory, $idRoom, $preferredRateplanId = 0)
    {
        $idProperty = (int)$idProperty;
        $preferredRateplanId = (int)$preferredRateplanId;
        $key = $idProperty . '|' . $date . '|' . (int)$idCategory . '|' . (int)$idRoom . '|' . $preferredRateplanId;
        if (array_key_exists($key, $this->operationalOverrideCache)) {
            return $this->operationalOverrideCache[$key];
        }

        $sql = 'SELECT ro.price_cents
                FROM rateplan_override ro
                JOIN rateplan rp ON rp.id_rateplan = ro.id_rateplan
                WHERE rp.id_property = ?
                  AND rp.deleted_at IS NULL
                  AND ro.is_active = 1
                  AND ro.override_date = ?
                  AND (
                    (? > 0 AND ro.id_room = ?)
                    OR (? > 0 AND ro.id_room IS NULL AND ro.id_category = ?)
                    OR (ro.id_room IS NULL AND ro.id_category IS NULL)
                  )
                ORDER BY
                  CASE
                    WHEN ? > 0 AND ro.id_room = ? THEN 0
                    WHEN ? > 0 AND ro.id_room IS NULL AND ro.id_category = ? THEN 1
                    ELSE 2
                  END,
                  CASE WHEN ? > 0 AND ro.id_rateplan = ? THEN 0 ELSE 1 END,
                  ro.id_rateplan_override DESC
                LIMIT 1';
        $params = array(
            $idProperty,
            $date,
        ...
        $value = $this->fetchOneInt($sql, $params, true);
        $this->operationalOverrideCache[$key] = $value;
        return $value;
    }

    private function otaOverridePrice($idOtaAccount, $date, $idCategory, $idRoom)
    {
        if (!$this->otaPriceOverrideTableExists()) {
            return null;
        }
        ...
        $key = $idOtaAccount . '|' . $date . '|' . (int)$idCategory . '|' . (int)$idRoom;
        if (array_key_exists($key, $this->otaOverrideCache)) {
            return $this->otaOverrideCache[$key];
        }
        ...
        $sql = 'SELECT price_cents
                ...
                ORDER BY
                  CASE WHEN id_room IS NOT NULL THEN 0 WHEN id_category IS NOT NULL THEN 1 ELSE 2 END,
                  id_ota_price_override DESC
                LIMIT 1';
        $value = $this->fetchOneInt(
            $sql,
            array($idOtaAccount, $date, (int)$idRoom, (int)$idRoom, (int)$idCategory, (int)$idCategory),
            true
        );
        $this->otaOverrideCache[$key] = $value;
        return $value;
    }

    private function findOperationalOverrideId($idProperty, $overrideDate, $idCategory, $idRoom)
    {
        $sql = 'SELECT ro.id_rateplan_override
                FROM rateplan_override ro
                JOIN rateplan rp ON rp.id_rateplan = ro.id_rateplan
                WHERE rp.id_property = ?
                  AND rp.deleted_at IS NULL
                  AND ro.override_date = ?';
        $params = array((int)$idProperty, $overrideDate);
        if ((int)$idRoom > 0) {
            $sql .= ' AND ro.id_room = ?';
            $params[] = (int)$idRoom;
        } elseif ((int)$idCategory > 0) {
            $sql .= ' AND ro.id_room IS NULL AND ro.id_category = ?';
            $params[] = (int)$idCategory;
        } else {
            $sql .= ' AND ro.id_room IS NULL AND ro.id_category IS NULL';
        }
        $sql .= ' ORDER BY ro.id_rateplan_override DESC LIMIT 1';
        $value = $this->fetchOneInt($sql, $params, true);
        return $value !== null ? (int)$value : 0;
    }

    private function findOtaOverrideId($idOtaAccount, $overrideDate, $idCategory, $idRoom)
    {
        if (!$this->otaPriceOverrideTableExists()) {
            return 0;
        }

        $sql = 'SELECT id_ota_price_override
                FROM ota_price_override
                WHERE id_ota_account = ?
                  AND override_date = ?';
        $params = array((int)$idOtaAccount, $overrideDate);
        if ((int)$idRoom > 0) {
            $sql .= ' AND id_room = ?';
            $params[] = (int)$idRoom;
        } elseif ((int)$idCategory > 0) {
            $sql .= ' AND id_room IS NULL AND id_category = ?';
            $params[] = (int)$idCategory;
        } else {
            $sql .= ' AND id_room IS NULL AND id_category IS NULL';
        }
        $sql .= ' ORDER BY id_ota_price_override DESC LIMIT 1';
        $value = $this->fetchOneInt($sql, $params, true);
        return $value !== null ? (int)$value : 0;
    }

    private function ensureLegacyOperationalRateplan($idProperty, $currency = 'MXN')
    {
        $idProperty = (int)$idProperty;
        if (isset($this->legacyOperationalRateplanCache[$idProperty])) {
            return $this->legacyOperationalRateplanCache[$idProperty];
        }

        $code = 'LEGACY_OVERRIDES';
        $existingId = $this->fetchOneInt(
            'SELECT id_rateplan
             FROM rateplan
             WHERE id_property = ? AND code = ? AND deleted_at IS NULL
             LIMIT 1',
            array($idProperty, $code),
            true
        );
        if ($existingId !== null && (int)$existingId > 0) {
            $this->legacyOperationalRateplanCache[$idProperty] = (int)$existingId;
            return $this->legacyOperationalRateplanCache[$idProperty];
        }
        ...
        $stmt->execute(array(
            ...
            $code,
            'Legacy Overrides',
            'Legacy anchor for operational price overrides.',
            trim((string)$currency) !== '' ? (string)$currency : 'MXN'
        ));
        $this->legacyOperationalRateplanCache[$idProperty] = (int)$this->pdo->lastInsertId();
        return $this->legacyOperationalRateplanCache[$idProperty];
    }

    private function pricingStrategyForProperty($idProperty)
    {
        $idProperty = (int)$idProperty;
        if ($idProperty <= 0) {
            return 'use_bases';
        }
        if (isset($this->pricingStrategyCache[$idProperty])) {
            return $this->pricingStrategyCache[$idProperty];
        }
        if (!$this->pricingStrategyColumnExists()) {
            $this->pricingStrategyCache[$idProperty] = 'use_bases';
            return $this->pricingStrategyCache[$idProperty];
        }

        $row = $this->fetchOneRow(
            'SELECT
                COALESCE(NULLIF(TRIM(ps.pricing_strategy), \'\'), \'use_bases\') AS pricing_strategy
             FROM property p
             LEFT JOIN pms_settings ps
               ON ps.id_company = p.id_company
              AND (ps.id_property = p.id_property OR ps.id_property IS NULL)
             WHERE p.id_property = ?
               AND p.deleted_at IS NULL
             ORDER BY CASE WHEN ps.id_property = p.id_property THEN 0 ELSE 1 END, ps.id_setting DESC
             LIMIT 1',
            array($idProperty)
        );

        $strategy = $row && isset($row['pricing_strategy']) ? strtolower(trim((string)$row['pricing_strategy'])) : 'use_bases';
        if ($strategy === '') {
            $strategy = 'use_bases';
        }
        $this->pricingStrategyCache[$idProperty] = $strategy;
        return $this->pricingStrategyCache[$idProperty];
    }

    private function otaAccountPricingConfig($idOtaAccount)
    {
        $idOtaAccount = (int)$idOtaAccount;
        if ($idOtaAccount <= 0) {
            return array(
                ...
                'secondary_price_adjustment_pct' => 0.0
            );
        }
        if (isset($this->otaAccountPricingCache[$idOtaAccount])) {
            return $this->otaAccountPricingCache[$idOtaAccount];
        }
        if (!$this->otaPriceAdjustmentColumnsExist()) {
            $this->otaAccountPricingCache[$idOtaAccount] = array(
                'price_adjustment_mode' => 'none',
                'price_adjustment_value' => 0.0,
                'secondary_price_adjustment_pct' => 0.0
            );
        ...
        $secondarySelectSql = $this->otaSecondaryAdjustmentColumnExists()
            ? 'COALESCE(secondary_price_adjustment_pct, 0) AS secondary_price_adjustment_pct'
            : '0 AS secondary_price_adjustment_pct';

        $row = $this->fetchOneRow(
            'SELECT
                COALESCE(NULLIF(TRIM(price_adjustment_mode), \'\'), \'none\') AS price_adjustment_mode,
                COALESCE(price_adjustment_value, 0) AS price_adjustment_value,
                ' . $secondarySelectSql . '
             FROM ota_account
             WHERE id_ota_account = ?
               AND deleted_at IS NULL
        ...
        $mode = $row && isset($row['price_adjustment_mode']) ? strtolower(trim((string)$row['price_adjustment_mode'])) : 'none';
        if (!in_array($mode, array('none', 'percent', 'fixed'), true)) {
            $mode = 'none';
        }
        $value = $row && isset($row['price_adjustment_value']) ? (float)$row['price_adjustment_value'] : 0.0;
        $secondaryPercent = $row && isset($row['secondary_price_adjustment_pct']) ? (float)$row['secondary_price_adjustment_pct'] : 0.0;
        $this->otaAccountPricingCache[$idOtaAccount] = array(
            'price_adjustment_mode' => $mode,
            'price_adjustment_value' => $value,
            'secondary_price_adjustment_pct' => $secondaryPercent
    ...
    private function applyOtaAdjustmentToBaseCents($baseCents, $mode, $value)
    {
        $baseCents = max(0, (int)$baseCents);
        $mode = strtolower(trim((string)$mode));
        $value = (float)$value;

        if ($mode === 'percent') {
            $adjusted = (int)round($baseCents * (1 + ($value / 100.0)));
        } elseif ($mode === 'fixed') {
            $adjusted = $baseCents + (int)round($value * 100);
        } else {
            $adjusted = $baseCents;
        }

        return max(0, $this->roundUpToPesoCents($adjusted));
    }

    private function applyPercentAdjustmentToCents($baseCents, $percent)
    {
        $baseCents = max(0, (int)$baseCents);
        ...
        return max(0, (int)round($baseCents * (1 + ($percent / 100.0))));
    }

    private function pricingStrategyColumnExists()
    {
        if ($this->pricingStrategyColumnReady !== null) {
            return $this->pricingStrategyColumnReady;
        }
        try {
            $stmt = $this->pdo->prepare(
                'SELECT COUNT(*)
            ...
            $stmt->execute(array('pms_settings', 'pricing_strategy'));
            $this->pricingStrategyColumnReady = ((int)$stmt->fetchColumn()) > 0;
        } catch (Exception $e) {
            $this->pricingStrategyColumnReady = false;
        }
        return $this->pricingStrategyColumnReady;
    }

    private function otaPriceAdjustmentColumnsExist()
    {
        if ($this->otaPriceAdjustmentColumnsReady !== null) {
            return $this->otaPriceAdjustmentColumnsReady;
        }
        try {
            $stmt = $this->pdo->prepare(
                'SELECT COUNT(*)
            ...
            $this->otaPriceAdjustmentColumnsReady = ((int)$stmt->fetchColumn()) >= 2;
        } catch (Exception $e) {
            $this->otaPriceAdjustmentColumnsReady = false;
        }
        return $this->otaPriceAdjustmentColumnsReady;
    }

    private function otaSecondaryAdjustmentColumnExists()
    {
        if ($this->otaSecondaryAdjustmentColumnReady !== null) {
            return $this->otaSecondaryAdjustmentColumnReady;
        }
        try {
            $stmt = $this->pdo->prepare(
                'SELECT COUNT(*)
            ...
            $this->otaSecondaryAdjustmentColumnReady = ((int)$stmt->fetchColumn()) > 0;
        } catch (Exception $e) {
            $this->otaSecondaryAdjustmentColumnReady = false;
        }
        return $this->otaSecondaryAdjustmentColumnReady;
    }

    private function otaPriceOverrideTableExists()
    {
        if ($this->otaPriceOverrideTableReady !== null) {
            return $this->otaPriceOverrideTableReady;
        }
        try {
            $stmt = $this->pdo->prepare(
                'SELECT COUNT(*)
            ...
            $stmt->execute(array('ota_price_override'));
            $this->otaPriceOverrideTableReady = ((int)$stmt->fetchColumn()) > 0;
        } catch (Exception $e) {
            $this->otaPriceOverrideTableReady = false;
        }
        return $this->otaPriceOverrideTableReady;
    }
    ...
    private function roundUpToPesoCents($amountCents)
    {
        $amountCents = (int)$amountCents;
        if ($amountCents <= 0) {
            return 0;
        }
        return (int)(ceil($amountCents / 100) * 100);
    }
