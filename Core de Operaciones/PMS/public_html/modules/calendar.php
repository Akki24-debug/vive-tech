...
$calendarOwnerBulkCopyEnabled = pms_is_owner_user();
...
if (!function_exists('calendar_fetch_live_mxn_usd_rate')) {
    function calendar_fetch_live_mxn_usd_rate()
    {
        if (session_status() === PHP_SESSION_ACTIVE) {
            $cachedRate = isset($_SESSION['calendar_mxn_usd_rate']) ? (float)$_SESSION['calendar_mxn_usd_rate'] : 0.0;
            $cachedAt = isset($_SESSION['calendar_mxn_usd_rate_at']) ? (int)$_SESSION['calendar_mxn_usd_rate_at'] : 0;
            if ($cachedRate > 0 && $cachedAt > 0 && (time() - $cachedAt) < 900) {
                return $cachedRate;
            }
        }
        ...
        if (function_exists('curl_init')) {
            ...
            curl_setopt($ch, CURLOPT_USERAGENT, 'VLV-PMS/1.0');
            $raw = curl_exec($ch);
            curl_close($ch);
        }
        ...
        $rate = isset($json['rates']['USD']) ? (float)$json['rates']['USD'] : 0.0;
        if ($rate > 0 && session_status() === PHP_SESSION_ACTIVE) {
            $_SESSION['calendar_mxn_usd_rate'] = $rate;
            $_SESSION['calendar_mxn_usd_rate_at'] = time();
        }
        return $rate > 0 ? $rate : 0.0;
    }
}

if (!function_exists('calendar_settings_column_exists')) {
    function calendar_settings_column_exists(PDO $pdo, $tableName, $columnName)
    {
...
$calendarGlobalOtaRows = array();
if ($otaAccountsByProperty) {
    foreach ($otaAccountsByProperty as $otaRowsTmp) {
        if (!is_array($otaRowsTmp)) {
            continue;
        }
        foreach ($otaRowsTmp as $otaRowTmp) {
            $otaIdTmp = isset($otaRowTmp['id_ota_account']) ? (int)$otaRowTmp['id_ota_account'] : 0;
            if ($otaIdTmp <= 0 || isset($calendarGlobalOtaRows[$otaIdTmp])) {
                continue;
            }
            $calendarGlobalOtaRows[$otaIdTmp] = $otaRowTmp;
        }
    }
}
if ($calendarGlobalOtaRows) {
    $calendarGlobalOtaRows = array_values($calendarGlobalOtaRows);
}
if (function_exists('pms_ota_options_for_property')) {
    foreach ($properties as $propertyRow) {
        ...
        $calendarOtaOptionsByProperty[$propertyCodeTmp] = $calendarGlobalOtaRows;
...
if ($calendarGlobalOtaRows) {
    $calendarOtaOptionsByProperty['*'] = $calendarGlobalOtaRows;
}
if ($otaAccountsByProperty) {
...
if (!function_exists('calendar_normalize_inline_icon_html')) {
    function calendar_normalize_inline_icon_html($value)
    {
        $value = trim((string)$value);
        if ($value === '') {
            return '';
        }
        if (preg_match('/^(&#x?[0-9A-Fa-f]+;|&[A-Za-z][A-Za-z0-9]+;)$/', $value, $match)) {
...
if (!function_exists('calendar_build_primary_lodging_icon_map')) {
    function calendar_build_primary_lodging_icon_map(PDO $pdo, $companyId, array $reservationPropertyById)
    {
        $result = array();
        if ($companyId <= 0 || !$reservationPropertyById) {
            return $result;
        }
        if (!function_exists('settings_column_exists') && !function_exists('calendar_settings_column_exists')) {
            $columnExists = false;
        } else {
            $columnExists = function_exists('settings_column_exists')
                ? settings_column_exists($pdo, 'pms_settings_lodging_catalog', 'icon_html')
                : calendar_settings_column_exists($pdo, 'pms_settings_lodging_catalog', 'icon_html');
        }
        if (!$columnExists) {
            return $result;
        }
        $reservationIds = array_values(array_unique(array_filter(array_map('intval', array_keys($reservationPropertyById)), function ($id) {
            return $id > 0;
        })));
        if (!$reservationIds) {
            return $result;
        }
        $chunks = array_chunk($reservationIds, 400);
        foreach ($chunks as $chunk) {
            $placeholders = implode(',', array_fill(0, count($chunk), '?'));
            $params = array_merge(array((int)$companyId), $chunk);
            $stmt = $pdo->prepare(
                'SELECT
                    x.id_reservation,
                    COALESCE(pslc.icon_html, \'\') AS icon_html
                 FROM (
                    SELECT
                        f.id_reservation,
                    ...
                    JOIN property pr
                     ...
                     AND pr.deleted_at IS NULL
                    JOIN pms_settings_lodging_catalog allowed
                      ON allowed.id_company = pr.id_company
                     AND allowed.id_sale_item_catalog = li.id_line_item_catalog
                     AND allowed.deleted_at IS NULL
                     AND COALESCE(allowed.is_active, 1) = 1
                     AND (allowed.id_property IS NULL OR allowed.id_property = pr.id_property)
                    WHERE pr.id_company = ?
                      AND f.id_reservation IN (' . $placeholders . ')
                      AND li.deleted_at IS NULL
                      AND COALESCE(li.is_active, 1) = 1
                 ...
                 LEFT JOIN property prop_scope
                   ON UPPER(TRIM(COALESCE(prop_scope.code, \'\'))) = UPPER(TRIM(COALESCE(x.property_code, \'\')))
                  AND prop_scope.id_company = ?
                  AND prop_scope.deleted_at IS NULL
                 LEFT JOIN pms_settings_lodging_catalog pslc
                   ON pslc.id_company = ?
                  AND pslc.id_sale_item_catalog = x.id_line_item_catalog
                  AND pslc.deleted_at IS NULL
                  AND COALESCE(pslc.is_active, 1) = 1
                  AND (pslc.id_property = prop_scope.id_property OR pslc.id_property IS NULL)
                 WHERE x.rn = 1
                 ORDER BY x.id_reservation,
                          CASE WHEN pslc.id_property IS NULL THEN 1 ELSE 0 END,
                          pslc.id_setting_lodging'
            );
            $stmt->execute(array_merge($params, array((int)$companyId, (int)$companyId)));
            foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
                $reservationId = isset($row['id_reservation']) ? (int)$row['id_reservation'] : 0;
                if ($reservationId <= 0 || isset($result[$reservationId])) {
                    continue;
                }
                $iconHtml = calendar_normalize_inline_icon_html(isset($row['icon_html']) ? $row['icon_html'] : '');
                if ($iconHtml !== '') {
                    $result[$reservationId] = $iconHtml;
                }
            }
        }
        return $result;
...
if (!function_exists('calendar_payment_meta_upsert_safe')) {
    function calendar_payment_meta_upsert_safe($paymentId, $methodName, $reference, $status, $actorUserId, $serviceDate = null)
    {
        ...
        try {
            ...
            } catch (Exception $ignoreOldSignature) {
            }
        }
        ...
        } catch (Exception $ignoreFallback) {
        }
    }
}
...
if (!function_exists('calendar_find_open_folio_id_by_role')) {
    function calendar_find_open_folio_id_by_role($companyId, $reservationId, $role = 'lodging', $actorUserId = null)
    {
        ...
        try {
            ...
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
            if ($rows) {
                $explicitRoleMatches = array();
                $ambiguousLodgingMatches = array();
                foreach ($rows as $row) {
                    $folioId = isset($row['id_folio']) ? (int)$row['id_folio'] : 0;
                    if ($folioId <= 0) {
                        continue;
                    }
                    $folioName = isset($row['folio_name']) ? trim((string)$row['folio_name']) : '';
                    $folioNameLower = strtolower($folioName);
                    $hasServiceWord = ($folioNameLower !== '' && (strpos($folioNameLower, 'servicio') !== false || strpos($folioNameLower, 'service') !== false));
                    $hasLodgingHint = ($folioNameLower !== '' && (strpos($folioNameLower, 'hosped') !== false || in_array($folioNameLower, array('principal', 'main', 'hospedaje'), true)));
                    if ($role === 'services') {
                        if ($hasServiceWord && !$hasLodgingHint) {
                            $explicitRoleMatches[] = $row;
                        }
                        continue;
                    }
                    ...
                    if ($folioNameLower === '' || in_array($folioNameLower, array('principal', 'main'), true)) {
                        $ambiguousLodgingMatches[] = $row;
                    }
                }

                if ($explicitRoleMatches) {
                    $match = $explicitRoleMatches[0];
                    $folioId = isset($match['id_folio']) ? (int)$match['id_folio'] : 0;
                    if ($folioId > 0) {
                        if ($role === 'lodging') {
                            $stmtNormalizeLodging = $pdo->prepare(
                                'UPDATE folio
                                 SET folio_name = ?
                                 WHERE id_folio = ?
                                   AND (
                            ...
                            $stmtNormalizeLodging->execute(array('Hospedaje', $folioId));
                        }
                        return $folioId;
                    }
                }

                if ($role === 'lodging' && count($ambiguousLodgingMatches) === 1) {
                    $match = $ambiguousLodgingMatches[0];
                    $folioId = isset($match['id_folio']) ? (int)$match['id_folio'] : 0;
                    if ($folioId > 0) {
                        $stmtNormalizeAmbiguous = $pdo->prepare(
                            'UPDATE folio
                             SET folio_name = ?
                             WHERE id_folio = ?'
                        );
                        $stmtNormalizeAmbiguous->execute(array('Hospedaje', $folioId));
                        return $folioId;
                    }
                }
            }
...
if (!function_exists('calendar_resolve_target_lodging_catalog')) {
    function calendar_resolve_target_lodging_catalog($companyId, array $sourceReservation, $targetPropertyCode, $sourceReservationId)
    {
        $companyId = (int)$companyId;
        $sourceReservationId = (int)$sourceReservationId;
        $targetPropertyCode = strtoupper(trim((string)$targetPropertyCode));
        if ($companyId <= 0 || $sourceReservationId <= 0 || $targetPropertyCode === '') {
            return 0;
        }

        $pdo = pms_get_connection();
        $candidateIds = array();
        $sourceCheckIn = isset($sourceReservation['check_in_date']) ? trim((string)$sourceReservation['check_in_date']) : '';
        $sourceOtaAccountId = isset($sourceReservation['id_ota_account']) ? (int)$sourceReservation['id_ota_account'] : 0;
        $sourceReservationSourceId = isset($sourceReservation['id_reservation_source']) ? (int)$sourceReservation['id_reservation_source'] : 0;

        $sourceLodgingFolioId = 0;
        $stmtSourceLodgingFolio = $pdo->prepare(
            'SELECT f.id_folio
               FROM folio f
              WHERE f.id_reservation = ?
                AND f.deleted_at IS NULL
                ...
                AND LOWER(TRIM(COALESCE(f.status, \'open\'))) <> \'closed\'
                AND (
                  LOWER(TRIM(COALESCE(f.folio_name, \'\'))) = \'hospedaje\'
                  OR LOWER(TRIM(COALESCE(f.folio_name, \'\'))) IN (\'principal\', \'main\')
                  OR (
                    LOWER(TRIM(COALESCE(f.folio_name, \'\'))) LIKE \'%hosped%\'
                    AND LOWER(TRIM(COALESCE(f.folio_name, \'\'))) NOT LIKE \'%servicio%\'
                    AND LOWER(TRIM(COALESCE(f.folio_name, \'\'))) NOT LIKE \'%service%\'
                  )
                )
              ORDER BY CASE
                  WHEN LOWER(TRIM(COALESCE(f.folio_name, \'\'))) = \'hospedaje\' THEN 0
                  WHEN LOWER(TRIM(COALESCE(f.folio_name, \'\'))) IN (\'principal\', \'main\') THEN 1
                  ELSE 2
                END,
                f.id_folio ASC
              LIMIT 1'
        );
        $stmtSourceLodgingFolio->execute(array($sourceReservationId));
        $sourceLodgingFolioId = (int)$stmtSourceLodgingFolio->fetchColumn();
        if ($sourceLodgingFolioId > 0) {
            $stmtCatalogFromFolio = $pdo->prepare(
                'SELECT li.id_line_item_catalog
                  ...
                  WHERE li.id_folio = ?
                    ...
                    AND li.deleted_at IS NULL
                    AND COALESCE(li.is_active, 1) = 1
                  ORDER BY CASE WHEN li.service_date = ? THEN 0 ELSE 1 END,
                           li.created_at ASC,
                           li.id_line_item ASC
                  LIMIT 1'
            );
            $stmtCatalogFromFolio->execute(array($sourceLodgingFolioId, $sourceCheckIn));
            $catalogFromFolio = (int)$stmtCatalogFromFolio->fetchColumn();
            if ($catalogFromFolio > 0) {
                $candidateIds[] = $catalogFromFolio;
            }
        }

        if ($sourceOtaAccountId > 0) {
            $stmtCatalogFromOta = $pdo->prepare(
            ...
            $stmtCatalogFromOta->execute(array($companyId, $sourceOtaAccountId));
            $catalogFromOta = (int)$stmtCatalogFromOta->fetchColumn();
            if ($catalogFromOta > 0) {
                $candidateIds[] = $catalogFromOta;
            }
        }

        if ($sourceReservationSourceId > 0 && function_exists('pms_reservation_source_has_column') && pms_reservation_source_has_column($pdo, 'id_lodging_catalog')) {
            $stmtCatalogFromSource = $pdo->prepare(
            ...
            $stmtCatalogFromSource->execute(array($sourceReservationSourceId, $companyId));
            $catalogFromSource = (int)$stmtCatalogFromSource->fetchColumn();
            if ($catalogFromSource > 0) {
                $candidateIds[] = $catalogFromSource;
            }
        }
        ...
        foreach ($candidateIds as $candidateId) {
            ...
            $stmtAllowed->execute(array($companyId, $candidateId, $targetPropertyCode));
            $allowedCatalogId = (int)$stmtAllowed->fetchColumn();
            if ($allowedCatalogId > 0) {
                return $allowedCatalogId;
            }
        }
        ...
        $stmtDefault->execute(array($companyId, $targetPropertyCode));
        return (int)$stmtDefault->fetchColumn();
    }
}

if (!function_exists('calendar_duplicate_reservation_create')) {
    function calendar_duplicate_reservation_create($companyCode, $companyId, $sourceReservationId, $targetPropertyCode, $targetRoomCode, $targetCheckIn, $targetCheckOut, $actorUserId)
    {
        $companyCode = trim((string)$companyCode);
        $companyId = (int)$companyId;
        $sourceReservationId = (int)$sourceReservationId;
        $targetPropertyCode = strtoupper(trim((string)$targetPropertyCode));
        $targetRoomCode = strtoupper(trim((string)$targetRoomCode));
        $targetCheckIn = trim((string)$targetCheckIn);
        $targetCheckOut = trim((string)$targetCheckOut);
        $actorUserId = $actorUserId !== null ? (int)$actorUserId : null;
        if ($companyCode === '' || $companyId <= 0 || $sourceReservationId <= 0 || $targetPropertyCode === '' || $targetRoomCode === '') {
            throw new Exception('No se pudo preparar la duplicacion de la reservacion.');
        }
        ...
        $stmtSource->execute(array($sourceReservationId, $companyId));
        $sourceReservation = $stmtSource->fetch(PDO::FETCH_ASSOC);
        if (!$sourceReservation) {
            throw new Exception('Selecciona una reservacion valida.');
        }

        $sourceTotalCents = max(0, isset($sourceReservation['total_price_cents']) ? (int)$sourceReservation['total_price_cents'] : 0);
        $sourceGuestId = isset($sourceReservation['id_guest']) ? (int)$sourceReservation['id_guest'] : 0;
        $sourceAdults = isset($sourceReservation['adults']) ? (int)$sourceReservation['adults'] : 1;
        $sourceChildren = isset($sourceReservation['children']) ? (int)$sourceReservation['children'] : 0;
        $sourceCurrency = trim((string)(isset($sourceReservation['currency']) ? $sourceReservation['currency'] : 'MXN'));
        $sourceStatus = strtolower(trim((string)(isset($sourceReservation['status']) ? $sourceReservation['status'] : 'apartado')));
        $sourceNotesInternal = (string)(isset($sourceReservation['notes_internal']) ? $sourceReservation['notes_internal'] : '');
        $sourceNotesGuest = (string)(isset($sourceReservation['notes_guest']) ? $sourceReservation['notes_guest'] : '');
        $sourceOtaAccountId = isset($sourceReservation['id_ota_account']) ? (int)$sourceReservation['id_ota_account'] : 0;
        $sourceReservationSourceId = isset($sourceReservation['id_reservation_source']) ? (int)$sourceReservation['id_reservation_source'] : 0;
        $sourceName = trim((string)(isset($sourceReservation['source']) ? $sourceReservation['source'] : 'Directo'));

        $newReservationSets = pms_call_create_reservation_hold(
            $targetPropertyCode,
            $targetRoomCode,
            $targetCheckIn,
            $targetCheckOut,
        ...
        $newReservationRow = isset($newReservationSets[0][0]) ? $newReservationSets[0][0] : null;
        $newReservationId = ($newReservationRow && isset($newReservationRow['id_reservation'])) ? (int)$newReservationRow['id_reservation'] : 0;
        if ($newReservationId <= 0) {
            throw new Exception('No se pudo duplicar la reservacion.');
        }

        $shouldConfirm = ($sourceStatus !== 'apartado');
        if ($shouldConfirm) {
            if ($sourceGuestId <= 0) {
                throw new Exception('La reservacion origen no tiene huesped asignado para duplicar.');
            }
            $lodgingCatalogId = 0;
            if ($sourceTotalCents > 0) {
                $lodgingCatalogId = calendar_resolve_target_lodging_catalog($companyId, $sourceReservation, $targetPropertyCode, $sourceReservationId);
                if ($lodgingCatalogId <= 0) {
                    throw new Exception('No se encontro un concepto de hospedaje valido para duplicar la reservacion en la propiedad destino.');
                }
            }
            ...
            pms_call_procedure('sp_reservation_confirm_hold', array(
                ...
                $newReservationId,
                $sourceGuestId,
                $lodgingCatalogId > 0 ? $lodgingCatalogId : null,
                $sourceTotalCents,
                $sourceAdults,
                $sourceChildren,
                $actorUserId
            ...
            pms_call_procedure('sp_reservation_update', array(
                ...
                $newReservationId,
                null,
                $sourceOtaAccountId > 0 ? null : ($sourceReservationSourceId > 0 ? (string)$sourceReservationSourceId : $sourceName),
                $sourceOtaAccountId > 0 ? $sourceOtaAccountId : 0,
                null,
                null,
                null,
                $sourceAdults,
                $sourceChildren,
                $sourceNotesInternal !== '' ? $sourceNotesInternal : null,
                $sourceNotesGuest !== '' ? $sourceNotesGuest : null,
                $actorUserId
            ));
        } else {
            $stmtUpdateHold = $pdo->prepare(
                'UPDATE reservation
                    SET id_guest = ?,
                        adults = ?,
                        children = ?,
            ...
            $stmtUpdateHold->execute(array(
                $sourceGuestId > 0 ? $sourceGuestId : null,
                $sourceAdults,
                $sourceChildren,
                $sourceCurrency !== '' ? $sourceCurrency : 'MXN',
                $sourceName !== '' ? $sourceName : 'Directo',
                $sourceOtaAccountId > 0 ? $sourceOtaAccountId : null,
                $sourceOtaAccountId > 0 ? null : ($sourceReservationSourceId > 0 ? $sourceReservationSourceId : null),
                $sourceNotesInternal !== '' ? $sourceNotesInternal : null,
                $sourceNotesGuest !== '' ? $sourceNotesGuest : null,
                $newReservationId
            ));
        ...
        return array(
            ...
            'property_code' => $targetPropertyCode,
...
if (!function_exists('calendar_render_empty_cell')) {
    function calendar_render_empty_cell($dayInfo, $roomCode, $roomName, $propertyCode, $categoryCode = '', $isMonthDivider = false, $priceCents = null)
    {
        ...
        $priceCentsValue = $priceCents !== null ? (int)$priceCents : 0;
        echo '<td class="' . $className . '"'
            ...
            . ' data-price-cents="' . $priceCentsValue . '"'
            . '>'
            . '<span class="calendar-empty-selection-label" aria-hidden="true"></span>'
            . '</td>';
    }
}
...
$isCategoryAvailabilityMode = ($orderMode === 'category_availability');
...
if ($isCategoryAvailabilityMode && $baseDateObj < $todayDateObj) {
    $baseDateObj = clone $todayDateObj;
}
...
if ($calendarAction !== '') {
    ...
    } elseif ($calendarAction === 'duplicate_reservation') {
    ...
    } elseif (in_array($calendarAction, array('rateplan_override_quick', 'bulk_rateplan_override_set', 'bulk_rateplan_override_clear'), true)) {
        pms_require_permission('rateplans.edit');
    } elseif ($calendarAction === 'developer_clear_ota_overrides') {
        pms_require_permission('rateplans.edit');
    }
}
...
$duplicateReservationMessage = null;
$duplicateReservationError = null;
...
if ($calendarAction === 'prepare_new_block') {
    ...
    $quickGuestPhone = isset($_POST['quick_guest_phone']) ? trim((string)$_POST['quick_guest_phone']) : '';
            ...
            if ($reservationId <= 0) {
                ...
                if ($quickPriceCents !== null && $quickPriceCents > 0) {
                    ...
                    if ($quickMarkPaid && $quickPriceCents > 0) {
                        ...
                        if ($quickPaymentId > 0) {
                            calendar_payment_meta_upsert_safe(
                                $quickPaymentId,
                                $quickPaymentMethodName,
                                $quickPaymentReference,
                                'captured',
                                $actorUser,
                                $quickPaymentDate
                            );
                ...
                $quickGuestSets = pms_call_procedure('sp_guest_upsert', array(
                    ...
                    $quickGuestPhone !== '' ? $quickGuestPhone : null,
                    null,
                    0,
                    0,
                    null
    ...
    if ($reservationId <= 0) {
        $duplicateReservationError = 'Selecciona una reservacion valida.';
    } elseif ($targetPropertyCode === '' || $targetRoomCode === '' || $targetCheckIn === '' || $targetCheckOut === '') {
        $duplicateReservationError = 'Completa propiedad, habitacion y fechas destino para duplicar la reservacion.';
    } elseif (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $targetCheckIn) || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $targetCheckOut)) {
        $duplicateReservationError = 'Las fechas destino no son validas.';
    } elseif ($targetCheckOut <= $targetCheckIn) {
        $duplicateReservationError = 'La fecha de salida debe ser mayor a la fecha de entrada.';
    } else {
        try {
            $duplicateResult = calendar_duplicate_reservation_create(
                $companyCode,
                $companyId,
                $reservationId,
                $targetPropertyCode,
            ...
            $calendarFocusReservationId = isset($duplicateResult['id_reservation']) ? (int)$duplicateResult['id_reservation'] : 0;
            $calendarFocusPropertyCode = isset($duplicateResult['property_code']) ? strtoupper(trim((string)$duplicateResult['property_code'])) : $targetPropertyCode;
            $calendarFocusRoomCode = isset($duplicateResult['room_code']) ? strtoupper(trim((string)$duplicateResult['room_code'])) : $targetRoomCode;
            $calendarFocusCheckIn = isset($duplicateResult['check_in_date']) ? trim((string)$duplicateResult['check_in_date']) : $targetCheckIn;
            $duplicateReservationMessage = 'Reservacion duplicada correctamente.';
        } catch (Exception $e) {
            $duplicateReservationError = $e->getMessage();
        }
    }
    ...
    $overrideOtaAccountId = isset($_POST['override_ota_account_id']) ? (int)$_POST['override_ota_account_id'] : 0;
    ...
    if ($propertyCodeOverride === '' || $overrideDate === '' || $overridePriceRaw === '') {
        $rateplanErrors[] = 'Completa fecha y precio para el override.';
    } else {
        try {
            if ($overrideOtaAccountId > 0) {
                $rateplanPricingService->upsertOtaOverride(
                    $propertyCodeOverride,
                    $overrideOtaAccountId,
                    $overrideDate,
                    $overridePriceCents,
                    $categoryIdOverride,
                    $roomIdOverride,
                ...
                $rateplanMessages[] = 'Override OTA guardado (' . $overrideDate . ').';
            } else {
                $rateplanPricingService->upsertOperationalOverride(
                    $propertyCodeOverride,
                    $overrideDate,
                    $overridePriceCents,
                    $categoryIdOverride,
                ...
                $rateplanMessages[] = 'Override operativo guardado (' . $overrideDate . ').';
            }
        } catch (Exception $e) {
            $rateplanErrors[] = $e->getMessage();
        }
    }
} elseif (in_array($calendarAction, array('bulk_rateplan_override_set', 'bulk_rateplan_override_clear'), true)) {
    $rawPayload = isset($_POST['bulk_rateplan_override_payload']) ? (string)$_POST['bulk_rateplan_override_payload'] : '';
    $payload = json_decode($rawPayload, true);
    $overrideNotes = isset($_POST['override_notes']) ? trim((string)$_POST['override_notes']) : '';
    if (!is_array($payload) || !$payload) {
        $rateplanErrors[] = 'No se recibieron celdas validas para actualizar precios.';
    } else {
        $updatedCount = 0;
        foreach ($payload as $item) {
            ...
            $propertyCodeOverride = isset($item['property_code']) ? strtoupper(trim((string)$item['property_code'])) : '';
            $overrideOtaAccountId = isset($item['ota_account_id']) ? (int)$item['ota_account_id'] : 0;
            $categoryIdOverride = isset($item['category_id']) && $item['category_id'] !== '' ? (int)$item['category_id'] : null;
            $roomIdOverride = isset($item['room_id']) && $item['room_id'] !== '' ? (int)$item['room_id'] : null;
            $overrideDate = isset($item['date']) ? trim((string)$item['date']) : '';
            if ($propertyCodeOverride === '' || $overrideDate === '') {
                continue;
            }
            try {
                pms_require_property_access($propertyCodeOverride);
                if ($calendarAction === 'bulk_rateplan_override_clear') {
                    if ($overrideOtaAccountId > 0) {
                        if ($rateplanPricingService->clearOtaOverride($propertyCodeOverride, $overrideOtaAccountId, $overrideDate, $categoryIdOverride, $roomIdOverride)) {
                            $updatedCount++;
                        }
                    } else {
                        if ($rateplanPricingService->clearOperationalOverride($propertyCodeOverride, $overrideDate, $categoryIdOverride, $roomIdOverride)) {
                            $updatedCount++;
                        }
                    }
                    continue;
                ...
                $overridePriceRaw = isset($item['price']) ? trim((string)$item['price']) : '';
                if ($overridePriceRaw === '') {
                    continue;
                }
                $overridePriceCents = (int)round(((float)str_replace(',', '.', $overridePriceRaw)) * 100);
        ...
        if ($updatedCount > 0) {
            $rateplanMessages[] = $calendarAction === 'bulk_rateplan_override_clear'
                ? ('Se quitaron ' . $updatedCount . ' precio(s) fijos.')
                : ('Se actualizaron ' . $updatedCount . ' precio(s).');
        } elseif (!$rateplanErrors) {
            $rateplanErrors[] = $calendarAction === 'bulk_rateplan_override_clear'
                ? 'No habia precios fijos para quitar.'
                : 'No se actualizaron precios.';
        }
    }
} elseif ($calendarAction === 'developer_clear_ota_overrides') {
    if (!$calendarOwnerBulkCopyEnabled) {
        $rateplanErrors[] = 'Accion no autorizada.';
    } else {
        $developerOtaAccountId = isset($_POST['developer_ota_account_id']) ? (int)$_POST['developer_ota_account_id'] : 0;
        if ($developerOtaAccountId <= 0) {
            $rateplanErrors[] = 'Selecciona una OTA valida.';
        } else {
            try {
                $ownerOtaRow = pms_get_connection()->prepare(
                    'SELECT
                        oa.id_ota_account,
                        p.code AS property_code,
                        COALESCE(NULLIF(TRIM(oa.ota_name), \'\'), CONCAT(\'OTA #\', oa.id_ota_account)) AS ota_name
                     FROM ota_account oa
                     JOIN property p
                       ON p.id_property = oa.id_property
                     WHERE oa.id_ota_account = ?
                       AND p.id_company = ?
                       AND oa.deleted_at IS NULL
                       AND p.deleted_at IS NULL
                     LIMIT 1'
                );
                $ownerOtaRow->execute(array($developerOtaAccountId, $companyId));
                $ownerOtaData = $ownerOtaRow->fetch(PDO::FETCH_ASSOC);
                if (!$ownerOtaData) {
                    throw new RuntimeException('OTA no encontrada.');
                }
                pms_require_property_access(isset($ownerOtaData['property_code']) ? (string)$ownerOtaData['property_code'] : '', true);
                $removedCount = $rateplanPricingService->clearAllOtaOverridesForAccount($developerOtaAccountId);
                if ($removedCount > 0) {
                    $rateplanMessages[] = 'Se eliminaron ' . $removedCount . ' overrides de ' . (isset($ownerOtaData['ota_name']) ? (string)$ownerOtaData['ota_name'] : ('OTA #' . $developerOtaAccountId)) . '.';
                } else {
                    $rateplanErrors[] = 'No habia overrides activos para esa OTA.';
                }
            } catch (Exception $e) {
                $rateplanErrors[] = $e->getMessage();
            }
    ...
    if ($orderMode === 'category_availability') {
        ...
        foreach ($categoryRows as $cat) {
            ...
            $categoryIndex[$cid] = array(
                ...
                'default_base_price_cents' => isset($cat['default_base_price_cents']) ? (int)$cat['default_base_price_cents'] : 0,
                'min_price_cents' => isset($cat['min_price_cents']) ? (int)$cat['min_price_cents'] : 0,
        ...
        if (!$categoryIndex) {
            foreach ($bundle['rooms'] as $room) {
                ...
                $categoryIndex[$cid] = array(
                    ...
                    'order_index' => 0,
                    'id_rateplan' => 0,
                    'default_base_price_cents' => isset($room['default_base_price_cents']) ? (int)$room['default_base_price_cents'] : 0,
                    'min_price_cents' => isset($room['min_price_cents']) ? (int)$room['min_price_cents'] : 0,
        ...
        $propertyOtaRows = $calendarGlobalOtaRows;
        ...
        foreach ($categoryIndex as $cid => $cat) {
            ...
            $priceCalendarRows = array();
            $otaPriceRows = array();
            if (!empty($cat['category_code'])) {
                try {
                    $priceCalendarRows = $rateplanPricingService->getCalendarPricesByCodes(
                        $propCode,
                        '',
                        $cat['category_code'],
                        null,
            ...
            foreach ($propertyOtaRows as $otaRow) {
                $otaId = isset($otaRow['id_ota_account']) ? (int)$otaRow['id_ota_account'] : 0;
                $otaMode = isset($otaRow['price_adjustment_mode']) ? strtolower(trim((string)$otaRow['price_adjustment_mode'])) : 'none';
                $otaValue = isset($otaRow['price_adjustment_value']) ? (float)$otaRow['price_adjustment_value'] : 0.0;
                $otaSecondaryPct = isset($otaRow['secondary_price_adjustment_pct']) ? (float)$otaRow['secondary_price_adjustment_pct'] : 0.0;
                if ($otaId <= 0) {
                    continue;
                }
                $hasPrimaryAdjustment = !($otaMode === 'none' || abs($otaValue) < 0.0001);
                $hasSecondaryAdjustment = abs($otaSecondaryPct) >= 0.0001;
                if (!$hasPrimaryAdjustment && !$hasSecondaryAdjustment) {
                    continue;
                }
                $otaCalendarRows = array();
                foreach ($priceCalendarRows as $priceRow) {
                    $dateForOta = isset($priceRow['calendar_date']) ? (string)$priceRow['calendar_date'] : '';
                    if ($dateForOta === '') {
                        continue;
                    }
                    try {
                        $otaPriceRow = $rateplanPricingService->buildOtaPriceBreakdown(
                            $otaId,
                            (int)$propertiesByCode[$propCode]['id_property'],
                            $dateForOta,
                            $cid,
                            null,
                            $priceRow
                        );
                        $otaPriceRow['day_index'] = isset($priceRow['day_index']) ? (int)$priceRow['day_index'] : count($otaCalendarRows);
                        $otaCalendarRows[] = $otaPriceRow;
                    } catch (Exception $e) {
                    }
                }
                $otaPriceRows[] = array(
                    'id_ota_account' => $otaId,
                    'ota_name' => isset($otaRow['ota_name']) ? (string)$otaRow['ota_name'] : ('OTA #' . $otaId),
                    'platform' => isset($otaRow['platform']) ? (string)$otaRow['platform'] : 'other',
                    'color_hex' => isset($otaRow['color_hex']) ? (string)$otaRow['color_hex'] : '',
                    'price_adjustment_mode' => $otaMode,
                    'price_adjustment_value' => $otaValue,
                    'secondary_price_adjustment_pct' => $otaSecondaryPct,
                    'calendar_rows' => $otaCalendarRows
                );
            }
            ...
            $categoryView[] = array(
                ...
                'price_calendar_rows' => $priceCalendarRows,
                'ota_price_rows' => $otaPriceRows
            );
        }
...
$calendarPrimaryLodgingIconByReservation = array();
if ($calendarReservationPropertyById) {
    try {
        $pdoCalendarLodgingIcon = pms_get_connection();
        $calendarPrimaryLodgingIconByReservation = calendar_build_primary_lodging_icon_map(
            $pdoCalendarLodgingIcon,
            $companyId,
            $calendarReservationPropertyById
        );
    } catch (Exception $e) {
...
$calendarMxnUsdRate = 0.0;
if ($calendarOwnerBulkCopyEnabled) {
    try {
        $calendarMxnUsdRate = calendar_fetch_live_mxn_usd_rate();
    } catch (Exception $e) {
        $calendarMxnUsdRate = 0.0;
    }
}
...
<section class="calendar-card">
  <form method="post" class="calendar-controls" id="calendar-controls-form">
    ...
    <div class="calendar-action-bar">
      ...
      <div class="calendar-filters">
        ...
        <label>
          Fecha base
          <input type="date" name="start_date" value="<?php echo htmlspecialchars($baseDate, ENT_QUOTES, 'UTF-8'); ?>"<?php echo $isCategoryAvailabilityMode ? ' min="' . htmlspecialchars(date('Y-m-d'), ENT_QUOTES, 'UTF-8') . '"' : ''; ?> required onchange="this.form.submit();">
        </label>
        <label>
          Orden
  ...
  <?php endif; ?>
  <?php if ($duplicateReservationMessage): ?>
    <p class="success"><?php echo htmlspecialchars($duplicateReservationMessage, ENT_QUOTES, 'UTF-8'); ?></p>
  <?php endif; ?>
  <?php if ($advanceError): ?>
  ...
  <?php endif; ?>
  <?php if ($duplicateReservationError): ?>
    <p class="error"><?php echo htmlspecialchars($duplicateReservationError, ENT_QUOTES, 'UTF-8'); ?></p>
  <?php endif; ?>
    ...
    <?php foreach ($calendarBundles as $bundleIndex => $bundle): ?>
      ...
      <?php endif; ?>
        ...
        <?php if ($categoryView && $calendarDays): ?>
          ...
          ?>
          <div class="calendar-scroll" data-scroll-date="<?php echo htmlspecialchars($baseDate, ENT_QUOTES, 'UTF-8'); ?>" data-today="<?php echo htmlspecialchars(date('Y-m-d'), ENT_QUOTES, 'UTF-8'); ?>" data-lock-scroll-date="1">
          <table class="calendar-table calendar-category-availability" data-property-code="<?php echo htmlspecialchars((string)$propertyCodeRow, ENT_QUOTES, 'UTF-8'); ?>" data-owner-copy-enabled="<?php echo $calendarOwnerBulkCopyEnabled ? '1' : '0'; ?>" data-usd-rate="<?php echo htmlspecialchars(number_format((float)$calendarMxnUsdRate, 6, '.', ''), ENT_QUOTES, 'UTF-8'); ?>" style="--calendar-day-count: <?php echo (int)$dayCount; ?>; --calendar-room-width: <?php echo (int)$categoryRoomWidth; ?>px; --calendar-day-width: <?php echo (int)$categoryDayWidth; ?>px; width: <?php echo (int)$categoryTableWidth; ?>px; min-width: <?php echo (int)$categoryTableWidth; ?>px;">
              <thead>
                <?php
                  $monthNames = array(
                      1 => 'Enero',
                ...
                <tr class="calendar-day-row">
                  <?php foreach ($calendarDays as $day):
                    ...
                    if ($isToday && !$isCategoryAvailabilityMode) {
                        $dayClass[] = 'is-today';
                    }
                    if ($weekday >= 6) {
                        $dayClass[] = 'is-weekend';
              ...
              <tbody>
                <?php foreach ($categoryView as $cat):
                  ...
                  $catCode = isset($cat['category_code']) ? (string)$cat['category_code'] : '';
                  $catRoomCodes = isset($roomCodesByCategory[$catId]) && is_array($roomCodesByCategory[$catId]) ? array_values(array_unique($roomCodesByCategory[$catId])) : array();
                  $catRoomCodesText = $catRoomCodes ? implode(', ', $catRoomCodes) : 'Sin habitaciones';
                  $catMiddlePriceCents = isset($cat['default_base_price_cents']) ? (int)$cat['default_base_price_cents'] : 0;
                  $catMinPriceCents = isset($cat['min_price_cents']) ? (int)$cat['min_price_cents'] : 0;
                  if ($catMinPriceCents <= 0) {
                      $catMinPriceCents = $catMiddlePriceCents;
                  }
                  if ($catMiddlePriceCents <= 0) {
                      $catMiddlePriceCents = $catMinPriceCents;
                  }
                  $catMaxPriceCents = $catMiddlePriceCents > 0 ? ($catMiddlePriceCents * 2) : 0;
                  $availabilityRowKey = 'availability:' . $catId;
                ...
                ?>
                  <tr class="calendar-category-availability-row">
                    <th class="room-cell category-cell" title="<?php echo htmlspecialchars('Categoria: ' . $catName . ' - Habitaciones: ' . $catRoomCodesText, ENT_QUOTES, 'UTF-8'); ?>">
                      <strong><?php echo htmlspecialchars($catName, ENT_QUOTES, 'UTF-8'); ?></strong>
                      <span class="room-subtitle category-room-list"><?php echo htmlspecialchars($catRoomCodesText, ENT_QUOTES, 'UTF-8'); ?></span>
                    ...
                    <?php for ($i = 0; $i < $dayCount; $i++):
                      ...
                      if (!$isCategoryAvailabilityMode && $dayInfo && isset($dayInfo['is_today']) && (int)$dayInfo['is_today'] === 1) {
                          $cellClass .= ' is-today';
                      }
                      if ($availableCount <= 0) {
                    ...
                    ?>
                      <td class="<?php echo $cellClass; ?> js-calendar-availability-select"
                          data-selection-kind="availability"
                          data-row-key="<?php echo htmlspecialchars($availabilityRowKey, ENT_QUOTES, 'UTF-8'); ?>"
                          ...
                          data-category-code="<?php echo htmlspecialchars($catCode, ENT_QUOTES, 'UTF-8'); ?>"
                          data-availability="<?php echo (int)$availableCount; ?>">
                  ...
                  <?php
                    $priceByIndex = array();
                    foreach (isset($cat['price_calendar_rows']) ? $cat['price_calendar_rows'] : array() as $row) {
                  ...
                  <tr class="calendar-rateplan-row">
                    <th class="room-cell rateplan-cell">
                      <div class="rateplan-cell-main">
                        <div class="rateplan-name">Precio operativo</div>
                        <?php if (!empty($cat['ota_price_rows'])): ?>
                          <button
                            type="button"
                            class="calendar-ota-toggle"
                            data-ota-toggle-for="<?php echo (int)$catId; ?>"
                            aria-expanded="true"
                            title="Contraer filas OTA">
                            <span class="calendar-ota-toggle-label">OTAs</span>
                            <span class="calendar-ota-toggle-icon" aria-hidden="true">-</span>
                          </button>
                        <?php endif; ?>
                      </div>
                    </th>
                    <?php for ($i = 0; $i < $dayCount; $i++):
                      ...
                      $priceDisplay = number_format($finalCents / 100, 0, '.', '');
                      $cellClasses = 'calendar-cell rateplan-price js-rateplan-price';
                      $isAtMinimum = $catMinPriceCents > 0 && $finalCents > 0 && $finalCents <= $catMinPriceCents;
                      if (!$isCategoryAvailabilityMode && $dayInfo && isset($dayInfo['is_today']) && (int)$dayInfo['is_today'] === 1) {
                    ...
                    ?>
                      <td class="<?php echo $cellClasses; ?>"
                          data-selection-kind="price"
                          data-row-key="<?php echo htmlspecialchars('price:operational:' . $catId, ENT_QUOTES, 'UTF-8'); ?>"
                          data-day-index="<?php echo (int)$i; ?>"
                          data-property-code="<?php echo htmlspecialchars((string)$propertyCodeRow, ENT_QUOTES, 'UTF-8'); ?>"
                          data-category-id="<?php echo (int)$catId; ?>"
                          data-category-code="<?php echo htmlspecialchars($catCode, ENT_QUOTES, 'UTF-8'); ?>"
                          data-ota-account-id="0"
                          data-ota-name=""
                          data-date="<?php echo htmlspecialchars($dateKey, ENT_QUOTES, 'UTF-8'); ?>"
                          data-price="<?php echo htmlspecialchars($priceDisplay, ENT_QUOTES, 'UTF-8'); ?>"
                          data-price-cents="<?php echo (int)$finalCents; ?>"
                          data-min-price-cents="<?php echo (int)$catMinPriceCents; ?>"
                          data-mid-price-cents="<?php echo (int)$catMiddlePriceCents; ?>"
                          data-max-price-cents="<?php echo (int)$catMaxPriceCents; ?>"
                          data-is-at-minimum="<?php echo $isAtMinimum ? '1' : '0'; ?>"
                          data-has-fixed-price="<?php echo $overrideCents !== null ? '1' : '0'; ?>">
                        <span class="rateplan-price-amount"><?php echo htmlspecialchars($priceDisplay, ENT_QUOTES, 'UTF-8'); ?></span>
                      </td>
                    <?php endfor; ?>
                  </tr>
                  <?php foreach ((isset($cat['ota_price_rows']) && is_array($cat['ota_price_rows'])) ? $cat['ota_price_rows'] : array() as $otaPriceRow): ?>
                    <?php
                      $otaPriceByIndex = array();
                      $otaColorHex = calendar_normalize_hex_color(isset($otaPriceRow['color_hex']) ? (string)$otaPriceRow['color_hex'] : '');
                      $otaRowStyle = '';
                      if ($otaColorHex !== '') {
                          $otaRowStyle = '--calendar-ota-accent:' . $otaColorHex
                              . '; --calendar-ota-accent-soft:' . calendar_hex_to_rgba($otaColorHex, 0.12)
                              . '; --calendar-ota-accent-hover:' . calendar_hex_to_rgba($otaColorHex, 0.18)
                              . '; --calendar-ota-accent-border:' . calendar_hex_to_rgba($otaColorHex, 0.42);
                      }
                      foreach ((isset($otaPriceRow['calendar_rows']) && is_array($otaPriceRow['calendar_rows'])) ? $otaPriceRow['calendar_rows'] : array() as $row) {
                          $idx = isset($row['day_index']) ? (int)$row['day_index'] : null;
                          if ($idx !== null) {
                              $otaPriceByIndex[$idx] = $row;
                          }
                      }
                    ?>
                    <tr class="calendar-rateplan-row calendar-rateplan-row-ota<?php echo $otaColorHex !== '' ? ' has-ota-accent' : ''; ?>" data-ota-group="<?php echo (int)$catId; ?>"<?php echo $otaRowStyle !== '' ? ' style="' . htmlspecialchars($otaRowStyle, ENT_QUOTES, 'UTF-8') . '"' : ''; ?>>
                      <th class="room-cell rateplan-cell">
                        <div
                          class="rateplan-name js-calendar-ota-menu"
                          data-property-code="<?php echo htmlspecialchars((string)$propertyCodeRow, ENT_QUOTES, 'UTF-8'); ?>"
                          data-category-id="<?php echo (int)$catId; ?>"
                          data-ota-account-id="<?php echo isset($otaPriceRow['id_ota_account']) ? (int)$otaPriceRow['id_ota_account'] : 0; ?>"
                          data-ota-name="<?php echo htmlspecialchars((string)$otaPriceRow['ota_name'], ENT_QUOTES, 'UTF-8'); ?>"
                          data-row-key="<?php echo htmlspecialchars('price:ota:' . (isset($otaPriceRow['id_ota_account']) ? (int)$otaPriceRow['id_ota_account'] : 0) . ':' . $catId, ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars((string)$otaPriceRow['ota_name'], ENT_QUOTES, 'UTF-8'); ?></div>
                      </th>
                      <?php for ($i = 0; $i < $dayCount; $i++):
                        $dayInfo = isset($calendarDayMap[$i]) ? $calendarDayMap[$i] : null;
                        $dateKey = $dayInfo && isset($dayInfo['date_key']) ? (string)$dayInfo['date_key'] : '';
                        $row = isset($otaPriceByIndex[$i]) ? $otaPriceByIndex[$i] : null;
                        $finalCents = $row && isset($row['final_price_cents']) ? (int)$row['final_price_cents'] : 0;
                        $otaMinPriceCents = $row && isset($row['min_price_cents']) ? (int)$row['min_price_cents'] : 0;
                        $overrideCents = $row && array_key_exists('ota_override_price_cents', $row) ? $row['ota_override_price_cents'] : null;
                        $priceDisplay = number_format($finalCents / 100, 0, '.', '');
                        $cellClasses = 'calendar-cell rateplan-price js-rateplan-price';
                        $isAtMinimum = $otaMinPriceCents > 0 && $finalCents > 0 && $finalCents <= $otaMinPriceCents;
                        if (!$isCategoryAvailabilityMode && $dayInfo && isset($dayInfo['is_today']) && (int)$dayInfo['is_today'] === 1) {
                            $cellClasses .= ' is-today';
                        }
                        if ($overrideCents !== null) {
                      ...
                      ?>
                        <td class="<?php echo $cellClasses; ?>"
                            data-selection-kind="price"
                            data-row-key="<?php echo htmlspecialchars('price:ota:' . (isset($otaPriceRow['id_ota_account']) ? (int)$otaPriceRow['id_ota_account'] : 0) . ':' . $catId, ENT_QUOTES, 'UTF-8'); ?>"
                            data-day-index="<?php echo (int)$i; ?>"
                            data-property-code="<?php echo htmlspecialchars((string)$propertyCodeRow, ENT_QUOTES, 'UTF-8'); ?>"
                            data-category-id="<?php echo (int)$catId; ?>"
                            data-category-code="<?php echo htmlspecialchars($catCode, ENT_QUOTES, 'UTF-8'); ?>"
                            ...
                            data-price="<?php echo htmlspecialchars($priceDisplay, ENT_QUOTES, 'UTF-8'); ?>"
                            data-price-cents="<?php echo (int)$finalCents; ?>"
                            data-min-price-cents="<?php echo (int)$otaMinPriceCents; ?>"
                            data-mid-price-cents="<?php echo (int)$catMiddlePriceCents; ?>"
                            data-max-price-cents="<?php echo (int)$catMaxPriceCents; ?>"
                            data-is-at-minimum="<?php echo $isAtMinimum ? '1' : '0'; ?>"
                            data-has-fixed-price="<?php echo $overrideCents !== null ? '1' : '0'; ?>">
      ...
      <?php elseif ($rooms && $calendarDays): ?>
        ...
        <div class="calendar-scroll" data-scroll-date="<?php echo htmlspecialchars($baseDate, ENT_QUOTES, 'UTF-8'); ?>" data-today="<?php echo htmlspecialchars(date('Y-m-d'), ENT_QUOTES, 'UTF-8'); ?>">
          <table class="calendar-table" data-property-code="<?php echo htmlspecialchars((string)$propertyCodeRow, ENT_QUOTES, 'UTF-8'); ?>" data-owner-copy-enabled="<?php echo $calendarOwnerBulkCopyEnabled ? '1' : '0'; ?>" data-usd-rate="<?php echo htmlspecialchars(number_format((float)$calendarMxnUsdRate, 6, '.', ''), ENT_QUOTES, 'UTF-8'); ?>" style="--calendar-day-count: <?php echo (int)$dayCount; ?>; --calendar-room-width: <?php echo (int)$roomLabelWidth; ?>px; --calendar-day-width: <?php echo (int)$roomDayWidth; ?>px; width: <?php echo (int)$roomTableWidth; ?>px; min-width: <?php echo (int)$roomTableWidth; ?>px;">
            <thead>
              <?php
                $monthNames = array(
                    1 => 'Enero',
            ...
            <tbody>
              <?php
                $roomPriceRowsByRoomId = array();
                foreach ($rooms as $roomPriceMeta) {
                    $roomPriceId = isset($roomPriceMeta['id_room']) ? (int)$roomPriceMeta['id_room'] : 0;
                    $roomPriceCode = isset($roomPriceMeta['room_code']) ? trim((string)$roomPriceMeta['room_code']) : '';
                    if ($roomPriceId <= 0 || $roomPriceCode === '') {
                        continue;
                    }
                    try {
                        $roomPriceCalendarRows = $rateplanPricingService->getCalendarPricesByCodes(
                            $propertyCodeRow,
                            '',
                            '',
                            $roomPriceCode,
                        ...
                        $roomPriceRowsByRoomId[$roomPriceId] = array();
                        foreach ($roomPriceCalendarRows as $roomPriceRow) {
                            $roomPriceDayIndex = isset($roomPriceRow['day_index']) ? (int)$roomPriceRow['day_index'] : -1;
                            if ($roomPriceDayIndex < 0) {
                                continue;
                            }
                            $roomPriceRowsByRoomId[$roomPriceId][$roomPriceDayIndex] = isset($roomPriceRow['final_price_cents'])
                                ? (int)$roomPriceRow['final_price_cents']
                                : 0;
                        }
                    } catch (Exception $e) {
                        $roomPriceRowsByRoomId[$roomPriceId] = array();
              ...
              <?php endif;
                ...
                $roomPriceRows = isset($roomPriceRowsByRoomId[$roomId]) ? $roomPriceRowsByRoomId[$roomId] : array();
                $cursor = 0;
              ?>
                <tr>
                  <th class="room-cell">
                  ...
                  <?php foreach ($rowReservations as $reservation):
                    ...
                    if ($startOffset > $cursor) {
                        for ($gap = $cursor; $gap < $startOffset && $gap < $dayCount; $gap++) {
                            ...
                            $gapPriceCents = isset($roomPriceRows[$gap]) ? (int)$roomPriceRows[$gap] : 0;
                            calendar_render_empty_cell($gapDay, $roomCode, $roomName, $propertyCodeRow, $categoryCode, $isMonthDivider, $gapPriceCents);
                        }
                        $cursor = $startOffset;
                    }
                    ...
                    $primaryLodgingIconHtml = (!$isBlock && $reservationId > 0 && isset($calendarPrimaryLodgingIconByReservation[$reservationId]))
                        ? (string)$calendarPrimaryLodgingIconByReservation[$reservationId]
                        : '';
                              ...
                              <?php if (!$isBlock && !$isQuickApartado && ($statusNormalized === 'apartado' || $hasFinancialCapsules)): ?>
                                ...
                                <?php else: ?>
                                  <span class="reservation-total-row">
                                    <?php if ($primaryLodgingIconHtml !== ''): ?>
                                      <span class="reservation-lodging-icon" title="Hospedaje principal"><?php echo $primaryLodgingIconHtml; ?></span>
                                    <?php endif; ?>
                                    <span
                  ...
                  for ($rest = $cursor; $rest < $dayCount; $rest++) {
                      ...
                      $restPriceCents = isset($roomPriceRows[$rest]) ? (int)$roomPriceRows[$rest] : 0;
                      calendar_render_empty_cell($restDay, $roomCode, $roomName, $propertyCodeRow, $categoryCode, $isMonthDivider, $restPriceCents);
                  }
                  ?>
                </tr>
              <?php endforeach; ?>
...
<script>
  ...
  window.pmsCalendarDeveloperEnabled = <?php echo $calendarOwnerBulkCopyEnabled ? '1' : '0'; ?>;
