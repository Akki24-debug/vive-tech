...
function rw_collect_line_item_descendants($rootLineItemId)
{
    ...
    try {
        ...
        $stmt->execute(array($rootLineItemId));
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        foreach ($rows as $row) {
            $childId = isset($row['child_id']) ? (int)$row['child_id'] : 0;
            if ($childId > 0 && $childId !== $rootLineItemId) {
...
function rw_mark_line_item_hierarchy_deleted(array $lineItemIds, $actorUserId)
{
...
function rw_delete_line_item_tree($reservationId, $rootLineItemId, $actorUserId)
{
    ...
    try {
        ...
        rw_mark_line_item_hierarchy_deleted($deleteOrder, $actorUserId);
