...
$lodgingFolioCatalogsByProperty = array();
try {
    $pdo = pms_get_connection();
    $lodgingFolioSelectedByScope = array();
    $stmtLodgingFolioTable = $pdo->prepare(
        'SELECT COUNT(*)
           FROM information_schema.TABLES
          WHERE TABLE_SCHEMA = DATABASE()
            AND TABLE_NAME = \'pms_settings_lodging_folio_catalog\''
    );
    $stmtLodgingFolioTable->execute();
    if (((int)$stmtLodgingFolioTable->fetchColumn()) > 0) {
        $stmtLodgingFolioSelection = $pdo->prepare(
            'SELECT DISTINCT
                s.id_sale_item_catalog,
                UPPER(TRIM(COALESCE(pr.code, \'\'))) AS property_code
             FROM pms_settings_lodging_folio_catalog s
             LEFT JOIN property pr
               ON pr.id_property = s.id_property
              AND pr.deleted_at IS NULL
             WHERE s.id_company = ?
        ...
        $stmtLodgingFolioSelection->execute(array($companyId));
        foreach ($stmtLodgingFolioSelection->fetchAll(PDO::FETCH_ASSOC) as $selectedRow) {
            $selectedCatalogId = isset($selectedRow['id_sale_item_catalog']) ? (int)$selectedRow['id_sale_item_catalog'] : 0;
            if ($selectedCatalogId <= 0 || !isset($serviceCatalogsById[$selectedCatalogId])) {
                continue;
            }
            $selectedScope = isset($selectedRow['property_code']) ? strtoupper(trim((string)$selectedRow['property_code'])) : '';
            if ($selectedScope === '') {
            ...
            if (!isset($lodgingFolioSelectedByScope[$selectedScope])) {
                $lodgingFolioSelectedByScope[$selectedScope] = array();
            }
            $lodgingFolioSelectedByScope[$selectedScope][$selectedCatalogId] = true;
        }
    }

    $appendCatalogToLodgingFolioScope = function (array &$scopeMap, array $catalogRow) {
        $catalogId = isset($catalogRow['id_service_catalog']) ? (int)$catalogRow['id_service_catalog'] : 0;
        if ($catalogId <= 0) {
            return;
        }
    ...
    foreach ($lodgingFolioSelectedByScope as $selectedIdsByScope) {
        foreach (array_keys($selectedIdsByScope) as $selectedId) {
            $selectedId = (int)$selectedId;
            if ($selectedId <= 0 || !isset($serviceCatalogsById[$selectedId])) {
                continue;
            }
            $appendCatalogToLodgingFolioScope($lodgingFolioCatalogsByProperty, $serviceCatalogsById[$selectedId]);
        }
    }

    foreach ($lodgingFolioCatalogsByProperty as &$scopeRows) {
        usort($scopeRows, function ($a, $b) {
            $aCategory = isset($a['category_name']) ? (string)$a['category_name'] : '';
            $bCategory = isset($b['category_name']) ? (string)$b['category_name'] : '';
            $cmpCategory = strcasecmp($aCategory, $bCategory);
...
if (!function_exists('reservations_lodging_folio_catalogs_for_property')) {
    function reservations_lodging_folio_catalogs_for_property(array $map, $propertyCode)
    {
        $prop = strtoupper((string)$propertyCode);
        $out = array();
        if (isset($map['*'])) {
...
if (!function_exists('reservations_payment_meta_upsert_safe')) {
    function reservations_payment_meta_upsert_safe($paymentId, $methodName, $reference, $status, $actorUserId, $serviceDate = null)
    {
    ...
    function reservations_create_payment_line_item($folioId, $reservationId, $paymentCatalogId, $serviceDate, $amountCents, $status, $actorUserId, $methodName = '', $reference = '')
    {
        ...
        if ($paymentCatalogId > 0) {
            ...
            $stmt->execute(array(
                $actorUserId,
                $folioId,
                trim((string)$methodName) !== '' ? trim((string)$methodName) : (
                    trim((string)$reference) !== '' ? trim((string)$reference) : 'Pago manual'
                ),
                $serviceDate,
                $amountCents,
                $amountCents,
...
if (!function_exists('reservations_mark_line_item_hierarchy_deleted')) {
    function reservations_mark_line_item_hierarchy_deleted(array $lineItemIds, $actorUserId)
    {
        $lineItemIds = array_values(array_unique(array_filter(array_map('intval', $lineItemIds), function ($id) {
            return $id > 0;
        })));
        if (!$lineItemIds) {
            return;
            ...
            $stmt = $pdo->prepare(
                'UPDATE line_item_hierarchy
                  ...
                  WHERE deleted_at IS NULL
                    AND (
                      id_line_item_parent IN (' . $placeholders . ')
                      OR id_line_item_child IN (' . $placeholders . ')
                    )'
            );
            $stmt->execute(array_merge(array((int)$actorUserId > 0 ? (int)$actorUserId : null), $lineItemIds, $lineItemIds));
        } catch (Exception $e) {
        }
    }
}

if (!function_exists('reservations_delete_sale_item_tree')) {
    function reservations_delete_sale_item_tree($reservationId, $rootSaleItemId, $actorUserId, $fallbackFolioId = 0, $skipPercentRecalcOnDelete = 1)
    {
        $reservationId = (int)$reservationId;
        $rootSaleItemId = (int)$rootSaleItemId;
        $fallbackFolioId = (int)$fallbackFolioId;
        $skipPercentRecalcOnDelete = (int)$skipPercentRecalcOnDelete === 1 ? 1 : 0;
        if ($reservationId <= 0 || $rootSaleItemId <= 0) {
            return 0;
        }

        $deleteOrder = array();
        $descendants = reservations_collect_sale_item_descendants($rootSaleItemId);
        if ($descendants) {
            $deleteOrder = array_reverse(array_values(array_unique(array_map('intval', $descendants))));
        }
        $deleteOrder[] = $rootSaleItemId;
        ...
        try {
            ...
            reservations_mark_line_item_hierarchy_deleted($deleteOrder, $actorUserId);
            ...
            foreach ($deleteOrder as $deleteSaleId) {
                ...
                pms_call_procedure('sp_sale_item_upsert', array(
                    'delete',
                    $deleteSaleId,
                    $snapshotFolioId > 0 ? $snapshotFolioId : $fallbackFolioId,
                    $reservationId,
    ...
    if ($propertyCode === '' || $roomCode === '' || $checkIn === '' || $checkOut === '') {
      ...
      if ($reservationId <= 0) {
          ...
          try {
              ...
              $guestLastNameValue = trim((string)$guestLast);
              $guestMaidenNameValue = trim((string)$guestMaiden);
              $guestFullNameValue = trim($guestNames . ' ' . $guestLastNameValue . ' ' . $guestMaidenNameValue);
              if ($guestId <= 0) {
                  $guestEmailValue = $guestEmail !== '' ? $guestEmail : null;
                  $stmt = $pdo->prepare('INSERT INTO guest (email, phone, names, last_name, maiden_name, full_name, language, is_active, created_at, updated_at) VALUES (?,?,?,?,?,?,?,1,NOW(),NOW())');
                  $stmt->execute(array(
                      $guestEmailValue,
                      $guestPhone !== '' ? $guestPhone : null,
                      $guestNames,
                      $guestLastNameValue !== '' ? $guestLastNameValue : null,
                      $guestMaidenNameValue !== '' ? $guestMaidenNameValue : null,
                      $guestFullNameValue !== '' ? $guestFullNameValue : null,
                      'es'
                  ));
                  ...
                  $stmt = $pdo->prepare(
                      'UPDATE guest
                          SET names = CASE WHEN ? IS NULL OR CHAR_LENGTH(?) = 0 THEN names ELSE ? END,
                              last_name = CASE WHEN ? IS NULL OR CHAR_LENGTH(?) = 0 THEN last_name ELSE ? END,
                              maiden_name = CASE WHEN ? IS NULL OR CHAR_LENGTH(?) = 0 THEN maiden_name ELSE ? END,
                              full_name = NULLIF(TRIM(CONCAT_WS(\' \',
                                  CASE WHEN ? IS NULL OR CHAR_LENGTH(?) = 0 THEN names ELSE ? END,
                                  CASE WHEN ? IS NULL OR CHAR_LENGTH(?) = 0 THEN last_name ELSE ? END,
                                  CASE WHEN ? IS NULL OR CHAR_LENGTH(?) = 0 THEN maiden_name ELSE ? END
                              )), \'\'),
                              phone = CASE WHEN ? IS NULL OR CHAR_LENGTH(?) = 0 THEN phone ELSE ? END,
                              updated_at = NOW()
                        WHERE id_guest = ?'
                  );
                  $stmt->execute(array(
    ...
    if ($reservationId > 0) {
        try {
            ...
            if (!$visibleTaxIds) {
                ...
                $deletedCount = 0;
                foreach (array_keys($visibleTaxIds) as $rootTaxId) {
                    $deletedCount += reservations_delete_sale_item_tree(
                        $reservationId,
                        (int)$rootTaxId,
                        $actorUserId,
                        $targetFolioId,
...
if ($action === 'create_sale_item' || $action === 'update_sale_item' || $action === 'delete_sale_item') {
    ...
    if ($reservationId > 0 && ($folioId > 0 || $saleId > 0)) {
        try {
            ...
            $targetFolioRole = 'services';
            if ($folioId > 0) {
                try {
                    $dbRole = pms_get_connection();
                    $stmtFolioRole = $dbRole->prepare(
                        'SELECT folio_name
                           FROM folio
                          WHERE id_folio = ?
                            AND id_reservation = ?
                    ...
                    $stmtFolioRole->execute(array($folioId, $reservationId));
                    $targetFolioName = (string)$stmtFolioRole->fetchColumn();
                    if ($targetFolioName !== '') {
                        $targetFolioRole = reservations_folio_role_by_name($targetFolioName);
                    }
                } catch (Exception $e) {
                    $targetFolioRole = 'services';
                }
            }
            $allowedServiceOptions = $targetFolioRole === 'lodging'
                ? reservations_lodging_folio_catalogs_for_property($lodgingFolioCatalogsByProperty, $propertyCodeForAction)
                : reservations_service_catalogs_for_property($serviceCatalogsByProperty, $propertyCodeForAction);
            ...
            if ($actionCode === 'create') {
                if ($targetCatalogId <= 0) {
                    throw new Exception($targetFolioRole === 'lodging'
                        ? 'Selecciona un concepto valido para el folio de hospedaje.'
                        : 'Selecciona un concepto de servicio valido.');
                }
                if (empty($allowedServiceMap)) {
                    throw new Exception($targetFolioRole === 'lodging'
                        ? 'No hay conceptos configurados para agregar al folio de hospedaje en esta propiedad.'
                        : 'No hay conceptos de servicio configurados para esta propiedad.');
                }
                if (!isset($allowedServiceMap[$targetCatalogId])) {
                    throw new Exception($targetFolioRole === 'lodging'
                        ? 'El concepto seleccionado no esta permitido para el folio de hospedaje de esta propiedad.'
                        : 'El concepto seleccionado no esta permitido como servicio para esta propiedad.');
                }
            ...
            if ($actionCode === 'update'
                ...
                if ($protectedPrimaryLodgingId > 0 && $saleId === $protectedPrimaryLodgingId) {
                    ...
                    reservations_delete_sale_item_tree($reservationId, $saleId, $actorUserId, $folioId, 1);
                ...
                reservations_delete_sale_item_tree($reservationId, $saleId, $actorUserId, $folioId, $skipPercentRecalcOnDelete);
    ...
    if ($reservationId > 0 && ($folioId > 0 || $paymentId > 0)) {
        try {
            ...
            if ($actionCode === 'create') {
                ...
                $rootSnapshot = reservations_fetch_sale_item_snapshot($paymentId);
                if (!$rootSnapshot) {
                    throw new Exception('No se encontro el pago a eliminar.');
                }
                $rootReservationId = isset($rootSnapshot['id_reservation']) ? (int)$rootSnapshot['id_reservation'] : 0;
                if ($rootReservationId > 0 && $rootReservationId !== $reservationId) {
                    throw new Exception('El pago no pertenece a la reserva seleccionada.');
                }
                if ($folioId <= 0 && isset($rootSnapshot['id_folio'])) {
                    $folioId = (int)$rootSnapshot['id_folio'];
                }
                reservations_delete_sale_item_tree($reservationId, $paymentId, $actorUserId, $folioId, 1);
            }
...
.reservation-ota-info-card[data-secret-line-items-card] {
  cursor: default;
}
.reservation-ota-info-secret-view {
  display: none;
}
.reservation-ota-info-card.is-secret-active .reservation-ota-info-default-view {
  display: none;
}
.reservation-ota-info-card.is-secret-active .reservation-ota-info-secret-view {
  display: block;
}
.reservation-ota-info-secret-note {
  margin: 0 0 10px;
  font-size: 0.78rem;
  color: #9fb7d1;
}
.reservation-ota-info-secret-wrap {
  max-height: 320px;
}
.reservation-ota-info-secret-table td,
.reservation-ota-info-secret-table th {
  white-space: nowrap;
}
.reservation-ota-info-secret-type {
  font-weight: 700;
  color: #cfe7ff;
}
.reservation-summary-card {
...
foreach ($openReservationIds as $reservationId) {
    ...
    if ($reservationInfoConfigRows) {
        $otaInfoRowIndexByLabel = array();
        $otaInfoRowIndexBySourceLabel = array();
        foreach ($reservationInfoConfigRows as $cfg) {
            ...
            if ($label === '') {
                $label = 'Catalogo #' . $catalogId;
            }
            $labelKey = strtolower($label);
            $defaultLabel = isset($cfg['default_label']) ? trim((string)$cfg['default_label']) : '';
            $itemName = isset($cfg['item_name']) ? trim((string)$cfg['item_name']) : '';
            $sourceLabelKeys = array();
            foreach (array($defaultLabel, $itemName) as $sourceLabelCandidate) {
                $sourceLabelCandidate = trim((string)$sourceLabelCandidate);
                if ($sourceLabelCandidate === '') {
                    continue;
                }
                $sourceLabelKeys[strtolower($sourceLabelCandidate)] = true;
            }
            if (isset($otaInfoRowIndexByLabel[$labelKey])) {
                $existingIdx = (int)$otaInfoRowIndexByLabel[$labelKey];
                $otaInfoRowIndexByCatalog[$catalogId] = $existingIdx;
                if (isset($otaInfoRows[$existingIdx])) {
                    if (!isset($otaInfoRows[$existingIdx]['catalog_ids']) || !is_array($otaInfoRows[$existingIdx]['catalog_ids'])) {
                        $otaInfoRows[$existingIdx]['catalog_ids'] = array();
                    }
                    $otaInfoRows[$existingIdx]['catalog_ids'][$catalogId] = true;
                    if (!isset($otaInfoRows[$existingIdx]['source_labels']) || !is_array($otaInfoRows[$existingIdx]['source_labels'])) {
                        $otaInfoRows[$existingIdx]['source_labels'] = array();
                    }
                    foreach (array_keys($sourceLabelKeys) as $sourceLabelKey) {
                        $otaInfoRows[$existingIdx]['source_labels'][$sourceLabelKey] = true;
                        if (!isset($otaInfoRowIndexBySourceLabel[$sourceLabelKey])) {
                            $otaInfoRowIndexBySourceLabel[$sourceLabelKey] = $existingIdx;
                        }
                    }
                }
                continue;
            ...
            $otaInfoRowIndexByLabel[$labelKey] = count($otaInfoRows);
            foreach (array_keys($sourceLabelKeys) as $sourceLabelKey) {
                if (!isset($otaInfoRowIndexBySourceLabel[$sourceLabelKey])) {
                    $otaInfoRowIndexBySourceLabel[$sourceLabelKey] = count($otaInfoRows);
                }
            }
            $otaInfoRows[] = array(
                'id_line_item_catalog' => $catalogId,
                'catalog_ids' => array($catalogId => true),
                'source_labels' => $sourceLabelKeys,
                'label' => $label,
                'catalog_type' => isset($cfg['catalog_type']) ? (string)$cfg['catalog_type'] : '',
                'item_count' => 0,
                'quantity_total' => 0.0,
                'amount_total_cents' => 0,
                'amount_components_cents' => array(),
                'first_service_date' => '',
                'last_service_date' => ''
            );
        }
        ...
        foreach ($otaInfoSourceRows as $otaInfoRow) {
            if (!reservations_line_item_is_active_for_summary($otaInfoRow, false)) {
                continue;
            }
            if (reservations_line_item_is_payment_like($otaInfoRow)) {
                continue;
            }
            $otaInfoSaleItemId = isset($otaInfoRow['id_sale_item']) ? (int)$otaInfoRow['id_sale_item'] : 0;
            $otaInfoParentSaleId = isset($otaInfoRow['id_parent_sale_item']) ? (int)$otaInfoRow['id_parent_sale_item'] : 0;
            if ($otaInfoSaleItemId > 0 && $otaInfoParentSaleId > 0 && $childContributesToParentTotal($otaInfoParentSaleId, $otaInfoSaleItemId)) {
                continue;
            }
            ...
            $otaInfoSourceItemName = isset($otaInfoRow['item_name']) ? trim((string)$otaInfoRow['item_name']) : '';
            $otaInfoSourceCategory = isset($otaInfoRow['category_name'])
                ? trim((string)$otaInfoRow['category_name'])
                : (isset($otaInfoRow['subcategory_name']) ? trim((string)$otaInfoRow['subcategory_name']) : '');
            $otaInfoSourceDefaultLabel = $otaInfoSourceCategory !== '' && $otaInfoSourceItemName !== ''
                ? ($otaInfoSourceCategory . ' / ' . $otaInfoSourceItemName)
                : $otaInfoSourceItemName;
            $otaInfoMatchedIdx = null;
            foreach (array($otaInfoSourceDefaultLabel, $otaInfoSourceItemName) as $otaInfoSourceLabelCandidate) {
                $otaInfoSourceLabelCandidate = strtolower(trim((string)$otaInfoSourceLabelCandidate));
                if ($otaInfoSourceLabelCandidate === '') {
                    continue;
                }
                if (isset($otaInfoRowIndexBySourceLabel[$otaInfoSourceLabelCandidate])) {
                    $otaInfoMatchedIdx = (int)$otaInfoRowIndexBySourceLabel[$otaInfoSourceLabelCandidate];
                    break;
                }
            }
            if ($otaInfoMatchedIdx === null) {
                if ($catalogId <= 0 || !isset($otaInfoRowIndexByCatalog[$catalogId])) {
                    continue;
                }
                $otaInfoMatchedIdx = (int)$otaInfoRowIndexByCatalog[$catalogId];
            }
            if (!isset($otaInfoRows[$otaInfoMatchedIdx])) {
                continue;
            }
            $otaInfoServiceDate = isset($otaInfoRow['service_date']) ? trim((string)$otaInfoRow['service_date']) : '';
            $otaInfoRows[$otaInfoMatchedIdx]['item_count']++;
            $otaInfoRows[$otaInfoMatchedIdx]['quantity_total'] += (float)(isset($otaInfoRow['quantity']) ? $otaInfoRow['quantity'] : 0);
            $otaInfoAmountCents = $otaInfoSaleItemId > 0
                ? (int)$finalTotalForSaleItem($otaInfoSaleItemId)
                : (int)(isset($otaInfoRow['amount_cents']) ? $otaInfoRow['amount_cents'] : 0);
            $otaInfoRows[$otaInfoMatchedIdx]['amount_total_cents'] += $otaInfoAmountCents;
            if (!isset($otaInfoRows[$otaInfoMatchedIdx]['amount_components_cents']) || !is_array($otaInfoRows[$otaInfoMatchedIdx]['amount_components_cents'])) {
                $otaInfoRows[$otaInfoMatchedIdx]['amount_components_cents'] = array();
            }
            $otaInfoRows[$otaInfoMatchedIdx]['amount_components_cents'][] = $otaInfoAmountCents;
            $serviceDate = $otaInfoServiceDate;
            if ($serviceDate !== '') {
                if ($otaInfoRows[$otaInfoMatchedIdx]['first_service_date'] === '' || $serviceDate < $otaInfoRows[$otaInfoMatchedIdx]['first_service_date']) {
                    $otaInfoRows[$otaInfoMatchedIdx]['first_service_date'] = $serviceDate;
                }
                if ($otaInfoRows[$otaInfoMatchedIdx]['last_service_date'] === '' || $serviceDate > $otaInfoRows[$otaInfoMatchedIdx]['last_service_date']) {
                    $otaInfoRows[$otaInfoMatchedIdx]['last_service_date'] = $serviceDate;
                }
            }
        }
    }
    ...
    $secretLineItemTypeLabels = array(
        'sale_item' => 'Concepto',
        'tax_item' => 'Impuesto',
        'payment' => 'Pago',
    ...
    $secretReservationLineItems = array();
    foreach ($saleItems as $secretSaleItemRow) {
        $secretType = strtolower(trim((string)(isset($secretSaleItemRow['item_type']) ? $secretSaleItemRow['item_type'] : 'sale_item')));
        if ($secretType === '') {
            $secretType = 'sale_item';
        }
        $secretSubcategory = isset($secretSaleItemRow['subcategory_name']) ? trim((string)$secretSaleItemRow['subcategory_name']) : '';
        $secretItemName = isset($secretSaleItemRow['item_name']) ? trim((string)$secretSaleItemRow['item_name']) : '';
        $secretDescription = isset($secretSaleItemRow['description']) ? trim((string)$secretSaleItemRow['description']) : '';
        $secretConceptLabel = $secretSubcategory !== '' && $secretItemName !== ''
            ? ($secretSubcategory . ' / ' . $secretItemName)
            : ($secretItemName !== '' ? $secretItemName : $secretDescription);
        $secretFolioId = isset($secretSaleItemRow['id_folio']) ? (int)$secretSaleItemRow['id_folio'] : 0;
        $secretFolioName = ($secretFolioId > 0 && isset($foliosById[$secretFolioId]['folio_name']))
            ? trim((string)$foliosById[$secretFolioId]['folio_name'])
            : '';
        if ($secretFolioName === '') {
            $secretFolioName = $secretFolioId > 0 ? ('Folio #' . $secretFolioId) : '--';
        }
        $secretReservationLineItems[] = array(
            'sort_type' => isset($secretLineItemTypeOrder[$secretType]) ? (int)$secretLineItemTypeOrder[$secretType] : (int)$secretLineItemTypeOrder['other'],
            'type' => $secretType,
            'type_label' => isset($secretLineItemTypeLabels[$secretType]) ? $secretLineItemTypeLabels[$secretType] : $secretLineItemTypeLabels['other'],
            'concept_label' => $secretConceptLabel !== '' ? $secretConceptLabel : ('Concepto #' . (int)(isset($secretSaleItemRow['id_sale_item']) ? $secretSaleItemRow['id_sale_item'] : 0)),
            'folio_label' => $secretFolioName,
            'service_date' => isset($secretSaleItemRow['service_date']) ? (string)$secretSaleItemRow['service_date'] : '',
            'status' => isset($secretSaleItemRow['status']) ? (string)$secretSaleItemRow['status'] : '',
            'quantity' => isset($secretSaleItemRow['quantity']) ? (float)$secretSaleItemRow['quantity'] : 0,
            'amount_cents' => isset($secretSaleItemRow['amount_cents']) ? (int)$secretSaleItemRow['amount_cents'] : 0,
            'currency' => ($secretFolioId > 0 && isset($foliosById[$secretFolioId]['currency']) && trim((string)$foliosById[$secretFolioId]['currency']) !== '')
                ? (string)$foliosById[$secretFolioId]['currency']
                : $summaryCurrency
        );
    }
    foreach ((array)$payments as $secretPaymentRow) {
        $secretFolioId = isset($secretPaymentRow['id_folio']) ? (int)$secretPaymentRow['id_folio'] : 0;
        $secretFolioName = ($secretFolioId > 0 && isset($foliosById[$secretFolioId]['folio_name']))
            ? trim((string)$foliosById[$secretFolioId]['folio_name'])
            : '';
        if ($secretFolioName === '') {
        ...
        $secretMethod = isset($secretPaymentRow['method']) ? trim((string)$secretPaymentRow['method']) : '';
        $secretReference = isset($secretPaymentRow['reference']) ? trim((string)$secretPaymentRow['reference']) : '';
        $secretConceptLabel = $secretMethod !== '' ? ('Pago / ' . $secretMethod) : 'Pago';
        if ($secretReference !== '') {
            $secretConceptLabel .= ' [' . $secretReference . ']';
        }
        $secretReservationLineItems[] = array(
            'sort_type' => (int)$secretLineItemTypeOrder['payment'],
            'type' => 'payment',
            'type_label' => $secretLineItemTypeLabels['payment'],
            'concept_label' => $secretConceptLabel,
            'folio_label' => $secretFolioName,
            'service_date' => isset($secretPaymentRow['service_date']) ? (string)$secretPaymentRow['service_date'] : '',
            'status' => isset($secretPaymentRow['status']) ? (string)$secretPaymentRow['status'] : '',
            'quantity' => 1,
            'amount_cents' => isset($secretPaymentRow['amount_cents']) ? (int)$secretPaymentRow['amount_cents'] : 0,
            'currency' => ($secretFolioId > 0 && isset($foliosById[$secretFolioId]['currency']) && trim((string)$foliosById[$secretFolioId]['currency']) !== '')
                ? (string)$foliosById[$secretFolioId]['currency']
                : $summaryCurrency
        );
    }
    usort($secretReservationLineItems, function ($a, $b) {
        $typeA = isset($a['sort_type']) ? (int)$a['sort_type'] : 999;
        $typeB = isset($b['sort_type']) ? (int)$b['sort_type'] : 999;
        if ($typeA !== $typeB) {
            return $typeA <=> $typeB;
        }
        $dateA = isset($a['service_date']) ? (string)$a['service_date'] : '';
        $dateB = isset($b['service_date']) ? (string)$b['service_date'] : '';
        if ($dateA !== $dateB) {
            return strcmp($dateA, $dateB);
        }
        $labelA = isset($a['concept_label']) ? (string)$a['concept_label'] : '';
        $labelB = isset($b['concept_label']) ? (string)$b['concept_label'] : '';
    ...
    <?php else: ?>
      ...
      <?php
        ...
        $guestProfileUrl = $hasGuestAssigned
            ? ('index.php?view=guests&guest_id=' . $linkedGuestId . '&guest_return_view=reservations&guest_return_reservation_id=' . (int)$reservationId)
            : '';
      ...
      <div class="reservation-side-stack">
        ...
        <div class="reservation-ota-info-card" data-secret-line-items-card>
          <div class="reservation-ota-info-body">
            <div class="reservation-ota-info-default-view" data-secret-default-view>
                  ...
                  <tbody>
                    <?php foreach ($otaInfoRowsVisible as $otaInfo): ?>
                      <?php
                        ...
                        $amountComponents = isset($otaInfo['amount_components_cents']) && is_array($otaInfo['amount_components_cents'])
                            ? array_values(array_filter($otaInfo['amount_components_cents'], function ($value) {
                                return $value !== null;
                            }))
                            : array();
                        $amountComponentsDisplay = '';
                        if (count($amountComponents) > 1) {
                            $amountComponentsParts = array();
                            foreach ($amountComponents as $componentCents) {
                                $amountComponentsParts[] = reservations_format_money((int)$componentCents, $summaryCurrency);
                            }
                            $amountComponentsDisplay = ' (' . implode(' + ', $amountComponentsParts) . ')';
                        }
                      ?>
                      <tr class="<?php echo $rowClass; ?>">
                        <td><?php echo htmlspecialchars(isset($otaInfo['label']) ? (string)$otaInfo['label'] : 'Concepto', ENT_QUOTES, 'UTF-8'); ?></td>
                        <td><?php echo htmlspecialchars(reservations_format_money($amountTotal, $summaryCurrency) . $amountComponentsDisplay, ENT_QUOTES, 'UTF-8'); ?></td>
                      </tr>
                    <?php endforeach; ?>
                  </tbody>
                </table>
            ...
            <div class="reservation-ota-info-secret-view" data-secret-view hidden>
              <div class="reservation-ota-info-head">
                <span class="reservation-ota-info-title">Line items relacionados</span>
              </div>
              <p class="reservation-ota-info-secret-note">Doble click para volver a la vista normal.</p>
              <?php if (!$secretReservationLineItems): ?>
                <p class="muted">No hay line items relacionados a esta reservaci&oacute;n.</p>
              <?php else: ?>
              <div class="table-scroll reservation-ota-info-secret-wrap">
                <table class="reservation-ota-info-table reservation-ota-info-secret-table">
                  <thead>
                    <tr>
                      <th>Tipo</th>
                      <th>Concepto</th>
                  ...
                  <tbody>
                    <?php foreach ($secretReservationLineItems as $secretRow): ?>
                      <?php
                        $secretQuantity = isset($secretRow['quantity']) ? (float)$secretRow['quantity'] : 0;
                        $secretQuantityDisplay = floor($secretQuantity) == $secretQuantity
                            ? (string)(int)$secretQuantity
                            : rtrim(rtrim(number_format($secretQuantity, 2, '.', ''), '0'), '.');
                      ?>
                      <tr>
                        <td class="reservation-ota-info-secret-type"><?php echo htmlspecialchars((string)$secretRow['type_label'], ENT_QUOTES, 'UTF-8'); ?></td>
                        <td><?php echo htmlspecialchars((string)$secretRow['concept_label'], ENT_QUOTES, 'UTF-8'); ?></td>
                        <td><?php echo htmlspecialchars((string)$secretRow['folio_label'], ENT_QUOTES, 'UTF-8'); ?></td>
                        <td><?php echo htmlspecialchars((string)$secretRow['service_date'], ENT_QUOTES, 'UTF-8'); ?></td>
                        <td><?php echo htmlspecialchars($secretQuantityDisplay, ENT_QUOTES, 'UTF-8'); ?></td>
                        <td><?php echo htmlspecialchars((string)$secretRow['status'], ENT_QUOTES, 'UTF-8'); ?></td>
                        <td><?php echo htmlspecialchars(reservations_format_money((int)$secretRow['amount_cents'], isset($secretRow['currency']) ? (string)$secretRow['currency'] : $summaryCurrency), ENT_QUOTES, 'UTF-8'); ?></td>
                      </tr>
                    <?php endforeach; ?>
                  </tbody>
                </table>
            ...
            <div class="folio-charge-tabs" data-reservation-tabs>
              ...
              <?php foreach ($folioDisplayRows as $idx => $folioInfo): ?>
                <?php
                  ...
                  $folioMetricSummary = isset($displayMetricsByFolio[$fid]) ? $displayMetricsByFolio[$fid] : array();
                  $folioChargeTotalCents = isset($folioMetricSummary['charges_cents']) ? (int)$folioMetricSummary['charges_cents'] : 0;
                  $folioPaymentsTotalCents = isset($folioMetricSummary['payments_cents']) ? (int)$folioMetricSummary['payments_cents'] : 0;
                ?>
                  ...
                  <div class="folio-section">
                    <div class="folio-section-header">
                      ...
                      <div class="folio-section-header-actions">
                        <span class="balance-item">Cobros: <strong><?php echo reservations_format_money($folioChargeTotalCents, $folioCurrency); ?></strong></span>
                        <span class="balance-item">Pagos: <strong><?php echo reservations_format_money($folioPaymentsTotalCents, $folioCurrency); ?></strong></span>
                        <?php if ($folioRole === 'services' || $folioRole === 'lodging'): ?>
                          <button
                            type="button"
                            ...
                            data-service-scope="<?php echo $folioRole === 'lodging' ? 'lodging-folio' : 'services'; ?>"
                          ><?php echo $folioRole === 'lodging' ? 'Nuevo ajuste' : 'Nuevo servicio'; ?></button>
                        <?php endif; ?>
...
$lodgingFolioCatalogMapJson = json_encode($lodgingFolioCatalogsByProperty, JSON_UNESCAPED_UNICODE);
...
window.pmsReservationLodgingFolioCatalogMap = <?php echo $lodgingFolioCatalogMapJson ? $lodgingFolioCatalogMapJson : '{}'; ?>;
...
document.addEventListener('click', function (ev) {
  ...
  var serviceScope = serviceBtn.getAttribute('data-service-scope') || 'services';
  var serviceCatalogMap = (serviceScope === 'lodging-folio'
    ? (window.pmsReservationLodgingFolioCatalogMap && typeof window.pmsReservationLodgingFolioCatalogMap === 'object'
      ? window.pmsReservationLodgingFolioCatalogMap
      : {})
  ...
  pmsReservationOpenServiceLightbox({
    ...
    enableMarkPaid: true,
    serviceCatalogMap: serviceCatalogMap
  });
});
...
(function () {
  document.querySelectorAll('[data-secret-line-items-card]').forEach(function (card) {
    var defaultView = card.querySelector('[data-secret-default-view]');
    var secretView = card.querySelector('[data-secret-view]');
    if (!defaultView || !secretView) return;

    function setSecretMode(on) {
      card.classList.toggle('is-secret-active', on);
      defaultView.hidden = !!on;
      secretView.hidden = !on;
    }
    ...
    card.addEventListener('dblclick', function (ev) {
      if (
        ev.target.closest('a, button, input, select, textarea, label, form, summary, details')
      ) {
        return;
      }
      setSecretMode(!card.classList.contains('is-secret-active'));
    });
  });
})();
