...
pms_require_permission('sale_items.view');
...
$lineItemFilters = array(
    ...
    'catalog_id' => isset($_POST['sale_items_line_catalog_id']) ? (int)$_POST['sale_items_line_catalog_id'] : 0,
...
if ($lineItemFilters['catalog_id'] < 0) {
    $lineItemFilters['catalog_id'] = 0;
}
...
if ($lineItemFilters['catalog_type'] === 'none' && $lineItemFilters['catalog_id'] > 0) {
    $lineItemFilters['catalog_type'] = '';
}
...
$itemMessage = null;
$itemError   = null;
if (in_array($action, array('create_category', 'create_item', 'clone_item'), true)) {
    pms_require_permission('sale_items.create');
} elseif (in_array($action, array('update_category', 'delete_category', 'update_item', 'delete_item', 'update_line_item_type'), true)) {
    pms_require_permission('sale_items.edit');
} elseif (in_array($action, array('update_child_relation', 'update_child_links'), true)) {
    pms_require_permission('sale_items.relations_edit');
}
...
try {
    ...
    $sql = 'SELECT li.id_line_item, rel.id_parent_sale_item, li.id_folio, f.id_reservation, li.service_date, '
        ...
        . 'COALESCE(li.item_name, lic.item_name, \'\') AS item_name, cat.category_name, p.code AS property_code, p.name AS property_name, '
    ...
    if ($lineItemFilters['catalog_id'] > 0) {
        $sql .= 'AND li.id_line_item_catalog = ? ';
        $params[] = (int)$lineItemFilters['catalog_id'];
    }
    ...
    if ($lineItemFilters['search'] !== '') {
        $sql .= 'AND (COALESCE(li.item_name, lic.item_name, \'\') LIKE ? OR cat.category_name LIKE ? OR r.code LIKE ? OR f.folio_name LIKE ? OR g.names LIKE ? OR g.last_name LIKE ? OR rm.code LIKE ? OR rm.name LIKE ? OR li.item_type LIKE ? OR li.method LIKE ? OR li.reference LIKE ? OR li.description LIKE ?) ';
...
$lineItemCatalogOptions = array();
try {
    $catalogOptionSets = pms_call_procedure('sp_sale_item_catalog_data', array(
        $companyCode,
        $lineItemFilters['property'] === '' ? null : $lineItemFilters['property'],
        1,
        0,
        0
    ));
    $lineItemCatalogOptions = isset($catalogOptionSets[0]) ? $catalogOptionSets[0] : array();
} catch (Exception $e) {
    $lineItemCatalogOptions = array();
}
if (!$lineItemCatalogOptions) {
    $catalogOptionSets = sale_items_catalog_data_fallback(
        $companyId,
        $lineItemFilters['property'] === '' ? null : $lineItemFilters['property'],
        1,
        0,
...
if ($lineItemCatalogOptions) {
    usort($lineItemCatalogOptions, function ($a, $b) {
        $aLabel = strtolower(trim((string)($a['item_name'] ?? '')));
        $bLabel = strtolower(trim((string)($b['item_name'] ?? '')));
        if ($aLabel === $bLabel) {
            return strcmp((string)($a['property_code'] ?? ''), (string)($b['property_code'] ?? ''));
        }
...
function sale_items_render_filter_hiddens($filters, $conceptFilters, $lineItemFilters) {
    ...
    echo '<input type="hidden" name="sale_items_line_catalog_id" value="' . (int)$lineItemFilters['catalog_id'] . '">';
...
<div class="panel">
  <div class="panel-header spaced">
    ...
    <form method="post" class="form-inline">
      ...
      <label>Cat&aacute;logo espec&iacute;fico
        <select name="sale_items_line_catalog_id">
          <option value="0">(Todos)</option>
          <?php foreach ($lineItemCatalogOptions as $catalogOpt):
            $catalogId = isset($catalogOpt['id_sale_item_catalog']) ? (int)$catalogOpt['id_sale_item_catalog'] : 0;
            if ($catalogId <= 0) { continue; }
            $catalogLabel = sale_items_parent_label($catalogOpt, $categoriesById);
            $catalogTypeRaw = isset($catalogOpt['catalog_type']) ? (string)$catalogOpt['catalog_type'] : '';
            list($catalogTypeLabel, $catalogTypeClassIgnore) = sale_items_catalog_type_meta($catalogTypeRaw);
            if ($catalogTypeLabel !== '') {
              $catalogLabel .= ' | ' . $catalogTypeLabel;
            }
          ?>
            <option value="<?php echo (int)$catalogId; ?>" <?php echo (int)$lineItemFilters['catalog_id'] === $catalogId ? 'selected' : ''; ?>>
              <?php echo htmlspecialchars($catalogLabel, ENT_QUOTES, 'UTF-8'); ?>
            </option>
          <?php endforeach; ?>
        </select>
      </label>
    ...
    <div class="card" style="margin-bottom:12px;">
      ...
      <?php else: ?>
        ...
        <div class="grid-3" style="margin-top:8px;">
          ...
          <div><span class="muted">Precio unitario</span><div><?php echo htmlspecialchars(sale_items_format_money((int)$lineItemViewDetail['unit_price_cents'], $vdCurrency), ENT_QUOTES, 'UTF-8'); ?></div></div>
