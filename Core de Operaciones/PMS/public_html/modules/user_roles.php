...
} elseif ($action === 'duplicate_role') {
    $sourceRoleId = isset($_POST['target_role_id']) ? (int)$_POST['target_role_id'] : 0;
    if ($sourceRoleId <= 0) {
        $error = 'Selecciona un rol valido para duplicar.';
    } else {
        try {
            $sourceRole = pms_user_roles_load_role_detail_for_company($companyId, $sourceRoleId);
            if (!$sourceRole) {
                $error = 'No se encontro el rol origen.';
            } else {
                $sourceName = trim((string)(isset($sourceRole['name']) ? $sourceRole['name'] : 'Rol'));
                $sourcePropertyCode = trim((string)(isset($sourceRole['property_code']) ? $sourceRole['property_code'] : ''));
                $scopeFilter = $sourcePropertyCode === '' ? '__GLOBAL__' : $sourcePropertyCode;
                $rolesInScope = pms_user_roles_load_role_catalog_for_company($companyId, '', $scopeFilter);
                $taken = array();
                foreach ($rolesInScope as $scopeRoleRow) {
                    $existingName = strtolower(trim((string)(isset($scopeRoleRow['name']) ? $scopeRoleRow['name'] : '')));
                    if ($existingName !== '') {
                        $taken[$existingName] = true;
                    }
                }
                $baseDuplicateName = $sourceName . ' (copia)';
                $newRoleName = $baseDuplicateName;
                $suffix = 2;
                while (isset($taken[strtolower($newRoleName)])) {
                    $newRoleName = $baseDuplicateName . ' ' . $suffix;
                    $suffix++;
                }

                $sourcePermissions = pms_user_roles_load_role_permission_codes($sourceRoleId);
                $sourceDescription = isset($sourceRole['description']) ? trim((string)$sourceRole['description']) : '';
                $sourceIsActive = isset($sourceRole['is_active']) ? (int)$sourceRole['is_active'] : 1;

                $copySets = pms_call_procedure('sp_role_upsert', array(
                    $companyCode,
                    0,
                    $sourcePropertyCode === '' ? null : $sourcePropertyCode,
                    $newRoleName,
                ...
                $savedCopy = isset($copySets[0][0]) ? $copySets[0][0] : null;
                if (!$savedCopy || !isset($savedCopy['id_role'])) {
                    $error = 'No se pudo duplicar el rol.';
                } else {
                    $selectedRoleId = (int)$savedCopy['id_role'];
                    pms_call_procedure('sp_role_permission_sync', array(
                        $companyCode,
                        $selectedRoleId,
                        implode(',', $sourcePermissions),
                        $actorUserId
                    ));
                    $message = 'Rol duplicado: ' . $newRoleName;
                }
            }
        } catch (Exception $e) {
            ...
            <tr class="<?php echo $isSelected ? 'is-selected' : ''; ?>">
              ...
              <td>
                <form method="post">
                  ...
                  <input type="hidden" name="roles_filter_search" value="<?php echo htmlspecialchars($filters['search'], ENT_QUOTES, 'UTF-8'); ?>">
                  <input type="hidden" name="roles_filter_property" value="<?php echo htmlspecialchars($filters['property_code'], ENT_QUOTES, 'UTF-8'); ?>">
                  <button type="submit" name="user_roles_action" value="select_role_editor">Editar</button>
                  <button type="submit" name="user_roles_action" value="duplicate_role">Duplicar</button>
                </form>
              </td>
            </tr>
          <?php endforeach; ?>
