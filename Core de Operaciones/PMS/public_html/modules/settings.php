...
if (!function_exists('settings_column_exists')) {
    function settings_column_exists(PDO $pdo, $tableName, $columnName)
    {
        $tableName = trim((string)$tableName);
...
if (!function_exists('settings_google_drive_columns_ready')) {
    function settings_google_drive_columns_ready(PDO $pdo)
    {
        $required = array(
        ...
        foreach ($required as $columnName) {
            if (!settings_column_exists($pdo, 'pms_settings', $columnName)) {
                return false;
            }
        }
        return true;
...
if (!function_exists('settings_fetch_google_drive_config')) {
    function settings_fetch_google_drive_config(PDO $pdo, $companyId)
    {
        $config = array(
        ...
        if (!settings_google_drive_columns_ready($pdo)) {
            return $config;
        }
        $stmt = $pdo->prepare(
            'SELECT
        ...
        if ($row) {
            $config['google_drive_enabled'] = !empty($row['google_drive_enabled']) ? 1 : 0;
            $config['google_drive_client_id'] = isset($row['google_drive_client_id']) ? (string)$row['google_drive_client_id'] : '';
            $config['google_drive_client_secret'] = isset($row['google_drive_client_secret']) ? (string)$row['google_drive_client_secret'] : '';
            $config['google_drive_refresh_token'] = isset($row['google_drive_refresh_token']) ? (string)$row['google_drive_refresh_token'] : '';
            $config['google_drive_folder_id'] = isset($row['google_drive_folder_id']) ? (string)$row['google_drive_folder_id'] : '';
            $config['google_drive_spreadsheet_id'] = isset($row['google_drive_spreadsheet_id']) ? (string)$row['google_drive_spreadsheet_id'] : '';
        }
        return $config;
    }
}
...
if (!function_exists('settings_normalize_icon_html')) {
    function settings_normalize_icon_html($value)
    {
        $value = trim((string)$value);
        if ($value === '') {
            return '';
        }
        if (preg_match('/^(&#x?[0-9A-Fa-f]+;|&[A-Za-z][A-Za-z0-9]+;)$/', $value)) {
            return $value;
        }
...
if (isset($_POST['settings_action']) && $_POST['settings_action'] === 'save_lodging') {
    ...
    $iconMapRaw = isset($_POST['setting_lodging_icon_html']) && is_array($_POST['setting_lodging_icon_html'])
        ? $_POST['setting_lodging_icon_html']
        : array();
    try {
        ...
        if (settings_column_exists($pdoSettings, 'pms_settings_lodging_catalog', 'icon_html')) {
            ...
            if ($propCode !== '') {
                ...
                if ($scopePropertyId <= 0) {
                    throw new RuntimeException('No se encontro la propiedad para guardar iconos de hospedaje.');
                }
            }
            $stmtIcon = $pdoSettings->prepare(
                'UPDATE pms_settings_lodging_catalog
                    SET icon_html = ?
                  WHERE id_company = ?
                    AND id_sale_item_catalog = ?
                    AND deleted_at IS NULL
                    AND COALESCE(is_active, 1) = 1
                    AND ((? IS NULL AND id_property IS NULL) OR id_property = ?)'
            );
            foreach ($catalogIds as $catalogId) {
                $iconHtml = settings_normalize_icon_html(isset($iconMapRaw[$catalogId]) ? $iconMapRaw[$catalogId] : '');
                $stmtIcon->execute(array(
                    $iconHtml !== '' ? $iconHtml : null,
                    $companyId,
                    (int)$catalogId,
                    $scopePropertyId > 0 ? $scopePropertyId : null,
                    $scopePropertyId > 0 ? $scopePropertyId : null
                ));
            }
        }
...
if (isset($_POST['settings_action']) && $_POST['settings_action'] === 'save_lodging_folio_concepts') {
    $propCode = isset($_POST['settings_filter_property']) ? strtoupper((string)$_POST['settings_filter_property']) : '';
    if ($propCode !== '') {
        pms_require_property_access($propCode);
    }
    $catalogIds = isset($_POST['setting_lodging_folio_catalog_ids']) ? $_POST['setting_lodging_folio_catalog_ids'] : array();
    if (!is_array($catalogIds)) {
        $catalogIds = array();
    }
    $catalogIds = array_values(array_filter(array_map('intval', $catalogIds), function ($id) {
    ...
    try {
        ...
        $stmtExists->execute();
        if (((int)$stmtExists->fetchColumn()) <= 0) {
            throw new RuntimeException('Missing table pms_settings_lodging_folio_catalog. Run table SQL first.');
        }
        ...
        if ($propCode !== '') {
            ...
            $scopePropertyId = (int)$stmtProperty->fetchColumn();
            if ($scopePropertyId <= 0) {
                throw new RuntimeException('Unknown property for lodging folio settings.');
            }
        }
        ...
        if ($scopePropertyId > 0) {
            $stmtDeactivate = $pdoSettings->prepare(
                'UPDATE pms_settings_lodging_folio_catalog
                    SET is_active = 0,
                        deleted_at = NOW(),
                        updated_at = NOW()
                  WHERE id_company = ?
        ...
        if (!empty($catalogIds)) {
            $catalogCsv = implode(',', $catalogIds);
            $stmtInsert = $pdoSettings->prepare(
                'INSERT INTO pms_settings_lodging_folio_catalog (
                    id_company,
                    id_property,
                    id_sale_item_catalog,
                    is_active,
        ...
        $message = 'Conceptos del folio de hospedaje guardados.';
    } catch (Exception $e) {
        if (isset($startedTx) && $startedTx && isset($pdoSettings) && $pdoSettings instanceof PDO && $pdoSettings->inTransaction()) {
            $pdoSettings->rollBack();
        }
...
if (isset($_POST['settings_action']) && $_POST['settings_action'] === 'save_google_drive_export') {
    try {
        $pdo = pms_get_connection();
        if (!settings_google_drive_columns_ready($pdo)) {
            throw new RuntimeException('Faltan columnas en pms_settings. Aplica bd pms/migrate_pms_settings_google_drive.sql primero.');
        }

        $googleDriveEnabled = isset($_POST['settings_google_drive_enabled']) ? 1 : 0;
        $googleDriveClientId = trim((string)(isset($_POST['settings_google_drive_client_id']) ? $_POST['settings_google_drive_client_id'] : ''));
        $googleDriveClientSecret = trim((string)(isset($_POST['settings_google_drive_client_secret']) ? $_POST['settings_google_drive_client_secret'] : ''));
        $googleDriveRefreshToken = trim((string)(isset($_POST['settings_google_drive_refresh_token']) ? $_POST['settings_google_drive_refresh_token'] : ''));
        $googleDriveFolderId = trim((string)(isset($_POST['settings_google_drive_folder_id']) ? $_POST['settings_google_drive_folder_id'] : ''));
        $googleDriveSpreadsheetId = trim((string)(isset($_POST['settings_google_drive_spreadsheet_id']) ? $_POST['settings_google_drive_spreadsheet_id'] : ''));
        ...
        $stmtExisting->execute(array($companyId));
        $existingSettingId = (int)$stmtExisting->fetchColumn();

        if ($existingSettingId > 0) {
            $stmtUpdate = $pdo->prepare(
                'UPDATE pms_settings
                    SET google_drive_enabled = ?,
                        google_drive_client_id = ?,
                        google_drive_client_secret = ?,
                        google_drive_refresh_token = ?,
                        google_drive_folder_id = ?,
            ...
            $stmtUpdate->execute(array(
                ...
                $googleDriveSpreadsheetId !== '' ? $googleDriveSpreadsheetId : null,
                $existingSettingId,
            ));
        } else {
            $stmtInsert = $pdo->prepare(
        ...
        $message = 'Configuracion de Google Drive / Sheets guardada.';
    } catch (Exception $e) {
        $error = 'No se pudo guardar la configuracion de Google Drive / Sheets: ' . $e->getMessage();
    }
}
...
$googleDriveSettingsReady = settings_google_drive_columns_ready(pms_get_connection());
$currentGoogleDriveConfig = settings_fetch_google_drive_config(pms_get_connection(), $companyId);
...
$selectedLodgingIconMap = array();
try {
    $pdoLodgingIcons = pms_get_connection();
    if (settings_column_exists($pdoLodgingIcons, 'pms_settings_lodging_catalog', 'icon_html')) {
        if ($filters['property_code'] !== '') {
            $stmtLodgingIcons = $pdoLodgingIcons->prepare(
                'SELECT pslc.id_sale_item_catalog, COALESCE(pslc.icon_html, \'\') AS icon_html
            ...
            $stmtLodgingIcons->execute(array($companyId, $companyId, strtoupper((string)$filters['property_code'])));
        } else {
            $stmtLodgingIcons = $pdoLodgingIcons->prepare(
                'SELECT pslc.id_sale_item_catalog, COALESCE(pslc.icon_html, \'\') AS icon_html
                   FROM pms_settings_lodging_catalog pslc
                  WHERE pslc.id_company = ?
                    AND pslc.id_property IS NULL
                    AND pslc.deleted_at IS NULL
                    AND COALESCE(pslc.is_active, 1) = 1'
            );
            $stmtLodgingIcons->execute(array($companyId));
        }
        foreach ($stmtLodgingIcons->fetchAll(PDO::FETCH_ASSOC) as $iconRow) {
            $catalogId = isset($iconRow['id_sale_item_catalog']) ? (int)$iconRow['id_sale_item_catalog'] : 0;
            if ($catalogId <= 0) {
                continue;
            }
            $selectedLodgingIconMap[$catalogId] = settings_normalize_icon_html(isset($iconRow['icon_html']) ? $iconRow['icon_html'] : '');
        }
    }
} catch (Exception $e) {
    $selectedLodgingIconMap = array();
...
$selectedLodgingFolioCatalogIds = array();
try {
    $pdoFallback = pms_get_connection();
    $stmtExists = $pdoFallback->prepare(
        'SELECT COUNT(*)
    ...
    $stmtExists->execute();
    $hasLodgingFolioCatalogTable = ((int)$stmtExists->fetchColumn()) > 0;
    if ($hasLodgingFolioCatalogTable) {
        if ($filters['property_code'] !== '') {
            $stmtFallback = $pdoFallback->prepare(
                'SELECT DISTINCT s.id_sale_item_catalog
        ...
        $selectedLodgingFolioCatalogIds = array_values(array_filter(array_map('intval', $stmtFallback->fetchAll(PDO::FETCH_COLUMN)), function ($id) {
            return $id > 0;
        }));
    }
} catch (Exception $e) {
...
$selectedLodgingFolioCatalogMap = array();
foreach ($selectedLodgingFolioCatalogIds as $sid) {
    $selectedLodgingFolioCatalogMap[(int)$sid] = true;
}
...
<div class="panel settings-tab-panel" data-settings-tab-panel="general">
  ...
  <p class="muted">Aqui se guardan las credenciales para dejar lista la exportacion directa futura de reportes hacia Google Drive o Google Sheets.</p>
  <?php if (!$googleDriveSettingsReady): ?>
    <p class="error">Faltan columnas en <code>pms_settings</code>. Aplica primero <code>bd pms/migrate_pms_settings_google_drive.sql</code>.</p>
  <?php else: ?>
    <form method="post" class="form-grid grid-2">
      <input type="hidden" name="settings_action" value="save_google_drive_export">
      <label class="full">
        <span>Integracion habilitada</span>
        <input type="checkbox" name="settings_google_drive_enabled" value="1" <?php echo !empty($currentGoogleDriveConfig['google_drive_enabled']) ? 'checked' : ''; ?>>
      </label>
      <label class="full">Client ID
        <input type="text" name="settings_google_drive_client_id" value="<?php echo htmlspecialchars((string)$currentGoogleDriveConfig['google_drive_client_id'], ENT_QUOTES, 'UTF-8'); ?>" placeholder="Google OAuth Client ID">
      </label>
      <label class="full">Client secret
        <textarea name="settings_google_drive_client_secret" rows="3" placeholder="Google OAuth Client secret"><?php echo htmlspecialchars((string)$currentGoogleDriveConfig['google_drive_client_secret'], ENT_QUOTES, 'UTF-8'); ?></textarea>
      </label>
      <label class="full">Refresh token
        <textarea name="settings_google_drive_refresh_token" rows="4" placeholder="Refresh token de Google OAuth"><?php echo htmlspecialchars((string)$currentGoogleDriveConfig['google_drive_refresh_token'], ENT_QUOTES, 'UTF-8'); ?></textarea>
      </label>
      <label>Folder ID
        <input type="text" name="settings_google_drive_folder_id" value="<?php echo htmlspecialchars((string)$currentGoogleDriveConfig['google_drive_folder_id'], ENT_QUOTES, 'UTF-8'); ?>" placeholder="Opcional: carpeta destino en Drive">
      </label>
      <label>Spreadsheet ID
        <input type="text" name="settings_google_drive_spreadsheet_id" value="<?php echo htmlspecialchars((string)$currentGoogleDriveConfig['google_drive_spreadsheet_id'], ENT_QUOTES, 'UTF-8'); ?>" placeholder="Opcional: hoja destino en Sheets">
      </label>
      <div class="form-actions full">
        <button type="submit">Guardar Google Drive / Sheets</button>
      </div>
    </form>
  <?php endif; ?>
</div>
  ...
  <form method="post" class="form-grid grid-2">
    ...
    <div class="concept-checklist full">
      ...
      <?php else: ?>
        ...
        ?>
          <div class="settings-lodging-row">
            ...
            <label class="settings-lodging-icon-field">
              <span>Icono calendario</span>
              <input
                type="text"
                name="setting_lodging_icon_html[<?php echo $id; ?>]"
                value="<?php echo htmlspecialchars(isset($selectedLodgingIconMap[$id]) ? (string)$selectedLodgingIconMap[$id] : '', ENT_QUOTES, 'UTF-8'); ?>"
                maxlength="12"
                placeholder="🏠 o &#127969;">
            </label>
          </div>
...
<div class="panel settings-tab-panel" data-settings-tab-panel="concepts">
  <h3>Conceptos para agregar al folio de hospedaje</h3>
  <form method="post" class="form-grid grid-2">
    <input type="hidden" name="settings_action" value="save_lodging_folio_concepts">
    <input type="hidden" name="settings_filter_property" value="<?php echo htmlspecialchars($filters['property_code'], ENT_QUOTES, 'UTF-8'); ?>">
    <label class="full">Conceptos permitidos para cargos y ajustes manuales en folio de hospedaje</label>
    <div class="concept-checklist full">
      <?php if (!$lodgingFolioConcepts): ?>
        <p class="muted">Sin conceptos disponibles.</p>
      <?php else: ?>
        <?php foreach ($lodgingFolioConcepts as $c):
          $id = (int)$c['id_sale_item_catalog'];
          $label = (string)$c['item_name'];
          $cat = isset($c['category']) ? (string)$c['category'] : '';
          $prop = isset($c['property_code']) ? (string)$c['property_code'] : '';
          $full = ($prop !== '' ? $prop . ' - ' : '') . ($cat !== '' ? $cat . ' / ' : '') . $label;
          $checked = isset($selectedLodgingFolioCatalogMap[$id]) ? 'checked' : '';
        ?>
          <label class="checkbox-inline">
            <input type="checkbox" name="setting_lodging_folio_catalog_ids[]" value="<?php echo $id; ?>" <?php echo $checked; ?>>
            <?php echo htmlspecialchars($full, ENT_QUOTES, 'UTF-8'); ?>
          </label>
        <?php endforeach; ?>
      <?php endif; ?>
...
.settings-lodging-row { display: grid; gap: 8px; border: 1px solid rgba(255,255,255,0.08); border-radius: 8px; padding: 10px; }
.settings-lodging-icon-field { display: grid; gap: 6px; }
.settings-lodging-icon-field span { font-size: 0.78rem; color: #9fb7d5; }
.settings-lodging-icon-field input { max-width: 170px; }
