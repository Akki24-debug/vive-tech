...
pms_require_permission('ota_ical.view');
...
if (in_array($action, array('save_feed', 'delete_feed'), true)) {
    pms_require_permission('ota_ical.edit');
} elseif (in_array($action, array('sync_feed', 'sync_property'), true)) {
    pms_require_permission('ota_ical.sync');
}
if ($action !== '') {
    try {
        if ($action === 'save_feed') {
