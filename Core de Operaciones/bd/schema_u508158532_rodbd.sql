...
CREATE TABLE `ota_account` (
  ...
  `price_adjustment_mode` enum('none','percent','fixed') NOT NULL DEFAULT 'none',
  `price_adjustment_value` decimal(12,3) DEFAULT NULL,
...
--
-- Table structure for table `ota_price_override`
--

CREATE TABLE `ota_price_override` (
  `id_ota_price_override` bigint(20) NOT NULL,
  `id_ota_account` bigint(20) NOT NULL,
  `id_category` bigint(20) DEFAULT NULL,
...
CREATE TABLE `pms_settings_lodging_catalog` (
  ...
  `id_property` bigint(20) DEFAULT NULL,
  `id_sale_item_catalog` bigint(20) NOT NULL,
  `icon_html` varchar(32) DEFAULT NULL,
  `is_active` tinyint(1) NOT NULL DEFAULT 1,
  `deleted_at` datetime DEFAULT NULL,
  `created_at` datetime NOT NULL DEFAULT current_timestamp(),
  `created_by` bigint(20) DEFAULT NULL,
...
ALTER TABLE `ota_account`
  ...
  ADD KEY `idx_ota_account_price_adjustment` (`price_adjustment_mode`,`is_active`,`deleted_at`),
...
--
-- Indexes for table `ota_price_override`
--
ALTER TABLE `ota_price_override`
  ADD PRIMARY KEY (`id_ota_price_override`),
  ADD KEY `idx_ota_price_override_lookup` (`id_ota_account`,`override_date`,`is_active`),
  ADD KEY `idx_ota_price_override_category` (`id_category`,`override_date`),
  ADD KEY `idx_ota_price_override_room` (`id_room`,`override_date`);
...
--
-- AUTO_INCREMENT for table `ota_price_override`
--
ALTER TABLE `ota_price_override`
  MODIFY `id_ota_price_override` bigint(20) NOT NULL AUTO_INCREMENT;
...
--
-- Constraints for table `ota_price_override`
--
ALTER TABLE `ota_price_override`
  ADD CONSTRAINT `fk_ota_price_override_category` FOREIGN KEY (`id_category`) REFERENCES `roomcategory` (`id_category`) ON DELETE SET NULL ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_ota_price_override_ota_account` FOREIGN KEY (`id_ota_account`) REFERENCES `ota_account` (`id_ota_account`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_ota_price_override_room` FOREIGN KEY (`id_room`) REFERENCES `room` (`id_room`) ON DELETE SET NULL ON UPDATE CASCADE;
